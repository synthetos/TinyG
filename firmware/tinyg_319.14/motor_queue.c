...
// The mq buffer is typically set to 3 just so the steppers don't starve.
// It can be as large as 255, but there's no point and it only introduces 
// a control lag you don't want. 
#define MQ_BUFFER_SIZE 3 			// limited to 255 
...
uint8_t mq_queue_line(double steps[], double microseconds)
{
	...
	// trap a variety of conditions that would prevent queueing the line
	if (isfinite(microseconds) == FALSE) {
		TRAP(PSTR("Trapped infinite time move in mp_queue_line()"));
		return (TG_ZERO_LENGTH_MOVE);
	}
	if (microseconds < EPSILON) {
		TRAP(PSTR("Trapped zero time move in mp_queue_line()"));
		return (TG_ZERO_LENGTH_MOVE);
	}
