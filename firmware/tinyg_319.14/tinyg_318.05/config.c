...
enum cfgMnemonics { P_, V_,		// <-- profile and version records
	...
	MO, SR, FR, TH, TS, 
	JM, JS, RA, LI, HE, 
	HR, HC, HO, HB,				// 14 axis settings (adjust #define)
	...
	GU, GL, GP, GD, EA, 
	MM, MT, HM, EC, IC, 
	IL, EX, EE,					// 13 general settings (adjust #define)
...
static void _appmmJS(struct cfgSetting *s);
...
static void _appinJS(struct cfgSetting *s);
...
char fmmJS[] PROGMEM = "Jerk stall velocity%5.0f mm/min       $%c%s%1.0f\n";
...
char finJS[] PROGMEM = "Jerk stall velocity%5.2f in/min        $%c%s%1.0f\n";
...
char fdgJS[] PROGMEM = "Jerk stall veloc%8.0f deg/min      $%c%s%1.0f\n";
...
struct cfgSetting cfgList[COUNT_SETTINGS] = { 
	...
	{ CFG_X, JS, fmmJS, finJS, _appmmJS, _appinJS, (double) X_JERK_STALL_VELOCITY },
	...
	{ CFG_Y, JS, fmmJS, finJS, _appmmJS, _appinJS, (double) Y_JERK_STALL_VELOCITY },
	...
	{ CFG_Z, JS, fmmJS, finJS, _appmmJS, _appinJS, (double) Z_JERK_STALL_VELOCITY },
	...
	{ CFG_A, JS, fdgJS, fdgJS, _appdgJS, _appdgJS, (double) A_JERK_STALL_VELOCITY },
	...
	{ CFG_B, JS, fdgJS, fdgJS, _appdgJS, _appdgJS, (double) B_JERK_STALL_VELOCITY },
	...
	{ CFG_C, JS, fdgJS, fdgJS, _appdgJS, _appdgJS, (double) C_JERK_STALL_VELOCITY },
...
static void _appmmJM(struct cfgSetting *s) { 
	cfg.a[s->grp].jerk_max = s->value; 
	cfg.a[s->grp].jerk_transition_size = mp_get_jerk_transition_size
										 (cfg.a[s->grp].jerk_stall_velocity, 
										  cfg.a[s->grp].jerk_max);
}
static void _appmmJS(struct cfgSetting *s) { 
	cfg.a[s->grp].jerk_stall_velocity = s->value; 
	cfg.a[s->grp].jerk_transition_size = mp_get_jerk_transition_size
										 (cfg.a[s->grp].jerk_stall_velocity, 
										  cfg.a[s->grp].jerk_max);
}
...
static void _appinJS(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmJS(s); }
