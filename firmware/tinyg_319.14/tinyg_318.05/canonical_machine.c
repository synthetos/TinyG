...
/* 
 * cm_set_target() - set target ector in GM model
 *
 ...
 *	Axes that need processing are signaled in flags[]
...
{ 
	...
	for (i=X; i<=Z; i++) {
		if ((flag[i] < EPSILON) || (cfg.a[i].axis_mode == AXIS_DISABLED)) {
			...
			TRAP1(PSTR("%c axis using unsupported axis mode"), cfg_get_configuration_group_char(i));
		}
	}
...
{ 
	if ((status == TG_OK) || (status == TG_EAGAIN)) {
		mp_copy_vector(gm.position, gm.target, AXES);
	}
}
 ...
 * rates and increase the time to accommodate the rate limiting axis. 
 * The axis modes are taken into account.
 *
...
uint8_t cm_set_origin_offsets(double offset[])
{
	...
	mp_copy_vector(gm.target, gm.position, AXES);
 ...
 * The original source for these routines is from the grbl project
 */
