...
static void _print_str(const INDEX_T i);				// pribnt a string value
...
static uint8_t _get_coor(const INDEX_T i, cmdObj *cmd);
static uint8_t _get_motm(const INDEX_T i, cmdObj *cmd);
static uint8_t _get_plan(const INDEX_T i, cmdObj *cmd);
static uint8_t _get_path(const INDEX_T i, cmdObj *cmd);
static uint8_t _get_dist(const INDEX_T i, cmdObj *cmd);
...
static uint8_t _get_mpos(const INDEX_T i, cmdObj *cmd);	// get current machine position
...
// status report variables ({"sr":""}  and ? command)
...
char str_gc[] PROGMEM = "gc,gcod,[gc]";	 // gcode power-on reset defaults
...
//	 string *, print func, get func, set func  target for get/set,    default value
	...
	{ str_stat,_print_str, _get_stat,_set_nul, (double *)&tg.null, 0 },	// machine state
	...
	{ str_motm,_print_str, _get_motm,_set_nul, (double *)&tg.null, 0 },	// motion mode
	...
	{ str_posx,_print_lin, _get_pos, _set_nul, (double *)&tg.null, 0 },	// X position
	{ str_posy,_print_lin, _get_pos, _set_nul, (double *)&tg.null, 0 },	// Y position
	{ str_posz,_print_lin, _get_pos, _set_nul, (double *)&tg.null, 0 },	// Z position
	{ str_posa,_print_rot, _get_pos, _set_nul, (double *)&tg.null, 0 },	// A position
	{ str_posb,_print_rot, _get_pos, _set_nul, (double *)&tg.null, 0 },	// B position
	{ str_posc,_print_rot, _get_pos, _set_nul, (double *)&tg.null, 0 },	// C position
	{ str_mpox,_print_lin, _get_mpos,_set_nul, (double *)&tg.null, 0 },	// X machine position
	{ str_mpoy,_print_lin, _get_mpos,_set_nul, (double *)&tg.null, 0 },	// Y machine position
	{ str_mpoz,_print_lin, _get_mpos,_set_nul, (double *)&tg.null, 0 },	// Z machine position
	{ str_mpoa,_print_rot, _get_mpos,_set_nul, (double *)&tg.null, 0 },	// A machine position
	{ str_mpob,_print_rot, _get_mpos,_set_nul, (double *)&tg.null, 0 },	// B machine position
	{ str_mpoc,_print_rot, _get_mpos,_set_nul, (double *)&tg.null, 0 },	// C machine position
...
static char msg_stat0[] PROGMEM = "Reset";
static char msg_stat1[] PROGMEM = "Run";
static char msg_stat2[] PROGMEM = "Stop";
static char msg_stat3[] PROGMEM = "Feedhold";
static char msg_stat4[] PROGMEM = "resume";
static char msg_stat5[] PROGMEM = "Homing";
static PGM_P msg_stat[] PROGMEM = { msg_stat0, msg_stat1, msg_stat2, msg_stat3, msg_stat4, msg_stat5 };
...
static PGM_P msg_motm[] PROGMEM = { msg_g00, msg_g01, msg_g02, msg_g03, msg_g80 };
...
static char msg_g90[] PROGMEM = "G90 - absolute distance";
static char msg_g91[] PROGMEM = "G91 - incremental distance";
static PGM_P msg_dist[] PROGMEM = { msg_g90, msg_g91 };

static char msg_g94[] PROGMEM = "G94 - units per minute";
static char msg_g93[] PROGMEM = "G93 - inverse time";
...
static uint8_t _get_stat(const INDEX_T i, cmdObj *cmd)
{
	...
	strncpy_P(cmd->string,(PGM_P)pgm_read_word(&msg_stat[(uint8_t)cmd->value]),CMD_STRING_LEN);
...
static uint8_t _get_unit(const INDEX_T i, cmdObj *cmd)
{
	cmd->value = cm_get_units_mode();
	strncpy_P(cmd->string,(PGM_P)pgm_read_word(&msg_unit[(uint8_t)cmd->value]),CMD_STRING_LEN);
	cmd->value_type = VALUE_TYPE_INT32;
	return (TG_OK);
}
...
static uint8_t _get_coor(const INDEX_T i, cmdObj *cmd)
{
	cmd->value = cm_get_coord_system();
	strncpy_P(cmd->string,(PGM_P)pgm_read_word(&msg_coor[(uint8_t)cmd->value]),CMD_STRING_LEN);
	cmd->value_type = VALUE_TYPE_INT32;
	return (TG_OK);
}
...
static uint8_t _get_motm(const INDEX_T i, cmdObj *cmd)
{
	cmd->value = cm_get_motion_mode();
	strncpy_P(cmd->string,(PGM_P)pgm_read_word(&msg_motm[(uint8_t)cmd->value]),CMD_STRING_LEN);
	cmd->value_type = VALUE_TYPE_INT32;
	return (TG_OK);
}
...
static uint8_t _get_plan(const INDEX_T i, cmdObj *cmd)
{
	cmd->value = cm_get_select_plane();
	strncpy_P(cmd->string,(PGM_P)pgm_read_word(&msg_plan[(uint8_t)cmd->value]),CMD_STRING_LEN);
	cmd->value_type = VALUE_TYPE_INT32;
	return (TG_OK);
}
...
static uint8_t _get_path(const INDEX_T i, cmdObj *cmd)
{
	cmd->value = cm_get_path_control();
	strncpy_P(cmd->string,(PGM_P)pgm_read_word(&msg_path[(uint8_t)cmd->value]),CMD_STRING_LEN);
	cmd->value_type = VALUE_TYPE_INT32;
	return (TG_OK);
}
...
static uint8_t _get_dist(const INDEX_T i, cmdObj *cmd)
{
	cmd->value = cm_get_distance_mode();
	strncpy_P(cmd->string,(PGM_P)pgm_read_word(&msg_dist[(uint8_t)cmd->value]),CMD_STRING_LEN);
	cmd->value_type = VALUE_TYPE_INT32;
	return (TG_OK);
}
...
{
	cmd->value = cm_get_work_runtime_position(_get_position_axis(i));
//	if (cm_get_units_mode() == INCHES_MODE) cmd->value *= INCH_PER_MM;
	cmd->value_type = VALUE_TYPE_FLOAT;
	return (TG_OK);
}

static uint8_t _get_mpos(const INDEX_T i, cmdObj *cmd) 
{
	cmd->value = cm_get_machine_runtime_position(_get_position_axis(i));
//	if (cm_get_units_mode() == INCHES_MODE) cmd->value *= INCH_PER_MM;
	cmd->value_type = VALUE_TYPE_FLOAT;
	return (TG_OK);
}
...
uint8_t cfg_config_parser(char *str)
{
	...
	if (str[0] == '?') {					// cutout for status report
...
{
	if (strstr(PREFIX_GROUPS, str) != NULL) { // see config.h
		return (true);
	}
	return (false);
}
...
static void _print_str(const INDEX_T i)
{
	char format[CMD_FORMAT_LEN+1];
	cmdObj cmd;
	cmd_get(i, &cmd);
	fprintf(stderr, _get_format(i,format), cmd.string);
}
