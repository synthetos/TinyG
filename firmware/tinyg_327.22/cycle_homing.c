...
uint8_t cm_homing_cycle()
{
	...
	cm.canned_cycle_state = CY_STATE_NEW;
	return (TG_OK);
}

uint8_t cm_run_homing_cycle()				// outer runtime routine
{
	if (cm.canned_cycle_state == CY_STATE_OFF) { 
		return (TG_NOOP);
	}
...
uint8_t _cm_run_homing_cycle()				// inner runtime routine
{
	...
	if (cm.canned_cycle_state == CY_STATE_NEW) {
		cm.canned_cycle_state = CY_STATE_HOMING_X_START;
	...
	if ((CFG(X).homing_enable  == TRUE) && (cm.canned_cycle_state == CY_STATE_HOMING_X_START)) {
		cm.canned_cycle_state = CY_STATE_HOMING_X_WAIT;
	...
	if (cm.canned_cycle_state == CY_STATE_HOMING_X_WAIT) {
		cm.canned_cycle_state = CY_STATE_HOMING_Y_START;
		sw_clear_limit_switches();
	...
	if ((CFG(Y).homing_enable  == TRUE) && (cm.canned_cycle_state == CY_STATE_HOMING_Y_START)) {
		cm.canned_cycle_state = CY_STATE_HOMING_Y_WAIT;
	...
	if (cm.canned_cycle_state == CY_STATE_HOMING_Y_WAIT) {
		cm.canned_cycle_state = CY_STATE_HOMING_Z_START;
		sw_clear_limit_switches();
	...
	if ((CFG(Z).homing_enable  == TRUE) && (cm.canned_cycle_state == CY_STATE_HOMING_Z_START)) {
		cm.canned_cycle_state = CY_STATE_HOMING_Z_WAIT;
	...
	if (cm.canned_cycle_state == CY_STATE_HOMING_Z_WAIT) {
		cm.canned_cycle_state = CY_STATE_HOMING_A_START;
		sw_clear_limit_switches();
	...
	if ((CFG(A).homing_enable  == TRUE) && (cm.canned_cycle_state == CY_STATE_HOMING_A_START)) {
		cm.canned_cycle_state = CY_STATE_HOMING_A_WAIT;
	...
	if (cm.canned_cycle_state == CY_STATE_HOMING_A_WAIT) {
		cm.canned_cycle_state = CY_STATE_HOMING_RTZ_START;
		sw_clear_limit_switches();
	...
	if (cm.canned_cycle_state != CY_STATE_HOMING_RTZ_WAIT) {
		cm.canned_cycle_state = CY_STATE_HOMING_RTZ_WAIT;
	...
	if (cm.canned_cycle_state == CY_STATE_HOMING_RTZ_WAIT) {
		...
		cm.canned_cycle_state = CY_STATE_OFF;						//...don't come back
		return (TG_COMPLETE);
	}
	return (TG_OK);
}
