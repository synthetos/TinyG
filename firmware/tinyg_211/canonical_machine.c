...
{
	double 	l[3];		// XYZ lengths  
	double 	t[3];		// XYZ times to complete axis moves
	double	length;		// cartesian distance of traverse
	uint8_t slowest;	// axis number of slowest axis in the move
	...
	// compute the axis lengths and travel times
	for (uint8_t i = 0; i <= Z; i++) {
		l[i] = fabs(gm.target[i] - gm.position[i]);
		t[i] = l[i] / CFG(i).max_seek_rate;
	}
	// find the slowest axis in the move (rate-limiting axis)
	slowest = ((t[Z] > ((t[X] > t[Y]) ? t[X] : t[Y])) ? Z : ((t[X] > t[Y]) ? X : Y));

	// compute the resultant feed rate based on the rate-limiting axis
 	length = sqrt(square(l[X]) + square(l[Y]) + square(l[Z]));
	gm.seek_rate = (length / l[slowest]) * CFG(slowest).max_seek_rate;
