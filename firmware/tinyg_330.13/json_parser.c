...
static uint8_t _expand_group(cmdObj *cmd);
...
static uint8_t _json_parser(char *str)
{
	...
	// test and normalize input string
	ritorno(_normalize_json_string(str, JSON_STRING_LEN));

	// collect the NV pairs into structs and test for a group: e.g. {"x";null}
	for (i=0; i<CMD_ARRAY_SIZE; i++) {	
		if ((status = _get_nv_pair(&cmd_array[i], &str, &nesting_level)) != TG_OK) {
			if ((status == TG_UNRECOGNIZED_COMMAND) && (cmd->name[1] == NUL)) {
				return (_expand_group(cmd));
			} else {
				return (status);
			}
		}
	...
	// take action on the parsed string
	for (i=0; i<CMD_ARRAY_SIZE; i++) {
		if (cmd->value_type == VALUE_TYPE_NULL){// means GET the value
			...
			cmd_write_NVM_record(cmd->index, cmd);		// persist the value to NVM
		}
...
/*
 * _expand_group() - generate a list of objects by group
 *
 *	There are some special tokens that are excluded
 */

static uint8_t _expand_group(cmdObj *cmd)
{
	char group = *cmd->name;
	char exclusions[] = {"fc,ln,vl,sr,ms,gc,xwp,ywp,zwp,awp,bwp,cwp,xmp,ymp,zmp,amp,bmp,cmp"};

	for (INDEX_T i=0; i<cmd_get_max_index(); i++) {
		if (cmd_get_group(i) == group) {
	...
	(--cmd)->nx = NULL;							// unmark the last cmd object
	return (TG_OK);
}
