...
char str_sr[] PROGMEM = "sr,status_r,[sr]";				// status_report object
...
{
	tg_json_gcode_return(gc_gcode_parser(cmd->string), cmd->string, tg.out_buf);
	return (TG_OK);
}
...
uint8_t cfg_config_parser(char *str)
{
	...
	if ((str[0] == '$') && ((str[1] == NUL) || (str[2] == NUL))) {
		sr_print_configs(str); 				// print based on contents of the string
		return (TG_OK);
	}
