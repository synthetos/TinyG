...
//**** Serial IO Interrupt levels are mapped here ****
// See stepper.h for the stepper interrupt levels

//Chose only one:
//#define __XIO_INTERRUPTS_RXLO_TXLO
#define __XIO_INTERRUPTS_RXMED_TXLO
//#define __XIO_INTERRUPTS_RXLO_TXMED
//#define __XIO_INTERRUPTS_RXMED_TXMED

#ifdef __XIO_INTERRUPTS_RXLO_TXLO
#define ENABLE_RX_INTERRUPTS PMIC_EnableLowLevel();
#define DISABLE_RX_INTERRUPTS PMIC_DisableLowLevel();
#define ENABLE_TX_INTERRUPTS PMIC_EnableLowLevel();
#define DISABLE_TX_INTERRUPTS PMIC_DisableLowLevel();
#define CTRLA_RXON_TXON (USART_RXCINTLVL_LO_gc | USART_DREINTLVL_LO_gc)
#define CTRLA_RXON_TXOFF (USART_RXCINTLVL_LO_gc)
#define CTRLA_RXON_TXOFF_TXCON (USART_RXCINTLVL_LO_gc | USART_TXCINTLVL_LO_gc)
...
#ifdef __XIO_INTERRUPTS_RXMED_TXLO
#define ENABLE_RX_INTERRUPTS PMIC_EnableMediumLevel();
#define DISABLE_RX_INTERRUPTS PMIC_DisableMediumLevel();
#define ENABLE_TX_INTERRUPTS PMIC_EnableLowLevel();
#define DISABLE_TX_INTERRUPTS PMIC_DisableLowLevel();
...
#ifdef __XIO_INTERRUPTS_RXLO_TXMED
#define ENABLE_RX_INTERRUPTS PMIC_EnableLowLevel();
#define DISABLE_RX_INTERRUPTS PMIC_DisableLowLevel();
#define ENABLE_TX_INTERRUPTS PMIC_EnableMediumLevel();
#define DISABLE_TX_INTERRUPTS PMIC_DisableMediumLevel();
#define CTRLA_RXON_TXON (USART_RXCINTLVL_LO_gc | USART_DREINTLVL_MED_gc)
#define CTRLA_RXON_TXOFF (USART_RXCINTLVL_LO_gc)
#define CTRLA_RXON_TXOFF_TXCON (USART_RXCINTLVL_LO_gc | USART_TXCINTLVL_MED_gc)
...
#ifdef __XIO_INTERRUPTS_RXMED_TXMED
#define ENABLE_RX_INTERRUPTS PMIC_EnableMediumLevel();
#define DISABLE_RX_INTERRUPTS PMIC_DisableMediumLevel();
#define ENABLE_TX_INTERRUPTS PMIC_EnableMediumLevel();
#define DISABLE_TX_INTERRUPTS PMIC_DisableMediumLevel();
...
// USART ISR RX buffer size
//#define RX_BUFFER_SIZE (BUFFER_T)64	// uint8_t buffer type is OK
//#define RX_BUFFER_SIZE (BUFFER_T)128	// uint8_t buffer type is OK
#define RX_BUFFER_SIZE (BUFFER_T)255	// uint8_t buffer type is OK
//#define RX_BUFFER_SIZE (BUFFER_T)256	// uint16_t buffer type is required
//#define RX_BUFFER_SIZE (BUFFER_T)1024
//#define RX_BUFFER_SIZE (BUFFER_T)2048

#define XOFF_HI_WATER_MARK (RX_BUFFER_SIZE * 0.6)	// % to issue XOFF
#define XOFF_LO_WATER_MARK (RX_BUFFER_SIZE * 0.2)	// % to release XOFF
...
//	uint16_t uflags;			  // usart sub-system flags (UNUSED)
	...
	BUFFER_T next_tx_buf_head;	  // next TX buffer write index
...
// down here by its lonesome because it needs xioUSART defined
