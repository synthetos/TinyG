...
struct mpBufferPool {			// ring buffer for sub-moves
	volatile uint8_t request_finalize_run_buffer;
...
static void _mp_request_finalize_run_buffer(void);
...
static void _mp_request_finalize_run_buffer() 
{
	mb.request_finalize_run_buffer = TRUE;
}

void mp_invoke_finalize_run_buffer() 
{
	if (mb.request_finalize_run_buffer == TRUE) {
		mb.request_finalize_run_buffer = FALSE;
		_mp_finalize_run_buffer();
	}
}
...
static uint8_t _mp_exec_aline(struct mpBuffer *bf)
{
	switch (bf->move_state) {
		case MOVE_STATE_IDLE:	 { return(TG_NOOP); }
		case MOVE_STATE_NEW:	 { return(_mp_exec_accel_0(bf)); }
		case MOVE_STATE_ACCEL_1: { return(_mp_exec_accel_1(bf)); }
		case MOVE_STATE_ACCEL_2: { return(_mp_exec_accel_2(bf)); }
		case MOVE_STATE_CRUISE:  { return(_mp_exec_cruise(bf)); }
		case MOVE_STATE_DECEL_0: { return(_mp_exec_decel_0(bf)); }
		case MOVE_STATE_DECEL_1: { return(_mp_exec_decel_1(bf)); }
		case MOVE_STATE_DECEL_2: { return(_mp_exec_decel_2(bf)); }
	}
	return (TG_OK);
}
