...
int xio_putc_rs485(const char c, FILE *stream)
{
	...
	if ((RSu.usart->STATUS & USART_TX_REGISTER_READY_bm) != 0) { // 0x20
	...
	ENABLE_TX_INTERRUPTS;							// enable USART TX interrupts
...
//		RSu.rx_buf_head = (_BUFTYPE)(RX_BUFFER_SIZE -1);// -1 avoids the off-by-one error
...
//	if ((++RSu.rx_buf_head) > (_BUFTYPE)(RX_BUFFER_SIZE-1)) { // reset the head
