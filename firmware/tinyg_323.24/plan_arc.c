...
enum arArcState {
	...
	AR_STATE_NEW,
	AR_STATE_RUNNING,
	AR_STATE_END				// force the move to end (kill)
};
				...
				const double minutes)		// time to complete the move
{
	...
	copy_axis_vector(ar.target, target);
	ar.time = minutes;
	ar.theta = theta;
	ar.radius = radius;
	ar.axis_1 = axis_1;
...
uint8_t ar_run_arc() 
{
	// continuation cutout conditions
	if (ar.run_state == AR_STATE_OFF) { 
		return (TG_NOOP);
	}
	if (mp_test_write_buffer() == FALSE) { 
		return (TG_EAGAIN); 
	}
	// initialize arc variables
	if (ar.run_state == AR_STATE_NEW) {
		...
		ar.segment_length = ar.linear_travel / ar.segments;
		...
		ar.run_state = AR_STATE_RUNNING;
	}

	// compute an arc segment and exit
	if (ar.run_state == AR_STATE_RUNNING) {
		...
		ar.target[ar.axis_linear] += ar.segment_length;
