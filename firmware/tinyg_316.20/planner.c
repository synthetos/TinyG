...
static double _mp_get_tangential_jerk_factor(const struct mpBuffer *bf, const struct mpMovePlanner *m);
...
uint8_t mp_aline(const double target[], const double minutes)
{
	...
	if (t->buffer_state != MP_BUFFER_QUEUED) {
		...
		// use prev Vt adjusted by a factor to reduce tangential jerk
		m->initial_velocity_req = t->request_velocity;
		m->initial_velocity_req *= _mp_get_tangential_jerk_factor(t,m);
...
/*	
 * _mp_get_angular_velocity_factor()
 *
 *  Determine how much to downgrade velocity at the join based on the 
 *	difference in velocity ofthe joining lines and the corner velocity
 *	setting.
 */
//static double _mp_get_join_velocity_factor(const double Vprev, const double Vnext)
//{
//	return (min(1,(fabs(Vnext - Vprev)/cfg.cornering_velocity)));

//	double Vdelta = fabs(Vnext - Vprev);
//	if (Vdelta > cfg.cornering_velocity) {
//		return (Vdelta / cfg.cornering_velocity);
//	} else {
//		return (1);
//	}
//}
 ...
 *
 *  Estimate the magnitude of the off-axis jerk at the junction of two 
 *	lines (A & B) 
 *	The function returns 1 for a junction with no angle (a straight join), 
 *	and 0 for a 180 degree reversal. In between values return a cosine 
 *	value that is computed from half the join angle. The value is used to 
 *	down-grade the velocity at the junction to limit the jerk. Equation is:
 *
 ...
 *	It's weird getting an angle from a 6 dimensional unit vector, but 
 *	there you are.
 *
...
static double _mp_get_tangential_jerk_factor(const struct mpBuffer *bf, const struct mpMovePlanner *m)
{
	...
	double theta = acos((mm.unit_vec[X] * bf->unit_vec[X]) +
						(mm.unit_vec[Y] * bf->unit_vec[Y]) +
						(mm.unit_vec[Z] * bf->unit_vec[Z]) +
						(mm.unit_vec[A] * bf->unit_vec[A]) +
						(mm.unit_vec[B] * bf->unit_vec[B]) +
						(mm.unit_vec[C] * bf->unit_vec[C]));
...
//	double theta_factor = cos(theta/2);

//	double deltaV_factor = fabs(bf->request_velocity - m->initial_velocity_req);

//	if (deltaV > cfg.cornering_velocity) {
//		return (Vdelta / cfg.cornering_velocity);
//	} else {
//		return (1);
//	}
	
//	return (cos(theta/2));			// cosine factor
//	return (1-(theta/(M_PI/2)));	// linear factor
}
