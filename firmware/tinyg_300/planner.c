...
static uint8_t _mp_run_line(struct mpBuffer *m);
static uint8_t _mp_run_arc(struct mpBuffer *m);
static uint8_t _mp_run_dwell(struct mpBuffer *m);
static uint8_t _mp_run_stops(struct mpBuffer *m);
...
//static uint8_t _mp_line_to_arc(struct mpBuffer *p);
static uint8_t _mp_queue_head(void);
static uint8_t _mp_queue_body(void);
static uint8_t _mp_queue_tail(void);
static uint8_t _mp_queue_line_buffer(double Vi, double Vt, double len, uint8_t type);
static uint8_t _mp_compute_regions(void); 
static uint8_t _mp_recompute_previous(struct mpBuffer *p);
...
enum mpBufferState {			// m->buffer_state values 
...
enum mcMoveType {				// m->move_type values 
	MP_TYPE_NONE = 0,			// no move specified (MUST BE ZERO)
...
uint8_t mp_move_dispatcher(uint8_t kill) 
{
	...
	if ((m = mp_get_run_buffer()) == NULL) {	// NULL means nothing's running
		return (TG_NOOP);
	}
	if (kill) {
		m->move_state = MP_STATE_END;
		mr.run_flag = FALSE;				// finalize and return
		mp_end_run_buffer();
		return (TG_OK);
	}
	if (m->move_state == MP_STATE_NEW) {	// first time in?
 ...
 *
 * mp_get_prev_buffer()	   Return a pointer to the buffer immediately 
...
struct mpBuffer * mp_get_prev_buffer()
{
	return (mb.w->pv);
}
...
uint8_t mp_dwell(double seconds) 
{
	...
	if ((m = mp_get_write_buffer()) == NULL) { // get write buffer or fail
...
uint8_t mp_line(double x, double y, double z, double a, double minutes)
{
	...
	_mp_set_mm_position(m->target);		// set mm position for planning
	return(TG_OK);
}
...
static uint8_t _mp_run_line(struct mpBuffer *m) 
{
	...
	_mp_set_mr_position(m->target);		// set mr position for runtime
	return (TG_OK);
}
				...
				double minutes)		// time to complete the move
{
	...
	m->ending_velocity = m->starting_velocity;	// for consistency
...
static uint8_t _mp_run_arc(struct mpBuffer *m) 
{
	...
	if (m->move_state == MP_STATE_NEW) {
		...
		mr.target[m->axis_linear] = mr.position[m->axis_linear];
		m->move_state = MP_STATE_RUNNING;
	}
	// compute an arc segment and exit
	if (m->move_state == MP_STATE_RUNNING) {
		m->theta += mr.segment_theta;
		mr.target[m->axis_1] = mr.center_1 + sin(m->theta) * m->radius;
		mr.target[m->axis_2] = mr.center_2 + cos(m->theta) * m->radius;
		mr.target[m->axis_linear] += mr.segment_length;
...
uint8_t mp_aline(double x, double y, double z, double a, double minutes)
{
	...
	p = mp_get_prev_buffer();
	if ((p->move_type == MP_TYPE_ARC) && 
		(p->buffer_state != MP_BUFFER_EMPTY)) {		// q'd or running arc
		mm.previous_velocity = p->ending_velocity;
		mm.initial_velocity = mm.previous_velocity;	// +++ test various arc join speed changes up and down
		ritorno(_mp_compute_regions());	// compute region lengths & Vt
		ritorno(_mp_queue_head());		// queue a head if body_length != 0
		ritorno(_mp_queue_body());		// queue a body if body_length != 0
		ritorno(_mp_queue_tail());		// queue a tail if tail_length != 0
		return (TG_OK);
	...
	angular_jerk = _mp_estimate_angular_jerk(p);	// for path downgrades
	...
	// do the actual work
	ritorno(_mp_compute_regions());	   // compute region lengths & Vt
	ritorno(_mp_recompute_previous(p));// recompute previous tail
	ritorno(_mp_queue_head());		// queue a head if head_length != 0
	ritorno(_mp_queue_body());		// queue a body if body_length != 0
	ritorno(_mp_queue_tail());		// queue a tail if tail_length != 0
	return (TG_OK);
}
 ...
 * _mp_queue_head()				- queue a 3 part move
 * _mp_queue_body()				- queue a 2 part move with no body
 * _mp_queue_tail()				- queue a 2 part move with no head
 * _mp_queue_line_buffer()  	- helper helper for making line buffers
 * _mp_line_to_arc()			- handle arc cases
 * _mp_compute_regions() 		- compute region lengths and velocities
 * _mp_recompute_previous()		- join previous tail to Vi
 */
...
inline static uint8_t _mp_queue_head() {
	return(_mp_queue_line_buffer(mm.initial_velocity, mm.target_velocity,
								 mm.head_length, MP_TYPE_ACCEL));
}

inline static uint8_t _mp_queue_body() {
	return(_mp_queue_line_buffer(mm.target_velocity, mm.target_velocity,
		  						 mm.body_length, MP_TYPE_CRUISE));
}

inline static uint8_t _mp_queue_tail() {
	return(_mp_queue_line_buffer(mm.target_velocity, mm.final_velocity, 
								 mm.tail_length, MP_TYPE_DECEL));
}
...
static uint8_t _mp_queue_line_buffer(double Vi, double Vt, double len, uint8_t type)
{
	...
	if ((m = mp_get_write_buffer()) == NULL) { 	// get buffer or die trying
		return (TG_BUFFER_FULL_FATAL); 
	} 
	...
	for (uint8_t i=0; i < AXES; i++) { 			// copy unit vector from mm
		...
		mm.position[i] += len * m->unit_vec[i];	// set mm position
		m->target[i] = mm.position[i]; 
	}
	return(mp_queue_write_buffer(type));		// queue buffer and return
}
...
static uint8_t _mp_compute_regions() 
{
	...
	mm.tail_length = _mp_get_length(Vt, 0);
	mm.head_length = _mp_get_length(Vt, mm.initial_velocity);
...
static uint8_t _mp_recompute_previous(struct mpBuffer *p)
{
	...
	struct mpBuffer *m;			// pointer to active buffer

	// exit if the buffer anything but queued and idle. Leaves Ve=0
	if (p->buffer_state != MP_BUFFER_QUEUED) {
		return (TG_OK);
	}
	...
	if (fabs(mm.initial_velocity - mm.previous_velocity) < ROUNDING_ERROR) {
		...
		p->move_type = MP_TYPE_CRUISE;	// change tail to a cruise
		return (TG_OK);	// no need to update unit_vector or target
	} 

	// compute the new tail length
	length = (_mp_get_length(mm.previous_velocity, mm.initial_velocity));
...
//		p->length = length;
	...
	mp_queue_write_buffer(MP_TYPE_DECEL);
	return (TG_OK);
}
 ...
 *
 * 	Solving equation 5.11' for Time for acceleration 2nd halfif you know:
