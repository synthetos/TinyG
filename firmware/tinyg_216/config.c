 ...
 *	
 * Adding a new config element:
 *	- add into cfg struct
 *	- put hardwired defaults into hardware.h
 *	- adjust cfgTokens accordingly. Check subsequent defines are still valid
 *	- make a mnemonic and put in table in _cfg_sprintf_as_record()
 *	- adjust _cfg_load_hardwired_settings()
 *	- add to cfg_parse()
 *	- add to _cfg_tokenize_config_record()
 *	- add to _cfg_print_config_record()
 *	- add to _cfg_write_profile_to_NVM()
 *	- attempt compile it and see what you broken by changing the cfg struct
 */
/* TODO:
	- help screen
 */
...
void _cfg_migrate_config_data(uint16_t address, uint8_t old_length);
...
void _cfg_sprintf_as_record(char *record, uint8_t token, char axis, double value);

void _cfg_write_as_record_to_NVM(uint16_t base_addr, uint8_t token, uint8_t axis, double value);

void _cfg_write_record_to_NVM(uint16_t base_addr, char *record, uint8_t token, uint8_t axis);
void _cfg_read_record_from_NVM(uint16_t base_addr, char *record, uint8_t token, uint8_t axis);
void _cfg_write_to_NVM(uint16_t address, char *record);
void _cfg_read_from_NVM(uint16_t address, char *record, uint8_t size);
uint16_t _cfg_compute_record_address(const uint16_t base_addr, uint8_t token, uint8_t axis);
...
enum cfgTokens {
	...
	// per-axis settings - 4 sets: one for each axis
		...
		CFG_DEGREES_PER_STEP,	// motor parameter
		...
		CFG_TRAVEL_OFFSET,		// zero offset for min limit switch
...
int cfg_parse(char *block, uint8_t persist, uint8_t display)
{
	...
	switch (cp.token) {
		...
		case CFG_TRAVEL_OFFSET: 	CFG(cp.axis).travel_offset = cp.value; break;
...
inline void _cfg_computed() 
{
	...
	for (uint8_t i = X; i <= A; i++) {
		// steps_per_unit = 360 / (degree_per_step/microstep) / travel_per_rev
		CFG(i).steps_per_unit = (360 / (CFG(i).degree_per_step / CFG(i).microstep_mode)) / CFG(i).travel_rev;

		// max_seek_rate = 60 * seek_steps_sec / (360/degree_per_step/travel_rev)
		CFG(i).max_seek_rate = ((60 * (double)CFG(i).seek_steps_sec) /
							 	(360/CFG(i).degree_per_step/CFG(i).travel_rev));

		// max_feed_rate = 60 * feed_steps_sec / (360/degree_per_step/travel_rev)
...
uint8_t _cfg_tokenize_config_record()
{
	...
	if (cp.axis != -1) {
		switch(cp.record[1]) {
			...
			case 'D':
				switch (cp.record[2]) {
					case 'E': { cp.token = CFG_DEGREES_PER_STEP; break; }
					default: return(TG_UNRECOGNIZED_COMMAND);
				} return (TG_OK);
			...
			case 'T': 
				switch (cp.record[2]) {
					case 'O': { cp.token = CFG_TRAVEL_OFFSET; break; }
...
char cfs09[] PROGMEM = "  Homing mode (1=power-on)        %1.0f";
char cfs10[] PROGMEM = "  A axis radius              %5.3f";

char cfs11[] PROGMEM = "%c axis mapped to motor number  %4.0f";
char cfs12[] PROGMEM = "  %c axis - Axis mode          %5.0f";
char cfs13[] PROGMEM = "  %c axis - Seek steps / sec   %5.0f";
char cfs14[] PROGMEM = "  %c axis - Feed steps / sec   %5.0f";
char cfs15[] PROGMEM = "  %c axis - Degrees / step     %5.3f";
char cfs16[] PROGMEM = "  %c axis - Microstep mode     %5.0f";
char cfs17[] PROGMEM = "  %c axis - Motor polarity     %5.0f";
char cfs18[] PROGMEM = "  %c axis - Travel / rev       %5.2f";
char cfs19[] PROGMEM = "  %c axis - Travel zero offset %5.0f";
char cfs20[] PROGMEM = "  %c axis - Travel maximum     %5.0f";
char cfs21[] PROGMEM = "  %c axis - Travel warning     %5.0f";
char cfs22[] PROGMEM = "  %c axis - Power mgmt mode    %5.0f";
char cfs23[] PROGMEM = "  %c axis - Limit switch mode  %5.0f";
char cfs24[] PROGMEM = "Derived: max seek rate, max feed rate (change steps / sec to set)\n";
char cfs25[] PROGMEM = "  %c axis - %7.2f        %7.2f mm/min\n";
char cfs26[] PROGMEM = "  %c axis - %7.2f        %7.2f in/min\n";
char cfs27[] PROGMEM = "  %c axis - %7.2f        %7.2f degrees/min\n";

#define COMPUTED_VALUES 24
...
PGM_P cfgShow[] PROGMEM = {	
	...
	cfs20, cfs21, cfs22, cfs23, cfs24, cfs25, cfs26, cfs27
};

void cfg_print_config_records()
{
	uint16_t address = (cp.base_addr + CFG_RECORD_LEN);	// skip length record

	tg_alive();		// display build number 

	// best to count it out. A do / while checking for trailer could run away
	for (uint16_t i = 1; i < CFG_TRAILER_RECORD; i++) {
		_cfg_read_from_NVM(address, cp.record, CFG_RECORD_LEN);
		_cfg_print_config_record(cp.record);
		address += CFG_RECORD_LEN;
	}
	...
	printf_P((PGM_P)pgm_read_word(&cfgShow[COMPUTED_VALUES]));

	for (uint8_t i = X; i <= Z; i++) {
		char axis = GETAXISCHAR(i);
		if (cfg.gcode_units) {	// TRUE = mm
			printf_P((PGM_P)pgm_read_word(&cfgShow[COMPUTED_VALUES+1]), 
						axis, CFG(i).max_seek_rate, CFG(i).max_feed_rate);
		} else {
			printf_P((PGM_P)pgm_read_word(&cfgShow[COMPUTED_VALUES+2]), 
						axis, CFG(i).max_seek_rate, CFG(i).max_feed_rate);
		}
	}
	printf_P((PGM_P)pgm_read_word(&cfgShow[COMPUTED_VALUES+3]), 
						'A', CFG(A).max_seek_rate, CFG(A).max_feed_rate);
}

inline void _cfg_print_config_record(char *record)
{
...
char cff09[] PROGMEM = "HO%1.3f";	// Homing mode
char cff10[] PROGMEM = "RA%1.3f";	// A radius

char cff11[] PROGMEM = "%cMA%1.0f";	// Map axis to motor
char cff12[] PROGMEM = "%cMO%1.0f";	// Axis operating mode
char cff13[] PROGMEM = "%cSE%1.0f";	// Seek steps per second
char cff14[] PROGMEM = "%cFE%1.0f";	// Feed steps / sec
char cff15[] PROGMEM = "%cDE%1.3f";	// Degrees per step
char cff16[] PROGMEM = "%cMI%1.0f";	// Microstep mode
char cff17[] PROGMEM = "%cPO%1.0f";	// Polarity
char cff18[] PROGMEM = "%cRE%1.3f";	// Travel per revolution (mm, degrees)
char cff19[] PROGMEM = "%cTO%1.0f";	// Travel offset (mm)
char cff20[] PROGMEM = "%cTM%1.0f";	// Travel max (mm)
char cff21[] PROGMEM = "%cTW%1.0f";	// Travel warning
char cff22[] PROGMEM = "%cPW%1.0f";	// Power management mode
char cff23[] PROGMEM = "%cLI%1.0f";	// Limit switch mode

char cff24[] PROGMEM = "_T%1.0f";	// trailer token (trailer)
...
void _cfg_write_profile_to_NVM(uint16_t base_addr)
{
	...
	for (uint8_t axis = 0; axis <= 3; axis++) {	// cycle thru axes X,Y,Z,A
		...
		_cfg_write_as_record_to_NVM(base_addr, CFG_TRAVEL_OFFSET, axis,
											   CFG(axis).travel_offset); 
...
inline uint16_t _cfg_compute_record_address(const uint16_t base_addr, uint8_t token, uint8_t axis)
{
	if (token == CFG_TRAILER_TOKEN) {
...
void _cfg_load_hardwired_settings()
{
	...
	cfg.a[X].travel_rev = X_TRAVEL_REVOLUTION;
	cfg.a[Y].travel_rev = Y_TRAVEL_REVOLUTION;
	cfg.a[Z].travel_rev = Z_TRAVEL_REVOLUTION;
	cfg.a[A].travel_rev = A_TRAVEL_REVOLUTION;
	
	cfg.a[X].travel_offset = X_TRAVEL_OFFSET;
	cfg.a[Y].travel_offset = Y_TRAVEL_OFFSET;
	cfg.a[Z].travel_offset = Z_TRAVEL_OFFSET;
	cfg.a[A].travel_offset = A_TRAVEL_OFFSET;
...
// VESTIGAL CODE THAT MIGHT BE USEFUL AT SOME POINT
...
void _cfg_print_axis(uint8_t	axis)
{
	...
	printf_P(PSTR("  travel_max:       %7.2f millimeters total travel\n"), CFG(axis).travel_max);
	...
	printf_P(PSTR(" (steps_per_unit:    %7.2f microsteps / millimeter)\n\n"), CFG(axis).steps_per_unit);
}
*/
#endif
