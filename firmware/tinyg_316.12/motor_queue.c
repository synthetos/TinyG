...
uint8_t mq_queue_line(double steps[], double microseconds)
{
	...
	double major_axis_steps = 1;	// minimum line
	double zero[AXES] = { 0,0,0,0,0,0 };
	...
	// trap zero time, zero step, and other impossible moves
	if ((isfinite(microseconds) == FALSE) || (microseconds < EPSILON) ||
		(mp_get_vector_length(steps, zero) < 1)) {
...
//		return (TG_ZERO_LENGTH_MOVE);
//	}
//	if (isfinite(microseconds) == FALSE) {
//		return (TG_ZERO_LENGTH_MOVE);
//	}
//	if (mp_get_vector_length(steps, zero) < 1) { 
//		return (TG_ZERO_LENGTH_MOVE);
//	}
...
//	if (major_axis_steps > 1000000) {	//++++ DIAGNOSTIC
//		dump_mq_queue_line(steps, microseconds, major_axis_steps); //++++ DIAGNOSTIC
//	}
 	// another zero-length move case. 
	// The vector > 1 step but none of the individual axes are.
	if (major_axis_steps < 1) {
//		dump_mq_queue_line(steps, microseconds, major_axis_steps); //++++ DIAGNOSTIC
		return (TG_ZERO_LENGTH_MOVE);
	}
	// set dda clock frequency and substeps
	_mq_set_f_dda(&f_dda, &dda_substeps, major_axis_steps, microseconds, steps);
