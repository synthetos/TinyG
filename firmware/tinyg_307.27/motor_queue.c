					  ...
					  double microseconds)
{
	...
	if (microseconds < ROUNDING_ERROR) {
	...
	for (i=0; i<AXES; i++) {
		for (j=0; j<MOTORS; j++) {
			if (cfg.motor_map[j] == i) {
				mq.p->a[j].dir = ((steps[i] < 0) ? 1 : 0); // Verify direction
				mq.p->a[j].dir = mq.p->a[i].dir ^ CFG(i).polarity;
	...
	while ((microseconds *(*f_dda) *(*dda_substeps)) > (MAX_ULONG * 1000000)) {
		if (((*dda_substeps) = (*dda_substeps)/2) < 1) {
			...
			if ((microseconds *(*f_dda) *(*dda_substeps)) > (MAX_ULONG * 1000000)) {
				TRAP(major_axis_steps, PSTR("_mq_set_f_dda() line overflow: %f"))
				break;
			}
		}
	}
