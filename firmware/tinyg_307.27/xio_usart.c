 ...
 *
 *	Note: Originally I had the routine advancing the buffer head and 
 *		  comparing against the buffer tail to detect buffer full (it would
 *		  sleep if the buffer was full). This unfortunately collides with 
 *		  the buffer empty detection in the dequeue routine - causing the 
...
{
	if (ECHO(ds[gdev].flags) != 0) ds[gdev].x_putc(ds[gdev].c, stdout);
	return(ds[gdev].c);
}

static int _getc_NEWLINE(void)		// convert CRs and LFs to newlines if line mode
{
	if (LINEMODE(ds[gdev].flags) != 0) ds[gdev].c = '\n';
	if (ECHO(ds[gdev].flags) != 0) ds[gdev].x_putc(ds[gdev].c, stdout);
	return(ds[gdev].c);
}
...
static int _xio_readc_usart(const uint8_t dev, const char *buf)
{
	...
	return (((fptr_int_void)(pgm_read_word(&getsFuncs[(uint8_t)d->c])))()); // dispatch char
}
...
static int _gets_char(void)
{
	...
	if (ECHO(ds[gdev].flags) != 0) ds[gdev].x_putc(ds[gdev].c, stdout);// conditional echo
...
static int _gets_NEWLINE(void)				// handles any newline char
{
	...
	if (ECHO(ds[gdev].flags) != 0) ds[gdev].x_putc('\n',stdout);// echo a newline
