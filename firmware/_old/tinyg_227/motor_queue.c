...
{
	if (mq.head == mq.tail) { return FALSE; }			// buffer full
	return (TRUE);
}

struct mqMove * mq_queue_motor_buffer() 
{
	if (mq.tail == mq.head) { return (NULL); } 			// buffer full
	mq.p = &mq.move_buffer[mq.head];
	if (++mq.head >= MQ_BUFFER_SIZE) { mq.head = 0; };	// advance head
	return (mq.p);
}
...
struct mqMove * mq_dequeue_motor_buffer()
{
	...
	if (++next_tail >= MQ_BUFFER_SIZE) { next_tail = 0; }// incr w/wrap
	if (next_tail == mq.head) { return (NULL); }		// buffer empty
	mq.tail = next_tail;
