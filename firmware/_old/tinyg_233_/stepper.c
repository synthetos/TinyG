...
ISR(MOTOR_1_TIMER_ISR_vect)
{
	...
	if (--ax.a[MOTOR_1].postscale_counter) { // exit fast if you need to
		return;
	}
	ax.a[MOTOR_1].port->OUTSET = STEP_BIT_bm;	// turn X step bit on
	if (--ax.a[MOTOR_1].step_counter == 0) {	// end-of-move processing
		ax.a[MOTOR_1].timer->CTRLA = TC_CLK_OFF;// stop the clock
		ax.a[MOTOR_1].port->OUTSET = MOTOR_ENABLE_BIT_bm; // disable motor
	...
	ax.a[MOTOR_1].port->OUTCLR = STEP_BIT_bm;	// turn motor 1 step bit off
}
/*
ISR(MOTOR_1_TIMER_ISR_vect)
{
