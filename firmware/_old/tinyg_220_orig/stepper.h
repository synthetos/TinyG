...
struct Axes {						// All axes + some extra stuff
	...
	volatile uint_fast8_t stopped;	// set TRUE if STOP (FALSE for STARTed)
	volatile uint8_t active_axes;	// bits are set if axis is active. 0 = robot is idle
	volatile uint_fast8_t line_mode;// set TRUE if LINE command (FALSE for DWELLs)
	volatile struct mvMove *p;		// pointer to dequeued move structure
