...
{
	mc.position[X] = round(x * CFG(X).steps_per_unit);
	mc.position[Y] = round(y * CFG(Y).steps_per_unit);
	mc.position[Z] = round(z * CFG(Z).steps_per_unit); 
	mc.position[A] = round(z * CFG(Z).steps_per_unit); 
	return (TG_OK);
}
...
int mc_async_end()
{
	...
	mc.line_continue_state = MC_STATE_OFF;	// kill the generators
	mc.dwell_continue_state = MC_STATE_OFF;
	mc.stop_continue_state = MC_STATE_OFF;
	mc.arc_continue_state = MC_STATE_OFF;
	return (TG_OK);
...
{
	mc.target[X] = round(x * CFG(X).steps_per_unit);
	mc.target[Y] = round(y * CFG(Y).steps_per_unit);
	mc.target[Z] = round(z * CFG(Z).steps_per_unit); 
	mc.target[A] = round(a * CFG(A).steps_per_unit); 
...
int mc_arc_continue() 
{
	...
	while (ma.segment_counter <= ma.segments) {
		...
		for (uint8_t i = X; i < AXES; i++) {
			mc.target[i] = round(ma.dtarget[i] * CFG(i).steps_per_unit);
