...
uint8_t tg_application_startup(void)
{
	...
	if (cfg.homing_mode == TRUE) { 
		tg.status = cm_return_to_home();// inititate homing cycle
	}
...
---- Coding Continuations ----
	...
	Rules for writing a continuation routine:
	  ...
	  - The continuation is called initially by main routine, then will be 
	  	called repeatedly from the main controller loop. The continuation 
	  ...
	  - The continuation must return the following states, and may return 
	  	additional states to indicate various exception conditions:

		  -	TG_NOOP: No operation ocurred. This is the usual return from OFF.
		  			 All continuations must be callable with no effect when 
					 they are OFF (as they are called repeatedly wheterhe or 
					 not they are operating).

		  - TG_AGAIN: Routine is blocked or still processing (RUNNING).

		  - TG_OK; 	 Routine has just is completed - i.e. it has just 
		  			 transitioned to OFF. TG_OK should only be returned once.
