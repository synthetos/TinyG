...
These commands are active from the GCODE command line:\n\
  ...
  !    Feedhold - stop motion wihtout losing position\n\
...
fprintf_P(stderr, PSTR("\
Invoke self test by command $test=N   where N is one of:\n\
   1 smoke test\n\
   2 initialization tests\n\
   3 homing test (you must trip homing switches)\n\
   4 motion test (does a series of square and circle moves)\n\
   5 dwell test\n\
   6 feedhold test (you must enter ! and ~ to hold and restart, respectively)\n\
   7 M codes test  (a series of M codes intermingled with moves)\n\
   8 JSON test     (runs the motion test only with JSON commands)\n\
   9 inverse time test\n\
screen\n\n\
"));
_help_status_report_advisory();
...
{
fprintf_P(stderr, PSTR("\n\
Note: As of version 0.911 TinyG generates automatic status reports by default\n\
