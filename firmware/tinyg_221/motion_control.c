 ...
 *
 * Divide the line into 4 regions:
 *	- head		initial acceleration/deceleration to target velocity
 *	- body A	bisecting that target region (if it exists)
 *	- body B
 ...
 *
 * NOTE: This routine assumes that you will not call mc_arc() until the
 *	continuation has queued the last line segment. If you do, you will 
 *	clobber the currently executing line. The sequencing is enforced by 
 *	higher-level flow control in the receipt of new Gcode blocks.
 */
...
int mc_arc(double theta, 			// starting angle
		   ...
		   double linear_travel, 	// travel along linear axis (mm)
		   ...
		   double minutes)			// time to complete the move
{
	...
	if (mm_of_travel < cfg.mm_min_segment) { // too short to draw
		return (TG_ZERO_LENGTH_MOVE);
	}
	ma.segments = ceil(mm_of_travel / cfg.mm_min_segment);
