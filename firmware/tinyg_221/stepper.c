...
void st_init()
{
	...
	for (uint8_t i = MOTOR_1; i <= MOTOR_4; i++) {
		...
		st_set_power_mode(i, cfg.a[i].power_mode);	// low power idle
		st_set_microsteps(i, cfg.a[i].microstep_mode);// microsteps
 ...
 * st_set_power_mode() - setter needed by the config system
 */
...
{
	ax.a[motor].power_mode = power_mode;
}
