 ...
 * CFG_REVISION and CFG_HEADER revision must agree.
 * CFG_RECORD_LEN and CFG_LENGTH must agree.
 * If you change CFG_RECORD_LEN you must also change a statement in 
 * 	  _cfg_print_config_record()
 */

#define CFG_NVM_BASE 0x0000		// base address of usable NVM

#define CFG_REVISION 101 		// used to migrate data in firmware upgrades
#define CFG_RECORD_LEN 12 		// also change in _cfg_print_config_record()
#define CFG_PROFILE 1			// value used in profile and trailer records
...
int cfg_parse(char *text, uint8_t persist, uint8_t display);
...
struct cfgStructAxis {
	...
	double step_angle;		// degrees per whole step (ex: 1.8)
  	uint8_t microstep_mode;		// microsteps to apply for each axis (ex: 8)
	...
	double homing_rate;			// homing feed rate
...
struct cfgStructGlobal {
	...
	double mm_min_segment;		// arc drawing resolution in millimeters per segment
	double a_radius;			// radius for A axis feedrate computation
	uint8_t homing_mode;		// 0=off, 1=power-on (G28)

	// some random globals
	uint8_t homing_cycle_active;// TRUE = in a homing cycle state
	uint8_t kill;				// set TRUE if kill issued

 // axis structs
	struct cfgStructAxis a[AXES];// holds axes X,Y,Z,A
};
