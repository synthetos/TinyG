...
struct canonicalMachineSingleton {	// struct to manage cm globals and cycles
	uint8_t machine_state;			// These 3 make up the main state machine
	uint8_t cycle_state;
...
struct GCodeModel {						// Gcode model- meaning depends on context
	...
	uint32_t linenum;					// N word or autoincrement
...
#define MODAL_GROUP_COUNT MODAL_GROUP_M9
...
uint8_t cm_spindle_control(uint8_t spindle_mode);	// M3,M4,M5 spindle command
void cm_exec_spindle_control(uint8_t sprindle_mode);
