 ...
 *	  - cm_ function calls mp_gueue_<command> which puts it in the planning queue
 *	  - the planning queue gets to the function and calls _exec<command>
 *	  - ...which typically call back to the cm_exec<command> function.
 *
...
//		case MCODE_CHANGE_TOOL:	 // M6
	...
	st_prep_null();		// keeps the loader happy
	_free_run_buffer();
	return (TG_OK);
}
