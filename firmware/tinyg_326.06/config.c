...
#define CFG_VERSION ((double)0.90) // NVM version - NOT firmware revision
...
enum cfgMnemonics { P_, V_,		// <-- profile and version records. Must be first.
	...
	MO, SR, FR, TH, TS, 
	JM, CD, RA, LI, 
	HE, HR, HC, HO, HB,			// 14 axis settings (adjust #define COUNT_AXIS_SETTINGS)
	...
	JA, ML, MC, MT, HM, 
	EC, IC, IL, EX, EE,
	SE, SI,						// 17 general settings (adjust #define COUNT_GENERAL)
...
static void _appinHR(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmHR(s); }
static void _appinHC(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmHC(s); }
static void _appinHO(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmHO(s); }
static void _appinHB(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmHB(s); }
...
static void _cfg_print_setting(const struct cfgSetting *s)
{
	...
	if (inches_mode == FALSE) {
		...
		if (UNIT_CONVERSION_REQUIRED) value = s->value / MM_PER_INCH;
		if (s->grp == CFG_GENERAL) {
