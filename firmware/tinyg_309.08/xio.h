...
#define XIO_DROPLF		((uint32_t)1<<14)	// ignore <LF> on reads
#define XIO_NODROPLF	((uint32_t)1<<15)	// do not ignore <LF> on reads
#define XIO_LINEMODE	((uint32_t)1<<16)	// special <cr><lf> read handling
#define XIO_NOLINEMODE	((uint32_t)1<<17)	// no special <cr><lf> read handling
...
#define XIO_FLAG_BLOCK_bm	(1<<2)		// enable blocking RD and WR
#define XIO_FLAG_XOFF_bm 	(1<<3)		// XOFF flow control enabled
#define XIO_FLAG_ECHO_bm 	(1<<4)		// echo received chars to stderr
#define XIO_FLAG_CRLF_bm 	(1<<5)		// convert <LF> to <CR><LF> on writes
#define XIO_FLAG_DROPLF_bm 	(1<<6)		// ignore <LF> on reads
#define XIO_FLAG_LINEMODE_bm (1<<7)		// special handling for line-oriented text
// transient states
#define XIO_FLAG_TX_MUTEX_bm (1<<8)		// TX dequeue mutual exclusion flag
#define XIO_FLAG_EOL_bm		(1<<9)		// detected EOL (/n, /r, ;)
#define XIO_FLAG_EOF_bm 	(1<<10)		// detected EOF (NUL)
#define XIO_FLAG_IN_LINE_bm	(1<<11) 	// partial line is in buffer
#define XIO_FLAG_DO_XOFF_bm (1<<12) 	// flag to transmit an XOFF
#define XIO_FLAG_DO_XON_bm	(1<<13) 	// flag to transmit an XON
#define XIO_FLAG_IN_XOFF_bm (1<<14) 	// device is in XOFF (flow control)

#define XIO_FLAG_RESET_gm	(0x0FFF)	// used to clear the top bits

#define READ(a) 		(a & XIO_FLAG_RD_bm)	// non-zero if read enabled
#define WRITE(a)	 	(a & XIO_FLAG_WR_bm)	// non-zero if write enabled
#define BLOCKING(a) 	(a & XIO_FLAG_BLOCK_bm)	// etc.
#define EN_XOFF(a)		(a & XIO_FLAG_XOFF_bm)	// enable XOFF
#define ECHO(a)		 	(a & XIO_FLAG_ECHO_bm)
#define CRLF(a) 		(a & XIO_FLAG_CRLF_bm)
#define DROPLF(a) 		(a & XIO_FLAG_DROPLF_bm)
...
#define DO_XOFF(a) 		(a & XIO_FLAG_DO_XOFF_bm)
#define DO_XON(a) 		(a & XIO_FLAG_DO_XON_bm)
#define IN_XOFF(a) 		(a & XIO_FLAG_IN_XOFF_bm)
