...
//#define __DEBUG				// enables debug (see below & end-file notes)
...
/* 
 * More definitions of axes and motors 
 */
...
#define TINYG_VERSION "build 309.06 - \"Whooping Cough\""
 ...
 *
 *	TRAP usage examples:
 *		TRAP(PSTR("Line length %f"), m->length);
 *		TRAP2(PSTR("Failed division: %f / %f"), m->length, m->divisor);
 *		TRAP_IF_TRUE((m->divisor == 0), PSTR("Dividing by zero, Bucko: %f"), m->divisor);
 */
#ifdef __TRAPS 
#define TRAP(msg,a) { fprintf_P(stderr,PSTR("#### TRAP #### ")); \
