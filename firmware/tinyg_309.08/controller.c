...
{								// see tinyg.h for TINYG_VERSION string
	fprintf_P(stderr, PSTR("#### TinyG %S #### (type 'h' for help)\n"), (PSTR(TINYG_VERSION)));
...
{
	// XON/XOFF flow control indicator light
//	if (IN_XOFF(ds[XIO_DEV_USB].flags)) {
//		en_bit_on(0x08);
//	} else {
//		en_bit_off(0x08);		
//	}
...
//	xio_queue_RX_string_usb("$ec0\n");			// disable CR (LF only)
	xio_queue_RX_string_usb("$xss2000\n");		// set X seek steps to 2000
