...
//						  gm.position[A], gm.position[B], gm.position[C]);
						  gm.position[A]);
	return (TG_OK);
}
...
uint8_t cm_straight_traverse(double x, double y, double z, double a)
{
	...
	for (i=0; i < AXES; i++) {
		axis_time = (fabs(gm.target[i] - gm.position[i])) / CFG(i).seek_rate; // max seek rate
...
uint8_t cm_straight_feed(double x, double y, double z, double a)
{
	...
	for (i=0; i<AXES; i++) {
//		axis_time = (fabs(gm.target[i] - gm.position[i])) / CFG(i).seek_rate;
		axis_time = (fabs(gm.target[i] - gm.position[i])) / CFG(i).feed_rate; // max rate
		if (fastest_time < axis_time) {
			fastest_time = axis_time;
		}
	}
	// skip 0 length moves
	if (fastest_time < EPSILON) {
		return (TG_ZERO_LENGTH_MOVE);
	}
	// execute the move
	if (cfg.accel_enabled == TRUE) {
