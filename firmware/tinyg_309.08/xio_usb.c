...
//#include <avr/sleep.h>		// needed if blocking reads & writes
...
{
	// preempt the ISR if there are any flow control characters to send
	if (DO_XOFF(USB.flags) != FALSE) {			// read: if DO_XOFF == TRUE
		USBu.usart->DATA = XOFF; 				// send XOFF to TX DATA reg
		USB.flags &= ~XIO_FLAG_DO_XOFF_bm;		// clear XOFF flag
		USB.flags |= XIO_FLAG_IN_XOFF_bm;		// set XOFF flow control state
		return;
	}
	if (DO_XON(USB.flags) != FALSE) {
		USBu.usart->DATA = XON; 				// send XOFF to TX DATA reg
		USB.flags &= ~XIO_FLAG_DO_XON_bm;		// clear XOFF flag
		USB.flags &= ~XIO_FLAG_IN_XOFF_bm;		// clear XOFF flow control state
		return;
	}
	// otherwise send from the queue
...
//		PMIC_DisableLowLevel(); 				// disable USART TX interrupts
	...
	if (TX_MUTEX(USB.flags) == 0) {				// don't enter if func is already active
...
ISR(USB_RX_ISR_vect)	//ISR(USARTC0_RXC_vect)	// serial port C0 RX int 
{
	...
	if (c == SIG_KILL_CHAR) {	 				// trap Kill signal
		USB.signal = XIO_SIG_KILL;				// set signal value
		sig_kill();								// call sig handler
		return;
	}
/*	NOT CURRENTLY IMPLEMENTED
	if (c == SIG_TERM_CHAR) {					// trap Terminate signal
		USB.signal = XIO_SIG_KILL;
		sig_term();
		return;
	}
	if (c == SIG_PAUSE_CHAR) {					// trap Pause signal
		USB.signal = XIO_SIG_PAUSE;
		sig_pause();
		return;
	}
	if (c == SIG_RESUME_CHAR) {					// trap Resume signal
		USB.signal = XIO_SIG_RESUME;
		sig_resume();
		return;
	}
*/
	// filter out LFs if they are to be ignored
	if ((DROPLF(USB.flags) != FALSE) && (c == LF)) {
		return;
	}
	...
	if (USBu.rx_buf_head != USBu.rx_buf_tail) {	// write char unless full
		if (EN_XOFF(USB.flags) != FALSE) {		// if flow cntrl enabled 
			if (++(USBu.rx_buf_count) > XOFF_HI_WATER_MARK) {
				xio_xoff_usart(XIO_DEV_USB);
				en_bit_on(0x01);			// ++++++ light XOFF LED
			}
		}
		USBu.rx_buf[USBu.rx_buf_head] = c;
	} else { // buffer-full handling - tosses the incoming character
