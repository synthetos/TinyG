...
static uint8_t _json_parser(char *str)
{
	...
	for (i=0; i<CMD_ARRAY_SIZE; i++) {			// take action on cmdObj array
		if (cmd->value_type == VALUE_TYPE_NULL){// means GET the value
			...
			ritorno(cmd_set(cmd));
			cmd_write_NVM_value(cmd);			// persist the value to NVM
		}
...
//	strncpy(cmd->string, *pstr, CMD_STRING_LEN);// copy name from string
	strncpy(cmd->string, grp, CMD_STRING_LEN);	// prepend the group if there is a group
	strncpy(&cmd->string[strlen(grp)], *pstr, CMD_STRING_LEN); // cat group prefix to name
	if ((cmd->index = cmd_get_index(cmd->string)) == -1) { return (TG_UNRECOGNIZED_COMMAND);}
	cmd_get_token(cmd->index, cmd->token);
	strncpy(cmd->token, &cmd->string[strlen(grp)], CMD_TOKEN_LEN+1);// strip group if group
	*pstr = ++tmp;
