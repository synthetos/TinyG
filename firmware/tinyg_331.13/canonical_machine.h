...
struct GCodeModel {						// Gcode model- meaning depends on context
	...
	uint8_t absolute_override;			// G53 TRUE = move in abs coordinate - this block only (G53)
	uint8_t path_control;				// G61... EXACT_STOP, EXACT_PATH, CONTINUOUS
...
enum cmNextAction {					// these are in order to optimized CASE statement
	NEXT_ACTION_DEFAULT = 0,		// motion modes
...
uint8_t cm_set_machine_coords(double offset[]);		// G10
