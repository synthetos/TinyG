...
static uint8_t _gc_parse_gcode_block(char *buf) 
{
	...
	gn.next_action = NEXT_ACTION_DEFAULT;
		...
		switch(letter) {
			case 'G':
				switch((uint8_t)value) {
					...
					case 28: SET_NEXT_ACTION(NEXT_ACTION_GO_HOME);
					case 30: SET_NEXT_ACTION(NEXT_ACTION_HOMING_CYCLE);
					...
					case 61: {
						switch (_point(value)) {
							case 0: SET_PARAMETER(path_control, PATH_EXACT_STOP);
							case 1: SET_PARAMETER(path_control, PATH_EXACT_PATH); 
	...
	for (i=0; i<AXES; i++) {
		if (((gn.distance_mode == ABSOLUTE_MODE) || (gn.absolute_override == true)) && 
			(gf.target[i] < EPSILON)) {		
			gn.target[i] = cm_get_model_work_position(i); // get target from model
		}
	}
	return (_gc_execute_gcode_block());		// otherwise execute the block
}
...
static uint8_t _gc_execute_gcode_block()
{
	...
	switch (gn.next_action) {
		...
		case NEXT_ACTION_HOMING_CYCLE: { status = cm_homing_cycle(); break;}
