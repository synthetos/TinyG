 ...
 *	 - Note: In order to save significant RAM friendly names are discarded
 *		after they are resolved to an index The friendly name is processed 
 *		in the cmd->string field which maybe overwritten with a string value.
 *
...
static uint8_t _get_msg(cmdObj *cmd, prog_char_ptr msg, uint8_t value);
...
static uint8_t _get_sig(cmdObj *cmd);	// get device signature
static uint8_t _get_sr(cmdObj *cmd);	// run status report
static void _print_sr(cmdObj *cmd);		// run status report
static uint8_t _set_sr(cmdObj *cmd);	// set status report specification
static uint8_t _get_si(cmdObj *cmd);	// get status report interval
...
static uint8_t _get_stat(cmdObj *cmd);	// get machine state as value and string
static uint8_t _get_unit(cmdObj *cmd);	// (all these in this list are similar)
static uint8_t _get_coor(cmdObj *cmd);
static uint8_t _get_motm(cmdObj *cmd);
...
//	 string *, print func, get func, set func  target for get/set,    default value
	...
	{ str_sg, _print_int, _get_sig, _set_nul, (double *)&tg.null, 0}, 	// device signature
...
/**** DEVICE SIGNATURE ****
 * _get_sig() - get device signature
 */
static uint8_t _get_sig(cmdObj *cmd) 
{
//	uint8_t sig[11];
...
static uint8_t _get_si(cmdObj *cmd) 
{
	_get_ui8(cmd);
...
/**** Reporting functions ****************************************/
/* _get_msg()  - helper to get display message
...
//	strncpy_P(cmd->string, (PGM_P)pgm_read_word(msg+(value*2)), CMD_STRING_LEN);
	return (TG_OK);
...
{
	return(_get_msg(cmd, (prog_char_ptr)msg_stat, cm_get_machine_state()));
...
{
	return(_get_msg(cmd, (prog_char_ptr)msg_unit, cm_get_units_mode()));
}

static uint8_t _get_coor(cmdObj *cmd)
{
	return(_get_msg(cmd, (prog_char_ptr)msg_coor, cm_get_coord_system()));
}

static uint8_t _get_motm(cmdObj *cmd)
{
	return(_get_msg(cmd, (prog_char_ptr)msg_motm, cm_get_motion_mode()));
}

static uint8_t _get_plan(cmdObj *cmd)
{
	return(_get_msg(cmd, (prog_char_ptr)msg_plan, cm_get_select_plane()));
}

static uint8_t _get_path(cmdObj *cmd)
{
	return(_get_msg(cmd, (prog_char_ptr)msg_path, cm_get_path_control()));
}

static uint8_t _get_dist(cmdObj *cmd)
{
	return(_get_msg(cmd, (prog_char_ptr)msg_dist, cm_get_distance_mode()));
}
...
{
	_get_ui8(cmd);
	return(_get_msg(cmd, (prog_char_ptr)msg_am, cmd->value));	// see 331.09 for old method
}
...
{
	cmdObj cmd;
	char exclude[] = {"sr,gc"};		// don't try to SET these tokens during init
	...
	for (cmd.index=0; cmd.index<CMD_INDEX_START_GROUPS; cmd.index++) {
		if (strstr(exclude, cmd_get_token(cmd.index, cmd.token)) != NULL) continue;
	...
	if (cmd.value == cfg.version) { // Case (1) NVM is set up and current revision. Load config from NVM
		...
		for (cmd.index=0; cmd.index<CMD_INDEX_START_GROUPS; cmd.index++) {
			...
			cmd.value = (double)pgm_read_float(&cfgArray[cmd.index].def_value);
			cmd_set(&cmd);
			if (cmd_write_NVM_value(&cmd) != TG_OK) {
				fprintf_P(stderr, PSTR("WARNING; Failed to update NVM in cfg_init()\n"));
			}
...
uint8_t cfg_config_parser(char *str)
{
	...
	if ((cmd->value_type != VALUE_TYPE_PARENT) && (cmd->value_type != VALUE_TYPE_NULL)) {
		...
		cmd_write_NVM_value(cmd);			// persist value
	}
...
/*
 * cmd_persist_offset() - write G54 (et al) offsets back to NVM
 */

uint8_t cmd_persist_offset(uint8_t coord_system, uint8_t axis, double offset)
{
	cmdObj cmd;
	sprintf(cmd.token, "g%2d%c", 53+coord_system, ("xyzabc")[axis]);
...
{
	char *grp = cmd->string;				// group token in the parent cmd object
 ...
 * (see 331.09 for record-oriented routines)
 */
uint8_t cmd_read_NVM_value(cmdObj *cmd)
{
	ASSERT_INDEX(TG_INTERNAL_ERROR);
