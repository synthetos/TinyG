...
//	char sr_token_match[1] = {"sr"};
	char sr_defaults[][CMD_TOKEN_LEN+1] = {"ln","xwp","ywp","zwp","vl","gi","ms"};
...
//	}
	...
	for (uint8_t i=0; i < (sizeof(sr_defaults)/(CMD_TOKEN_LEN+1)); i++) {
		cfg.status_report_spec[i] = cmd_get_index_by_token(sr_defaults[i]);
	}
}
