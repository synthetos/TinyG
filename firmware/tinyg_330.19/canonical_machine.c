 ...
 * cm_get_path_control() - get pathc control mode from gm struct
...
inline uint8_t cm_get_select_plane() { return gm.select_plane; }
inline uint8_t cm_get_path_control() { return gm.path_control; }
...
void cm_init()
{
	ZERO_MODEL_STATE(&gn);			// clear all values, pointers and status
	ZERO_MODEL_STATE(&gf);
	ZERO_MODEL_STATE(&gm);
	ZERO_MODEL_STATE(&gt);
...
uint8_t cm_set_inches_mode(uint8_t mode)
{
	gm.inches_mode = mode;		// true = inches
	return (TG_OK);
}

uint8_t cm_set_absolute_mode(uint8_t mode)
{
	gm.absolute_mode = mode;	// true = absolute mode 
	return (TG_OK);
}
					...
					uint8_t motion_mode)			// defined motion mode
{
	...
	// trap zero feed rate condition
	if ((gm.inverse_feed_rate_mode == FALSE) && (gm.feed_rate == 0)) {
//		INFO1(PSTR("Attempted arc %s with feed rate = zero"), tg.in_buf);
		return (TG_GCODE_FEEDRATE_ERROR);
	}
