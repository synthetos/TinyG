...
static void _mp_plan_block_chain(struct mpBuffer *bf);
...
static void _mp_reset_replannable_chain(void);
 ...
 * _mp_get_first_buffer(bf)	Return pointer to first buffer, i.e. the running block
 * _mp_get_last_buffer(bf)	Return pointer to last buffer, i.e. last block (zero)
 * _mp_clear_buffer(bf)		Zero the contents of the buffer
 * _mp_copy_buffer(bf,bp)	Copy the contents of bp into bf - preserve links
 *
 * Notes:
 ...
 * mp_get_runtime_velocity() - returns current running velocity
 * mp_get_runtime_linenum() - returns current running line number
 *
...
uint8_t mp_exec_move() 
{
	...
	return (TG_ERR);	// never supposed to get here
}

/**** STOP START & END ROUTINES ****************************************
...
uint8_t mp_aline(const double target[], const double minutes)
{
	...
	if (cm_get_path_control_mode() != PATH_EXACT_STOP) { // exact stop cases already zeroed
		bf->replannable = TRUE;
	...
	// replan the blocks and commit the current block
	_mp_plan_block_chain(bf);
	copy_axis_vector(mm.position, bf->target);	// update planning position
...
/***** ALINE HELPERS *****
 * _mp_plan_block_chain()
 * _mp_calculate_trapezoid()
 * _mp_get_target_length()
 * _mp_get_target_velocity()
 * _mp_get_junction_vmax()
 * _mp_get_corner_delta()
 * _mp_reset_replannable_chain()
 */

/*
 *	_mp_plan_block_chain() - replans the entire block chain. 
 *
 *	Plans all blocks between and including the first and block provided as bf - 
 *	which is typically the last block or the new block being added (See notes).
 *	Requires the following bf variables to be set and/or in known state:
 *
 *	bf->move_type		- plans alines. Ignores DWELL, START, STOP, and END blocks
 *	bf->move_state		- should be OFF for all blocks but the earliest
 *	bf->replannable		- replanning will stop at the earliest non-replannable block [1]
 *	bf->target[]		- block target position
 *	bf->unit[]			- block unit vector
 *	bf->time			- this is unused in this operation and need not be set
 *
 *	bf->head_length		- set to desired length for that section or zero to disable
 *	bf->body_length		- set to desired length for that section or zero to disable
 *	bf->tail_length		- set to desired length for that section or zero to disable
 *
 *	bf->entry_velocity	- set to desired velocity (mandatory)
 *	bf->cruise_velocity	- set to desired velocity (mandatory)
 *	bf->exit_velocity	- set to desired velocity (mandatory)
 *
 *	bf->entry_vmax		- use existing value or set to cruise_vmax if inline splice
 *	bf->cruise_vmax		- use existing value or recompute for feed override
 *	bf->exit_vmax;		- use existing value
 *	bf->delta_vmax;		- do not set; recomputed during replanning
 *	bf->braking_velocity- do not set; recomputed during replanning
 *
 *	bf->jerk			- use existing value
 *	bf->recip_jerk		- use existing value
 *	bf->cubert_jerk		- use existing value
 *
 * Notes:
 *	[1]	Whether or not the currently running block is replanned controlled by the
 *		bf->replannable setting. If FALSE the current running block will not be 
 *		replanned; if TRUE it will be.
 *
 *		In normal operation the first block is not replanned, but may be for 
 *		feedholds and feed overrides. In these cases the prep routines modify the 
 *		contents of the mr buffer and re-shuffle the block chain, re-enlisting the 
 *		current bf buffer with new parameters. These routines also set all blocks
 *		in the chain to be replannable so the chain can be recomputed 
 *		regardless of exact stops and previous replanning optimizations.
 */
void _mp_plan_block_chain(struct mpBuffer *bf)
{
	struct mpBuffer *bp = bf;

	// Backward planning pass. 
	// Find beginning of the chain and update the braking velocities. 
	// At the end *bp points to the first buffer before the replanning chain.
...
/*
 *	_mp_reset_replannable_chain() - resets all blocks in the planning chain to be replannable
 */	
void _mp_reset_replannable_chain()
{
 ...
 *
 * mp_plan_hold()	 - replan block chain to execute hold
 * mp_release_hold() - remove the hold and restart block chain
 *
 ...
 *		time to replan the block chain for the hold before the next aline 
...
uint8_t mp_plan_hold()
{
	...
	double braking_velocity = mr.segment_velocity;
	...
	// Case: feedhold deceleration fits in remaining distance in mr buffer
	if (braking_length < remaining_length) {
		...
		bp->hold_point = true;
		_mp_reset_replannable_chain();
		_mp_plan_block_chain(_mp_get_last_buffer());

	} else { // Case: feedhold deceleration exceeds remaining distance in mr buffer
		// replan mr to minimum (but non-zero) exit velocity
		mr.move_state = MOVE_STATE_TAIL;		
		mr.section_state = MOVE_STATE_NEW;
		mr.tail_length = remaining_length;
		mr.cruise_velocity = braking_velocity;
		mr.exit_velocity = _mp_get_target_velocity(braking_velocity, remaining_length, bp);	

		// plan next buffer into current bf (advancing bp)
		braking_velocity = mr.exit_velocity;

		do {
			_mp_copy_buffer(bp, bp->nx);
			braking_length = _mp_get_target_length(braking_velocity, 0, bp);
			if (braking_length < bp->length) {	// feedhold decel fits in bp buffer
				remaining_length = bp->length - braking_length;
				bp->length = braking_length;
				bp->entry_velocity = braking_velocity;
				bp->cruise_velocity = braking_velocity;	// used for trapezoids
				bp->exit_velocity = 0;
				bp = _mp_get_next_buffer(bp);
				break;
			} else {	// feedhold deceleration does not fit in the bp buffer
				remaining_length = bp->length - braking_length;
				bp->entry_velocity = braking_velocity;
				bp->exit_velocity = _mp_get_target_velocity(braking_velocity, bp->length, bp);	
				bp = _mp_get_next_buffer(bp);
			}
		} while (bp != bf);		// cutout if for some reason it wraps

		// setup the feed release point and replan the chain
		bp->entry_velocity = 0;
		bp->length = remaining_length - braking_length;
		bp->move_state = MOVE_STATE_NEW;
		bp->hold_point = true;
	...
	// set state to decelerate and exit
...
static uint8_t _mp_exec_aline(struct mpBuffer *bf)
{
	...
	// There are 3 things that can happen here. _mp_exec_aline_xxxx returned...
	//	(1) ...TG_EAGAIN because the mr buffer has more segments to run
	//	(2) ...TG_OK (or an error) because mr and bf buffers are done
	//	(3) ...TG_OK because mr is done but bf buffer should be run again
	//		   This case is indicated by bf->move_state == MOVE_STATE_NEW

	if (status == TG_EAGAIN) {
		cm_decr_status_report();				// decrement status report counter
	} else {
		mr.move_state = MOVE_STATE_OFF;			// reset mr buffer
		mr.section_state = MOVE_STATE_OFF;
		bf->nx->replannable = FALSE;			// prevent overplanning (Note 2)
		if (bf->move_state == MOVE_STATE_RUN) {	// if it's truly the end free the run buffer
			_mp_free_run_buffer();
		}
...
static uint8_t _mp_exec_aline_head()
{
	if (mr.section_state == MOVE_STATE_NEW) {
		if (mr.head_length < MIN_LINE_LENGTH) { 
	...
	if (mr.section_state == MOVE_STATE_RUN2) {
		...
		if (_mp_exec_aline_segment() == TG_COMPLETE) {
			if ((mr.body_length < MIN_LINE_LENGTH) && 
				(mr.tail_length < MIN_LINE_LENGTH)) { // end the move
				return(TG_OK);
			}
			mr.move_state = MOVE_STATE_BODY;
			mr.section_state = MOVE_STATE_NEW;
...
static uint8_t _mp_exec_aline_body() 
{
	...
	if (mr.section_state == MOVE_STATE_RUN) {
		if (_mp_exec_aline_segment() == TG_COMPLETE) {
			if (mr.tail_length < MIN_LINE_LENGTH) { // end the move
				return(TG_OK);
			} 
			mr.move_state = MOVE_STATE_TAIL;
			mr.section_state = MOVE_STATE_NEW;
...
{
	if (mr.section_state == MOVE_STATE_NEW) {
		if (mr.tail_length < MIN_LINE_LENGTH) { 
