...
// Put display strings in program memory. The order is very important
...
void cm_print_machine_state()
{
	...
	fprintf_P(stderr,(PGM_P)gmsFeed, gm.feed_rate * conversion, units);
	fprintf_P(stderr,(PGM_P)gmsPosX, vector[X] * conversion, units);
	fprintf_P(stderr,(PGM_P)gmsPosY, vector[Y] * conversion, units);
	fprintf_P(stderr,(PGM_P)gmsPosZ, vector[Z] * conversion, units);
	fprintf_P(stderr,(PGM_P)gmsPosA, vector[A]);
	fprintf_P(stderr,(PGM_P)gmsPosB, vector[B]);
	fprintf_P(stderr,(PGM_P)gmsPosC, vector[C]);
	fprintf_P(stderr,(PGM_P)gmsOfsI, gm.offset[0] * conversion, units);
	fprintf_P(stderr,(PGM_P)gmsOfsJ, gm.offset[1] * conversion, units);
	fprintf_P(stderr,(PGM_P)gmsOfsK, gm.offset[2] * conversion, units);
...
// put state display strings into program memory array
static char sts00[] PROGMEM = "\"reset\"";
static char sts01[] PROGMEM = "\"run\"";
static char sts02[] PROGMEM = "\"stop\"";
static char sts03[] PROGMEM = "\"hold\"";
static char sts04[] PROGMEM = "\"resume\"";
static PGM_P stsMsg[] PROGMEM = { sts00, sts01, sts02, sts03, sts04 };
...
//	fprintf_P(stderr,PSTR(", \"xyz\":[%1.4f,%1.4f,%1.4f]"),  vector[X], vector[Y], vector[Z]);
	...
	fprintf_P(stderr,PSTR(", \"stat\":%S"),(PGM_P)pgm_read_word(&stsMsg[cm.machine_state]));
