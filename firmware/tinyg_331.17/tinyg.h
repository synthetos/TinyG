 ...
 */
/* TODO
	- check on semantics of absokute override (G53) I don't think this is correct.
	- verify coordinate system handling in homing - particularly  cm_set_machine_coords()
	- change disable Acceleration to use the planner but set jerk to infinity. Comment out mp_line & continuation
	- update settings files for other machines
	- add a command for flushing he planning buffer to support incremental jogging
	- complete spindle handling
	- Make feedhold stop the spindle, too
	- add PWM channel / integrate with spindle
	- make rotary planar computations take motion mode into account 
	- get chip ID and install as read-only parameter
	- integrate M2, M30 PROGRAM_FLOW with machine state
	- comma tolerance and defacto separator for _parse_config_string()
	- dual-axis homing
	- update readme and wiki: correct tokens for configs. Add new ones
	- copy page from wiki
	- test, test, test
  DONE
	- check out why cmds are called with (i,cmd) and not just (cmd)
	- get velocity down to zero wne machine has stopped
	- suppress status report display on reset
	- integrate status report persistence
	- reduce NVM storage to a single word; eliminate migration function
	- fix "persist coordinate offsets" - this is wrong
	- fix auto-line numbering
	- test G92 / G92.1 / G92.2 / G92.3
	- install $m, $n, $$
*/
...
#define TINYG_BUILD_NUMBER   	331.17
#define TINYG_CONFIG_VERSION	TINYG_BUILD_NUMBER
