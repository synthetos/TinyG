...
//static void _print_group_list(cmdObj *cmd, char *list[]); // helper to print multiple groups
...
static uint8_t _get_motm(cmdObj *cmd)
{
	return(_get_msg_helper(cmd, (prog_char_ptr)msg_motm, cm_get_motion_mode()));
}
 ...
 * cmd_is()					- returns true if comd matches input string
 *
...
/*
uint8_t cmd_is(cmdObj *cmd, const char *str)
{
	if (strstr(cmd->token, str) != NULL) {
		return (true);
	}
	return (false);
}
