...
#include <stdio.h>				// needd for FILE def'n
...
struct tgController {			// main controller struct
	...
	FILE * std_in;				// stdin device assignment
	FILE * std_out;				// stdout device assignment
	FILE * std_err;				// stderr device assignment
	uint8_t i;					// temp for indexes

	double position[4];			// buffer for current position from gcode.c
	char msg[32];				// message buffer for traps
