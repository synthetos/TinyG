...
//#include <util/delay.h>	// uncomment if using optional step pulse delay
 ...
 *	The step bit pulse witdth must be > 1 uSec - that's why its separated.
 * 
 *	Note that the A axis is also used to time out dwells
 */
...
ISR(MOTOR_1_TIMER_ISR_vect)
{
	...
	if (!--ax.a[MOTOR_1].step_counter) {
		...
		ax.active_axes &= ~MOTOR_1_ACTIVE_BIT_bm;// clear motor active bit
		if (CFG(MOTOR_1).power_mode) {
			MOTOR_1_PORT.OUTSET = MOTOR_ENABLE_BIT_bm; // disable motor
		}
	}
	MOTOR_1_PORT.OUTCLR = STEP_BIT_bm;		// TURN STEP BIT OFF

	// end-of-move processing
	if (!ax.active_axes) {					// if all axes are done
...
{
	if (--ax.a[MOTOR_2].postscale_counter) { return; }
	if (ax.stopped) { return;}
	...
	if (!--ax.a[MOTOR_2].step_counter) {
		...
		if (CFG(MOTOR_2).power_mode) { MOTOR_2_PORT.OUTSET = MOTOR_ENABLE_BIT_bm; }
	}
...
{
	if (--ax.a[MOTOR_3].postscale_counter) { return; }
	if (ax.stopped) { return;}
	if (ax.line_mode) {	 MOTOR_3_PORT.OUTSET = STEP_BIT_bm;	}
	...
	if (!--ax.a[MOTOR_3].step_counter) {
		...
		if (CFG(MOTOR_3).power_mode) { MOTOR_3_PORT.OUTSET = MOTOR_ENABLE_BIT_bm; }
	}
...
{
	if (--ax.a[MOTOR_4].postscale_counter) { return; }
	if (ax.stopped) { return;}
	...
	if (!--ax.a[MOTOR_4].step_counter) {
		...
		if (CFG(MOTOR_4).power_mode) { MOTOR_4_PORT.OUTSET = MOTOR_ENABLE_BIT_bm; }
		ax.active_axes &= ~MOTOR_4_ACTIVE_BIT_bm;
	}
	MOTOR_4_PORT.OUTCLR = STEP_BIT_bm;
	if (!ax.active_axes) { st_execute_move(); }
}
...
uint8_t st_execute_move()
{
	...
	for (i = 0; i < MOTORS; i++) {
		...
		// (in this case the ternary is same as an if / else)
...
//		ax.a[i].timer_period = ax.p->a[i].period;	// UNUSED
		ax.a[i].step_counter = ax.p->a[i].steps;
		ax.a[i].postscale_value = ax.p->a[i].postscale;
		ax.a[i].postscale_counter = ax.a[i].postscale_value;
		ax.a[i].port->OUTCLR = MOTOR_ENABLE_BIT_bm;	// enable motor
		ax.a[i].timer->PER = ax.p->a[i].period;		// load and...
		ax.a[i].timer->CTRLA = TC_CLK_ON;			//... enable timer
		ax.active_axes |= (1<<i);					// set active bit
	}
	ax.mutex = FALSE;
	return (TG_OK);
}
