...
static int _tg_print_gcode_help_screen(void);
...
uint8_t gc_gcode_parser(char *block)
{
	...
	if (!(gp.status = _gc_parse_gcode_block(block))) { // parse & exec block or fail
...
int _gc_next_statement(char *letter, double *value_ptr, double *fraction_ptr, 
	...
	*letter = buf[*i];
	...
	(*i)++;
	if (!gc_read_double(buf, i, value_ptr)) {
		return(FALSE);
	}
...
int gc_read_double(char *buf, uint8_t *i, double *double_ptr) 
{
	char *start = buf + *i;
	char *end;
...
//			case 'B': SET_NEXT_STATE(target[B], gp.value);	// not supported
//			case 'C': SET_NEXT_STATE(target[C], gp.value);	// not supported
	...
	for (i = 0; i < AXES; i++) {
		if ((gn.absolute_mode || gn.absolute_override) && (!gf.target[i])) {
...
int _gc_execute_gcode_block() 
{
	...
	if (gn.next_action == NEXT_ACTION_GO_HOME) {
		if ((gp.status = cm_homing_cycle())) {		// initiates a homing cycle
			return (gp.status);						// error return
		}
	}
	...
	if (gf.set_origin_mode) {
		if ((gp.status = cm_set_origin_offsets(
						 gn.target[X], gn.target[Y], 
						 gn.target[Z], gn.target[A]))) {
			return (gp.status);		// error return
		}
	}
	...
	if ((gn.next_action == NEXT_ACTION_MOTION) && 
		...
		gp.status = cm_straight_traverse(gn.target[X], gn.target[Y], 
										 gn.target[Z], gn.target[A]);
		return (gp.status);
	}
	...
	if ((gn.next_action == NEXT_ACTION_MOTION) && 
		...
		gp.status = cm_straight_feed(gn.target[X], gn.target[Y], 
									 gn.target[Z], gn.target[A]);
		return (gp.status);
	}
	...
	if ((gn.next_action == NEXT_ACTION_MOTION) &&
	   ((gn.motion_mode == MOTION_MODE_CW_ARC) || 
		...
		gp.status = cm_arc_feed(gn.target[X], gn.target[Y], 
								gn.target[Z], gn.target[A],
...
int _tg_print_gcode_help_screen(void)
{
	fprintf_P(tg.std_err, PSTR("*** GCODE Help ***\n\
