...
#define __TRAPS				// enable trap generation
...
// USAGE: 	TRAP_GT(m->length,100,"Length: %f")
#ifdef __TRAPS
#define TRAP(a,m) { sprintf_P(tg.msg,m,a); tg_trap(tg.msg);}
#define TRAP_GT(a,b,m)	{if (a>b) { sprintf_P(tg.msg,m,a); tg_trap(tg.msg);}}
#define TRAP_LT(a,b,m)	{if (a<b) { sprintf_P(tg.msg,m,a); tg_trap(tg.msg);}}
#define TRAP_EQ(a,b,m)	{if (a=b) { sprintf_P(tg.msg,m,a); tg_trap(tg.msg);}}
#define TRAP_ZERO(a,m)	{if (a<ROUNDING_ERROR) { sprintf_P(tg.msg,m,a); tg_trap(tg.msg);}}
//#define TRAP_GT(a,b,m)	{if (a>b) { sprintf(tg.msg,m,a); tg_trap(tg.msg);}}
//#define TRAP_LT(a,b,m)	{if (a<b) { sprintf(tg.msg,m,a); tg_trap(tg.msg);}}
//#define TRAP_EQ(a,b,m)	{if (a=b) { sprintf(tg.msg,m,a); tg_trap(tg.msg);}}
//#define TRAP_ZERO(a,m)	{if (a<ROUNDING_ERROR) { sprintf(tg.msg,m,a); tg_trap(tg.msg);}}
#else
