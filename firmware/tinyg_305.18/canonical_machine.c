...
uint8_t cm_set_origin_offsets(double x, double y, double z, double a)
{
	...
	mp_set_position(gm.position[X], gm.position[Y], 
...
uint8_t cm_straight_traverse(double x, double y, double z, double a)
{
	...
	if (cfg.accel_enabled) {
		cm_status = mp_aline(gm.target[X], gm.target[Y], 
							 gm.target[Z], gm.target[A], longest_time);
	} else {
		cm_status = mp_line(gm.target[X], gm.target[Y], 
							gm.target[Z], gm.target[A], longest_time);
	}
...
uint8_t cm_run_homing_cycle()			// outer wrapper
{
	...
	if (mp_isbusy()) {   				// sync to the move queue
		return (TG_EAGAIN); 
	}
...
uint8_t _cm_run_homing_cycle()			// inner routine
{
	...
	if (CFG(Y).homing_enable && cy.state == CY_STATE_HOMING_Y_START) {
	...
	if (CFG(Z).homing_enable && cy.state == CY_STATE_HOMING_Z_START) {
	...
	if (CFG(A).homing_enable && cy.state == CY_STATE_HOMING_A_START) {
		cy.state = CY_STATE_HOMING_A_WAIT;
		cm_set_feed_rate(CFG(A).homing_rate);
	...
	if (cy.state != CY_STATE_HOMING_RTZ_WAIT) {
		...
		mp_set_position(gt.position[X], gt.position[Y], // MP layer must agree...
