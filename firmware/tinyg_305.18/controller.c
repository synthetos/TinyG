 ...
 *
 *	The first letter of an TEST mode line performs the following actions
 *
 ...
 *	selects. All modes use Q (Quit) to exit and return to test mode.
 */
...
static int _tg_print_test_help_screen(void);
...
void tg_init() 
{
	...
	// setup std devices defaults for standalone and master modes
	tg.std_in = fdev_usb;
	tg.std_out = fdev_usb;
	tg.std_err = fdev_usb;
#ifdef __SLAVE_MODE
	tg.std_in = fdev_rs485;		// override defaults
	tg.std_out = fdev_rs485;
#endif
}
...
void tg_alive() 
{
	...
	fprintf_P(tg.std_err, PSTR("===> TinyG %S\n"), (PSTR(TINYG_VERSION)));
	fprintf_P(tg.std_err, PSTR("     type 'h' for help\n"));
	_tg_prompt();
}
 ...
 *
 *	Memory usage notes:
 *		- The print to stderr is program memory
 *		- the msg passed to it is in RAM
 *		- typically the strings that get loaded into the RAM buffer 
 *		  are from program memory - see tinyg.h TRAP macros
 */

void tg_trap(char *msg)
{
#ifndef __SIMULATION_MODE
	fprintf_P(tg.std_err, PSTR("#### TRAP %s ####\n"), msg);
#endif
	return;
}
...
//		fprintf_P(tg.std_err, PSTR("Read next line %s\n"), tg.buf);	// ++++++ debug
	...
	if (tg.status == TG_EOF) {
		fprintf_P(tg.std_err, PSTR("End of command file\n"));
...
//			case 'V': return (_tg_reserved());	// reserved
			case 'H': return (_tg_print_test_help_screen());
...
{
	fprintf_P(tg.std_err, PSTR("tinyg[%S]*> "),(PGM_P)pgm_read_word(&tgModeStrings[tg.mode]));
...
static int _tg_print_test_help_screen(void)
{
	fprintf_P(tg.std_err, PSTR("*** TinyG Test Screen Help ***\n\
...
//	xio_queue_RX_string_usb("g0 x200\n");
//	xio_queue_RX_string_usb("g0 x20\n");

//	xio_queue_RX_string_usb("g1 f300 y20\n");		// G1's
