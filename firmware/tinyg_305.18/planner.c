...
#include <avr/pgmspace.h>		// for PSTR definition
...
static uint8_t _mp_set_braking_velocity(struct mpMovePlanner *p, struct mpMovePlanner *m);
...
//	if (mr.run_move == NULL) {
//		TRAP(mr.run_move, PSTR("mr.run_move is null: %f"));
//	}
 ...
 *	still supporting an accelereation to zero.
 */
 ...
 *	as a set of move buffers (buufers for head, body and tail, but 
 ...
 *	prior to the new move are recomptued (backplanned) to blend with the 
 ...
 *						new line, whcih accelerates to its cruise velocity
 *
...
//	TRAP_GT(m->length,100,PSTR("Length: %f"))
//	TRAP_GT(m->target_velocity,1000,PSTR("Speed: %f"))
	TRAP_LT(m->cruise_velocity,20,PSTR("Speed: %f"))
...
//			sprintf(tg.msg, "Backplanning error: %f", m->length);
			sprintf_P(tg.msg, PSTR("Backplanning error: %f"), m->length);
			tg_trap(tg.msg);
			break;
		}
	}
...
//			sprintf(tg.msg, "Lookback error: %f", m->length);
			sprintf_P(tg.msg, PSTR("Lookback error: %f"), m->length);
			tg_trap(tg.msg);
			break;
		}
...
static uint8_t _mp_compute_regions(double Vir, double Vt, double Vf, struct mpMovePlanner *m) 
{
	...
	if (m->length < MIN_LINE_LENGTH) {	// line is too short or zero
		TRAP(m->length, PSTR("Line too short"));
		return (0);
	}
...
//			sprintf(tg.msg, "Iteration error: %f", m->body_length);
			sprintf_P(tg.msg, PSTR("Iteration error: %f"), m->body_length);
			tg_trap(tg.msg);
			break;
		}
...
//		sprintf(tg.msg, "Region error: %f", m->body_length);
		sprintf_P(tg.msg, PSTR("Region error: %f"), m->body_length);
		tg_trap(tg.msg);
	}
	m->body_length = 0;
...
//	TRAP_ZERO(b->end_velocity, PSTR("End Velocity: %f"))	
	b->time = b->length / b->end_velocity;		// get time from length
	TRAP_ZERO(b->time, PSTR("Time: %f"))	
	mr.microseconds = uSec(b->time);
...
static uint8_t _mp_run_accel(struct mpBuffer *b)
{
	...
	if (b->move_state == MP_STATE_NEW) {
		...
		TRAP_ZERO(mr.midpoint_velocity, PSTR("Accel Midpoint Velocity: %f"))	
		...
		TRAP_ZERO(mr.segments, PSTR("Accel Segments: %f"))
...
static uint8_t _mp_run_decel(struct mpBuffer *b)
{
	...
	if (b->move_state == MP_STATE_NEW) {
		...
		mr.midpoint_velocity = (b->start_velocity + b->end_velocity) / 2;
		TRAP_ZERO(mr.midpoint_velocity, PSTR("Decel Midpoint Velocity: %f"))	
		mr.time = b->length / mr.midpoint_velocity;
		mr.midpoint_acceleration = mr.time * mm.linear_jerk_div2;
		for (uint8_t i=0; i < AXES; i++) {
		...
		// number of segments in *each half*
		mr.segments = round(round(uSec(mr.time / cfg.min_segment_time)) / 2);
		TRAP_ZERO(mr.segments, PSTR("Decel Segments: %f"))
		if (isinf(mr.segment_time = mr.time / (2 * mr.segments))) {
			return (TG_OK);					// cancel the move if too small
		}
		mr.microseconds = uSec(mr.segment_time);
...
//	TRAP_ZERO(b->end_velocity, PSTR("Finalize end_velocity is %f"))
	mr.time = mr.length / b->end_velocity; // ++++ 1#INF problem here (get time from length
	TRAP_ZERO(b->time, PSTR("Finalize time is %f"))
