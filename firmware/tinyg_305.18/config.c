...
struct cfgSetting {				// don;t mess with the order of the struct
	...
	uint8_t mnemonic;			// key: numeric token for mnemonic
...
static char *_cfg_get_mnemonic_string(const uint8_t mnum);
...
static int16_t _cfg_get_setting_index(const int8_t axis, const uint8_t mnemonic);
static struct cfgSetting *_cfg_get_setting_by_key(const int8_t axis, const uint8_t mnemonic);
...
static struct cfgSetting *_cfg_get_NVM_setting_by_key(const int8_t axis, const uint8_t mnemonic);
...
struct cfgSetting cfgList[COUNT_SETTINGS] = { 
	...
	{ NON_AXIS, EA, fmtEA, _applyEA, (double) 1 },
...
{
	fprintf_P(tg.std_err, PSTR("*** TinyG Configuration Help ***\n\
 ...
 *	config text blocks that are re-hydrated so they can be re-parsed to 
 *	populate the structs. They look like this:
 *
...
void cfg_dump_NVM(uint16_t start_record, uint16_t end_record, char *label)
{
	...
	for (uint16_t i=start_record; i<end_record;i++) {
		...
		fprintf_P(tg.std_err, PSTR("Record %d - %d %d %d %d %d %d [%c%s%1.2f]\n"),
									i, axis, mnemonic, 
...
static struct cfgSetting *_cfg_make_setting(const int8_t axis, const uint8_t mnemonic, const double value);
