...
uint8_t _mq_map_axis_to_motor(uint8_t axis);
...
/*
 * _mq_map_axis_to_motor()
 * _mq_map_motor_to_axis()
 */

uint8_t _mq_map_axis_to_motor(uint8_t axis)
{
	return cfg.a[axis].map_axis;
}

uint8_t _mq_map_motor_to_axis(uint8_t motor)
{
	return cfg.motor_map[motor];	// works from motors 0-3
}
...
//					  int32_t steps_b, int32_t steps_c, 
...
//		return (mq_queue_dwell(microseconds));		// queue it as a dwell
	...
	// map axis steps into motor structs
	int32_t steps[AXES] = { steps_x, steps_y, steps_z, steps_a };
	for (uint8_t i=0; i<AXES; i++) {
//		mq.p->a[i].period = 0;		// initialize
//		mq.p->a[i].postscale = 0;
//		mq.p->a[i].direction = 0;
		for (uint8_t j=0; j<MOTORS; j++) {
			if (cfg.motor_map[j] == i) {
				mq.p->a[j].steps = steps[i];
			}				
		}
	}
	// setup time base
	...
	// iterate through the motor structs
	for (uint8_t i = 0; i < MOTORS; i++) {
		if (mq.p->a[i].steps) { 				// skip motors w/ zero steps
			// set direction: (polarity is corrected during execute move)
			(mq.p->a[i].steps < 0) ? (mq.p->a[i].direction = 1): // CCW = 1
