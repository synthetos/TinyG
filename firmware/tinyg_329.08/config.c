 ...
 */
/*
 * See tinyg_docs.txt for a description of CONFIG INTERNALS.
 *
 * Adding a new config setting:
 *---> first steps:
 *	- add the setting to the global cfg struct in config.h (or wherever)
 *	- define a default value for it in settings.h and/or machine config files
 *---> in the SETTING-SPECIFIC REGION in this file:
 *	- add a non-colliding mnemonic to cfgMnemonics & mnemonics (see note 1)
 ...
 *
 * Notes:
 * 1) Settings are specified as a 2 character mnemonic preceded by a group.
 *	  Groups are things like axis specifiers (e.g. Y), motor specifiers,
 *	  (e.g. 2), or the general group (no prefix) - which carries all the
 *	  non-axis and non-motor settings.
 *
 *	  Mnemonics within an axis or motor group can be any 2 alpha chars.
 *	  Digits are not allowed. 
 *
 *	  Mnemonics for the general group cannot start with a group identifier,
 *	  i.e. they can't start w\an axis name or a motor number or PWM number.
 *		  - these are off limits for 1st chars: X,Y,Z,A,B,C,U,V,W
 *		  - these are also off limits: 			1,2,3,4,5,6,7,8,9,0
 *
 * 2) The order of display is set by the order of the settingList struct.
 *	  None of the other orders matter but are kept sequenced for easier 
 *	  reading and code maintenance.
 *
 * 3) Gcode defaults are held in the cfg struct as their "G" value, 
 ...
 *	  This affects how settings are displayed and entered.
 *	  ABC axes always use degrees regardless of prevailing unit mode
 */
/* TROUBLESHOOTING
 *	Unfortunatly this subsystem is more brittle than I'd like due to the fact
 *	that there are a lot of counts and indicies that must line up. I'd like to 
 *	refactor this to make it more robust. In the mean time, if you make changes 
 *	and have errors or unexplained behaviors check the following:
 *
 *	- Do the cfgMnemonics and *mnemonics arrays line up? Tokens and literals must agree
 *
 *  - Are all the COUNT_SETTINGS correct? The final COUNT_SETTINGS must be correct.
 *	  Check the contributions of the subordinate COUNT_ defines
 *
 *	- Is the cfgSetting cfgList[COUNT_SETTINGS] array properly sized? If this is 
 *	  off all havoc breaks loose. Make sure each motor block and axis block has 
 *	  all elements it needs. Check the general group to make sure you haven't left 
 *	  anything out. Count the total elements in the array and check against the 
 *	  value of COUNT_SETTINGS.
 *
 *	- Are you missing an _apply function or _fmt string? The compiler should let you know, 
 *
 *	Some thoughts on refactoring:
 *
 *	- Break the master array into smaller arrays of similar typed objects. Have 
 *	  separate arrays for the general group, motors and axes (and PWM when available). 
 *	  This eliminates the GRP construct and could allow 2 dimensional arrays for the 
 *	  motors and axes - which would simplify things
 *
 *	- Differentiate units by dynamic binding. Instead of carrying separate mm and inches
 *	  _fmt and _apply bindings carry a single _fmt and _apply binding for each item.
 *	  provide a _bind function that takes units as an argument (mm, inch, degrees)
 *	  and is called any time units are changed. This may also save some RAM space
 */
...
#define CFG_VERSION ((double)0.93) // NVM version - NOT firmware revision
 ...
 */
#define UNIT_CONVERSION_REQUIRED (s->app_mm != s->app_in)
 ...
 *	or you will either get "warning: excess elements in array initializer" 
 *	error or find that at some point in the list the menmonic and the 
 *	setting don't agree (it gets off-by-one, or some similar error).
 ...
 */
enum cfgGroups {
	CFG_GENERAL = -1,			// general group is negative
	...
	CFG_PWM2					//... this list can be extended
};
...
enum cfgMnemonics { P_, V_,		// <-- profile and version records. Must be first.

	// axis settings			// see display formats for definitions
	MO, SR, FR, TH, TS, 
	JM, CD, RA, SW, HS, 
	HL, HZ, HW,					// 13 axis settings (adjust #define COUNT_AXIS_SETTINGS)

	// motor settings
	MA, SA, TR, MI, PO,
	PW, 						// 6 motor settings (adjust #define COUNT_MOTOR_SETTINGS)

	// PWM settings
								// 0 PWM settings (adjust #define COUNT_PWM_SETTINGS)
	// general settings
	GU, GL, GP, GD, EA, 
	JA, ML, MC, MT, EC, 
	IC, IL, EX, EE, SE,
	SI,							// 16 general settings (adjust #define COUNT_GENERAL)
...
#define COUNT_AXIS_SETTINGS 13	// <-- this must agree with above
#define COUNT_MOTOR_SETTINGS 6	// <-- this must agree with above
#define COUNT_PWM_SETTINGS 0	// <-- this must agree with above
#define COUNT_GENERAL 16		// <-- this must agree with above
...
static void _applyCP(struct cfgSetting *s);
static void _applyCV(struct cfgSetting *s);

static void _applyMO(struct cfgSetting *s);
static void _applySW(struct cfgSetting *s);
...
static void _appmmJM(struct cfgSetting *s);
static void _appmmCD(struct cfgSetting *s);
static void _appmmRA(struct cfgSetting *s);
static void _appmmHS(struct cfgSetting *s);
static void _appmmHL(struct cfgSetting *s);
static void _appmmHZ(struct cfgSetting *s);
static void _appmmHW(struct cfgSetting *s);
...
static void _appinCD(struct cfgSetting *s);
static void _appinRA(struct cfgSetting *s);
static void _appinHS(struct cfgSetting *s);
static void _appinHL(struct cfgSetting *s);
static void _appinHZ(struct cfgSetting *s);
static void _appinHW(struct cfgSetting *s);

// axis settings - degree units
// these are pass throughs to MM functions - use macros instead
#define _appdgSR _appmmSR
#define _appdgFR _appmmFR
#define _appdgTH _appmmTH
#define _appdgTS _appmmTS
#define _appdgJM _appmmJM
#define _appdgCD _appmmCD
#define _appdgHS _appmmHS
#define _appdgHL _appmmHL
#define _appdgHZ _appmmHZ
#define _appdgHW _appmmHW
...
// general settings
static void _appmmJA(struct cfgSetting *s);
static void _appinJA(struct cfgSetting *s);
#define _appdgJA _appmmJA
static void _appmmML(struct cfgSetting *s);
static void _appinML(struct cfgSetting *s);
static void _appmmMC(struct cfgSetting *s);
static void _appinMC(struct cfgSetting *s);
...
static void _applySE(struct cfgSetting *s);
static void _applySI(struct cfgSetting *s);
...
char fmtSW[] PROGMEM = "SWitch mode        %5.0f [0,1]        $%c%s%1.0f\n";
...
char fmmJM[] PROGMEM = "Jerk maximum  %10.0f mm/min^3     $%c%s%1.0f\n";
char fmmCD[] PROGMEM = "Cornering delta   %5.4f mm           $%c%s%1.4f\n";
char fmmHS[] PROGMEM = "Homing Search vel. %5.0f mm/min       $%c%s%1.0f\n";
char fmmHL[] PROGMEM = "Homing Latch vel.  %5.0f mm/min       $%c%s%1.0f\n";
char fmmHZ[] PROGMEM = "Homing Zero offset %5.0f mm           $%c%s%1.0f\n";
char fmmHW[] PROGMEM = "Homing Work offset %5.0f mm           $%c%s%1.0f\n";
char fmmRA[] PROGMEM = "Radius value    %8.3f mm           $%c%s%1.3f\n";

// axis settings - inches mode (applies to XYZ axes)
char finSR[] PROGMEM = "Seek rate       %8.2f in/min       $%c%s%1.2f\n";
char finFR[] PROGMEM = "Feed rate       %8.2f in/min       $%c%s%1.2f\n";
char finTH[] PROGMEM = "Travel hard limit  %5.0f inches       $%c%s%1.0f\n";
char finTS[] PROGMEM = "Travel soft limit  %5.0f inches       $%c%s%1.0f\n";
char finJM[] PROGMEM = "Jerk maximum  %10.0f in/min^3     $%c%s%1.0f\n";
char finCD[] PROGMEM = "Cornering delta   %5.4f in           $%c%s%1.4f\n";
char finHS[] PROGMEM = "Homing Search vel %6.2f in/min       $%c%s%1.2f\n";
char finHL[] PROGMEM = "Homing Latch vel. %6.2f in/min       $%c%s%1.2f\n";
char finHZ[] PROGMEM = "Homing Zero offset %5.2f inches       $%c%s%1.2f\n";
char finHW[] PROGMEM = "Homing Work offset %5.2f inches       $%c%s%1.2f\n";
...
char fdgJM[] PROGMEM = "Jerk maximum  %10.0f deg/min^3    $%c%s%1.0f\n";
//char fdgCD[] PROGMEM = "Cornering delta  %5.4f deg          $%c%s%1.4f\n";
char fdgHS[] PROGMEM = "Homing Search vel. %5.0f deg/min      $%c%s%1.2f\n";
char fdgHL[] PROGMEM = "Homing Latch vel.  %5.0f deg/min      $%c%s%1.2f\n";
char fdgHZ[] PROGMEM = "Homing Zero offset %5.0f degrees      $%c%s%1.2f\n";
char fdgHW[] PROGMEM = "Homing Work offset %5.0f degrees      $%c%s%1.2f\n";
...
char fmmJA[] PROGMEM = "Junction acceleration      %6.0f mm/min^2     $%s%1.0f\n";
char finJA[] PROGMEM = "Junction acceleration      %6.0f in/min^2     $%s%1.0f\n";
char fmmML[] PROGMEM = "Min line segment length     %5.3f mm           $%s%1.4f\n";
char finML[] PROGMEM = "Min line segment length    %5.4f inches       $%s%1.4f\n";
char fmmMC[] PROGMEM = "Min arc segment length      %5.3f mm           $%s%1.4f\n";
char finMC[] PROGMEM = "Min arc segment length     %5.4f inches       $%s%1.4f\n";
...
// other gneral settings
char fmtSE[] PROGMEM = "Status Report Enable            %1.0f [0,1,2]      $%s%1.0f\n";
char fmtSI[] PROGMEM = "Status Report Interval        %3.0f ms           $%s%2.0f\n";
 ...
 *	and there are quite a lot of them. In a different world I'd just read 
 *	a config file or two and be done with all this. I'm not going to worry 
 *	about further memory optimization at this point, but could be achieved 
 *	with a combined "do-all" print and apply function (which I'm told is an 
 *	"anti-pattern").
 */

struct cfgSetting cfgList[COUNT_SETTINGS] = { 
	// starting version.profile record - must be first. Dont' mess with these.
	{ CFG_GENERAL, P_, fmtP_, fmtP_, _applyCP, _applyCP, (double) CFG_PROFILE },
	{ CFG_GENERAL, V_, fmtV_, fmtV_, _applyCV, _applyCV, (double) CFG_VERSION },
	...
	{ CFG_X, JM, fmmJM, finJM, _appmmJM, _appinJM, (double) X_JERK },
	{ CFG_X, CD, fmmCD, finCD, _appmmCD, _appinCD, (double) X_CORNER_DELTA },
	{ CFG_X, RA, fmmRA, finRA, _appmmRA, _appinRA, (double) 0 },
	{ CFG_X, SW, fmtSW, fmtSW, _applySW, _applySW, (double) X_SWITCH_MODE },
	{ CFG_X, HS, fmmHS, finHS, _appmmHS, _appinHS, (double) X_HOMING_SEARCH_VELOCITY },
	{ CFG_X, HL, fmmHL, finHL, _appmmHL, _appinHL, (double) X_HOMING_LATCH_VELOCITY },
	{ CFG_X, HZ, fmmHZ, finHZ, _appmmHZ, _appinHZ, (double) X_HOMING_ZERO_OFFSET },
	{ CFG_X, HW, fmmHW, finHW, _appmmHW, _appinHW, (double) X_HOMING_WORK_OFFSET },
	...
	{ CFG_Y, JM, fmmJM, finJM, _appmmJM, _appinJM, (double) Y_JERK },
	{ CFG_Y, CD, fmmCD, finCD, _appmmCD, _appinCD, (double) Y_CORNER_DELTA },
	{ CFG_Y, RA, fmmRA, finRA, _appmmRA, _appinRA, (double) 0 },
	{ CFG_Y, SW, fmtSW, fmtSW, _applySW, _applySW, (double) Y_SWITCH_MODE },
	{ CFG_Y, HS, fmmHS, finHS, _appmmHS, _appinHS, (double) Y_HOMING_SEARCH_VELOCITY },
	{ CFG_Y, HL, fmmHL, finHL, _appmmHL, _appinHL, (double) Y_HOMING_LATCH_VELOCITY },
	{ CFG_Y, HZ, fmmHZ, finHZ, _appmmHZ, _appinHZ, (double) Y_HOMING_ZERO_OFFSET },
	{ CFG_Y, HW, fmmHW, finHW, _appmmHW, _appinHW, (double) Y_HOMING_WORK_OFFSET },
	...
	{ CFG_Z, JM, fmmJM, finJM, _appmmJM, _appinJM, (double) Z_JERK },
	{ CFG_Z, CD, fmmCD, finCD, _appmmCD, _appinCD, (double) Z_CORNER_DELTA },
	{ CFG_Z, RA, fmmRA, finRA, _appmmRA, _appinRA, (double) 0 },
	{ CFG_Z, SW, fmtSW, fmtSW, _applySW, _applySW, (double) Z_SWITCH_MODE },
	{ CFG_Z, HS, fmmHS, finHS, _appmmHS, _appinHS, (double) Z_HOMING_SEARCH_VELOCITY },
	{ CFG_Z, HL, fmmHL, finHL, _appmmHL, _appinHL, (double) Z_HOMING_LATCH_VELOCITY },
	{ CFG_Z, HZ, fmmHZ, finHZ, _appmmHZ, _appinHZ, (double) Z_HOMING_ZERO_OFFSET },
	{ CFG_Z, HW, fmmHW, finHW, _appmmHW, _appinHW, (double) Z_HOMING_WORK_OFFSET },
	...
	{ CFG_A, JM, fdgJM, fdgJM, _appdgJM, _appdgJM, (double) A_JERK },
	{ CFG_A, CD, fmmCD, finCD, _appmmCD, _appinCD, (double) A_CORNER_DELTA },
	{ CFG_A, RA, fmmRA, finRA, _appmmRA, _appinRA, (double) A_RADIUS },
	{ CFG_A, SW, fmtSW, fmtSW, _applySW, _applySW, (double) A_SWITCH_MODE },
	{ CFG_A, HS, fdgHS, fdgHS, _appdgHS, _appdgHS, (double) A_HOMING_SEARCH_VELOCITY },
	{ CFG_A, HL, fdgHL, fdgHL, _appdgHL, _appdgHL, (double) A_HOMING_LATCH_VELOCITY },
	{ CFG_A, HZ, fdgHZ, fdgHZ, _appdgHZ, _appdgHZ, (double) A_HOMING_ZERO_OFFSET },
	{ CFG_A, HW, fdgHW, fdgHW, _appdgHW, _appdgHW, (double) A_HOMING_WORK_OFFSET },
	...
	{ CFG_B, JM, fdgJM, fdgJM, _appdgJM, _appdgJM, (double) B_JERK },
	{ CFG_B, CD, fmmCD, finCD, _appmmCD, _appinCD, (double) B_CORNER_DELTA },
	{ CFG_B, RA, fmmRA, finRA, _appmmRA, _appinRA, (double) B_RADIUS },
	{ CFG_B, SW, fmtSW, fmtSW, _applySW, _applySW, (double) B_SWITCH_MODE },
	{ CFG_B, HS, fdgHS, fdgHS, _appdgHS, _appdgHS, (double) B_HOMING_SEARCH_VELOCITY },
	{ CFG_B, HL, fdgHL, fdgHL, _appdgHL, _appdgHL, (double) B_HOMING_LATCH_VELOCITY },
	{ CFG_B, HZ, fdgHZ, fdgHZ, _appdgHZ, _appdgHZ, (double) B_HOMING_ZERO_OFFSET },
	{ CFG_B, HW, fdgHW, fdgHW, _appdgHW, _appdgHW, (double) B_HOMING_WORK_OFFSET },
	...
	{ CFG_C, JM, fdgJM, fdgJM, _appdgJM, _appdgJM, (double) C_JERK },
	{ CFG_C, CD, fmmCD, finCD, _appmmCD, _appinCD, (double) C_CORNER_DELTA },
	{ CFG_C, SW, fmtSW, fmtSW, _applySW, _applySW, (double) C_SWITCH_MODE },
	{ CFG_C, RA, fmmRA, finRA, _appmmRA, _appinRA, (double) C_RADIUS },
	{ CFG_C, HS, fdgHS, fdgHS, _appdgHS, _appdgHS, (double) C_HOMING_SEARCH_VELOCITY },
	{ CFG_C, HL, fdgHL, fdgHL, _appdgHL, _appdgHL, (double) C_HOMING_LATCH_VELOCITY },
	{ CFG_C, HZ, fdgHZ, fdgHZ, _appdgHZ, _appdgHZ, (double) C_HOMING_ZERO_OFFSET },
	{ CFG_C, HW, fdgHW, fdgHW, _appdgHW, _appdgHW, (double) C_HOMING_WORK_OFFSET },
	...
	{ CFG_GENERAL, JA, fmmJA, finJA, _appmmJA, _appinJA, (double) CORNER_ACCELERATION },
	{ CFG_GENERAL, ML, fmmML, finML, _appmmML, _appinML, (double) MIN_LINE_LENGTH },
	{ CFG_GENERAL, MC, fmmMC, finMC, _appmmMC, _appinMC, (double) MM_PER_ARC_SEGMENT },
	{ CFG_GENERAL, MT, fmtMT, fmtMT, _applyMT, _applyMT, (double) ESTD_SEGMENT_USEC },
	...
	{ CFG_GENERAL, SE, fmtSE, fmtSE, _applySE, _applySE, (double) STATUS_REPORT_ENABLE },
	{ CFG_GENERAL, SI, fmtSI, fmtSI, _applySI, _applySI, (double) STATUS_REPORT_INTERVAL },

	// ending version record - must be last
	{ CFG_GENERAL, _P, fmtP_, fmtP_, _applyCP, _applyCP, (double) CFG_VERSION }	// don't mess
};
...
static void _applyCP(struct cfgSetting *s) { cfg.cfg_profile = s->value;}
static void _applyCV(struct cfgSetting *s) { cfg.cfg_version = s->value;}

// motor settings - unit agnostic
static void _applyMA(struct cfgSetting *s) { cfg.m[MIDX(s->grp)].motor_map = (uint8_t)s->value; }
static void _applySA(struct cfgSetting *s) { cfg.m[MIDX(s->grp)].step_angle = s->value;
											_cfg_set_steps_per_unit(s); }

static void _appmmTR(struct cfgSetting *s) { cfg.m[MIDX(s->grp)].travel_rev = s->value;
											_cfg_set_steps_per_unit(s); }
static void _appinTR(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmTR(s); }

static void _applyMI(struct cfgSetting *s) { cfg.m[MIDX(s->grp)].microsteps = (uint8_t)s->value;
											st_set_microsteps(MIDX(s->grp), cfg.m[MIDX(s->grp)].microsteps);
											_cfg_set_steps_per_unit(s); }

static void _applyPO(struct cfgSetting *s) { cfg.m[MIDX(s->grp)].polarity = (uint8_t)s->value;
											st_set_polarity((MIDX(s->grp)), 
											cfg.m[MIDX(s->grp)].polarity); }

static void _applyPW(struct cfgSetting *s) { cfg.m[MIDX(s->grp)].power_mode = (uint8_t)s->value;}

// axis settings - unit agnostic
static void _applyMO(struct cfgSetting *s) { cfg.a[s->grp].axis_mode = (uint8_t)s->value;}
static void _applySW(struct cfgSetting *s) { cfg.a[s->grp].switch_mode = (uint8_t)s->value;}
...
static void _appmmJM(struct cfgSetting *s) { cfg.a[s->grp].jerk = s->value; }
static void _appmmCD(struct cfgSetting *s) { cfg.a[s->grp].corner_delta = s->value; }
static void _appmmRA(struct cfgSetting *s) { cfg.a[s->grp].radius = s->value; }
static void _appmmHS(struct cfgSetting *s) { cfg.a[s->grp].homing_search_velocity = s->value; }
static void _appmmHL(struct cfgSetting *s) { cfg.a[s->grp].homing_latch_velocity = s->value; }
static void _appmmHZ(struct cfgSetting *s) { cfg.a[s->grp].homing_zero_offset = s->value; }
static void _appmmHW(struct cfgSetting *s) { cfg.a[s->grp].homing_work_offset = s->value; }

// axis settings - inches units
static void _appinSR(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmSR(s); }
static void _appinFR(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmFR(s); }
static void _appinTH(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmTH(s); }
static void _appinTS(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmTS(s); }
static void _appinJM(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmJM(s); }
static void _appinCD(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmCD(s); }
static void _appinRA(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmRA(s); }
static void _appinHS(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmHS(s); }
static void _appinHL(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmHL(s); }
static void _appinHZ(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmHZ(s); }
static void _appinHW(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmHW(s); }

// general settings
static void _appmmML(struct cfgSetting *s) { cfg.min_segment_len = s->value; }
static void _appinML(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmML(s); }
static void _appmmMC(struct cfgSetting *s) { cfg.arc_segment_len = s->value; }
static void _appinMC(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmMC(s); }
static void _appmmJA(struct cfgSetting *s) { cfg.jerk_corner_acceleration = s->value; }
static void _appinJA(struct cfgSetting *s) { s->value *= MM_PER_INCH; _appmmJA(s); }

// common function for gcode settings
static void _applyGC(struct cfgSetting *s) { 
	switch ((int)s->value*10) {	// *10 to pick up 61.1 and any other NN.N gcode words
...
static void _applyMT(struct cfgSetting *s) { cfg.estd_segment_usec = s->value; }
...
//----- OTHER GENERAL APPLY FUNCTIONS -----

static void _applySE(struct cfgSetting *s) { 
	cfg.status_report_enabled = (uint8_t)s->value;
	cm_init_status_report();
}

// SI is delivered in ms - need to convert it to segment timing 
static void _applySI(struct cfgSetting *s) 
{
	if (s->value < STATUS_REPORT_MIN_MS) s->value = STATUS_REPORT_MIN_MS;
	if (s->value > STATUS_REPORT_MAX_MS) s->value = STATUS_REPORT_MAX_MS;
	cfg.status_report_interval = (uint8_t)ceil(s->value / (ESTD_SEGMENT_USEC / 1000));
	cm_init_status_report();
}
...
uint8_t cfg_config_parser(char *block, uint8_t display, uint8_t persist)
{
	...
	// parse the block into its basic parts; cutout if no value or bad value found
	uint8_t status = _cfg_parse_config_block(block, &cs.s);
	if (status != TG_OK) {
		if (status == TG_PARAMETER_NOT_FOUND) {
			s = _cfg_get_setting_by_key(cs.s.grp, cs.s.mnem);
			_cfg_print_setting(s);	// display setting if no value provided
		}
		return (status);
	}
 ...
 *		! % , ; ; @ 			punctuation chars removed
 *		^ ~ " ' <DEL>			punctuation chars removed
 *		$						remove leading $
 */
...
static uint8_t _cfg_normalize_config_block(char *block) 
{
	...
	while ((c = toupper(block[i++])) != 0) {// NUL character
		...
		if (strchr("-.?", c)) {				// catch valid non-alphanum's
		 	block[j++] = c; 
			continue;
		}
...
static uint8_t _cfg_parse_config_block(char *block, struct cfgSetting *s) 
{
	...
	// capture value
	if (block[i] == NUL) {
		return (TG_PARAMETER_NOT_FOUND);	// line has no value
	}
	if (read_double(block, &i, &s->value) == FALSE) {
 ...
 *	and the non-volatile memory records (persistence). NVM records are 
 *	binary versions of the config mnemonics and values that are re-hydrated 
 *	on extraction so they can be re-parsed to populate the config table. 
 *	NVM records look like this:
 *
 *		<grp_byte><mnemonic_byte><value_as_a_double>	(6 bytes total)
 *
...
static int16_t _cfg_get_setting_index(const int8_t grp, const int8_t mnem)
{
	...
	DEBUG2(dbCONFIG_DEBUG_ENABLED, PSTR("_cfg_get_setting_index() not found for %d, mnemonic %d"), grp, mnem);
...
static struct cfgSetting *_cfg_get_setting_by_index(const uint16_t index)
{
	if (index >= COUNT_SETTINGS) {		// range check the index
		DEBUG1(dbCONFIG_DEBUG_ENABLED, PSTR("_cfg_get_setting_by_index failed for index %d"), index)
		cs.status = TG_PARAMETER_NOT_FOUND;
		return(NULL);
	}
...
static struct cfgSetting *_cfg_get_setting_by_key(const int8_t grp, const int8_t mnem)
{
	...
	if ((i = _cfg_get_setting_index(grp, mnem)) == -1) {
		DEBUG2(dbCONFIG_DEBUG_ENABLED, PSTR("_cfg_get_setting_by_key() failed for %d, mnemonic %d"), grp, mnem)
		cs.status = TG_PARAMETER_NOT_FOUND;
		return (NULL);
	}
	return(&cfgList[i]);
...
static struct cfgSetting *_cfg_put_setting(const struct cfgSetting *s)
{
	...
	if ((t = _cfg_get_setting_by_key(s->grp, s->mnem)) == NULL) {
		DEBUG2(dbCONFIG_DEBUG_ENABLED, PSTR("_cfg_put_setting() failed for %d, mnemonic %d"), s->grp, s->mnem)
		cs.status = TG_PARAMETER_NOT_FOUND;
		return(NULL);
	}
...
static struct cfgSetting *_cfg_get_NVM_setting_by_key(const int8_t grp, const int8_t mnem)
{
	...
	if ((index = _cfg_get_setting_index(grp, mnem)) == -1) {
		DEBUG2(dbSHOW_CONFIG_DEBUG, PSTR("_cfg_get_NVM_setting_by_key() failed for %d, mnemonic %d"), grp, mnem)
		cs.status = TG_PARAMETER_NOT_FOUND;
		return (NULL);
	}
...
static uint8_t _cfg_put_NVM_setting(const struct cfgSetting *s)
{
	...
	if ((i = _cfg_get_setting_index(s->grp, s->mnem)) == -1) {
		DEBUG2(dbCONFIG_DEBUG_ENABLED, PSTR("_cfg_put_NVM_setting() failed for axis %d, mnemonic %d"), s->grp, s->mnem)
		cs.status = TG_PARAMETER_NOT_FOUND;
		return (cs.status);
	}
...
static void _cfg_print_setting(const struct cfgSetting *s)
{
	...
	uint8_t inches_mode = cm_get_inches_mode();
	...
	if (inches_mode == FALSE) {
		...
		if (UNIT_CONVERSION_REQUIRED) value = s->value * INCH_PER_MM;
		if (s->grp == CFG_GENERAL) {
 ...
 */
#ifdef __DEBUG
static void _dump_NVM_record(const int16_t record_number, const int8_t *nvm_record);
