...
uint8_t gc_gcode_parser(char *block)
{
	...
	if (block[0] == '$') { 				// config commands
		return(cfg_config_parser(block, TRUE, FALSE));
	}
	//++++++++ debug characters for feedhold and cycle start +++++++++++++
//	if (block[0] == '@') { return(cm_feedhold());}
//	if (block[0] == '#') { return(cm_cycle_start());}
...
static uint8_t _gc_parse_gcode_block(char *buf) 
{
	...
	while(_gc_next_statement(&gp.letter, &gp.value, &gp.fraction, buf, &i) == TRUE) {
		...
		//	case 'W': SET_NEXT_STATE(target[W], gp.value);	// reserved
			...
			case 'N': cm.linenum = gp.value; break;			// save line #
			default: gp.status = TG_UNSUPPORTED_STATEMENT;
		}
		// Process supported fractional gcodes
		if ((gp.letter == 'G') && ((uint8_t)gp.value == 61) && ((uint8_t)gp.fraction != 0)) {	// 61.1
			SET_NEXT_STATE(path_control_mode, PATH_EXACT_STOP);
		}
		if ((gp.letter == 'G') && ((uint8_t)gp.value == 92) && ((uint8_t)gp.fraction != 0)) {	// 92.1
...
static uint8_t _gc_execute_gcode_block() 
{
	...
	if (gn.next_action == NEXT_ACTION_RETURN_TO_HOME) {
		if ((gp.status = cm_return_to_home()) != TG_OK) {
			return (gp.status);	// error return
		}
	}
	...
	if (gn.next_action == NEXT_ACTION_HOMING_CYCLE) {
		if ((gp.status = cm_homing_cycle()) != TG_OK) {
			return (gp.status);	// error return
		}
	}
