...
static uint8_t _exec_aline_segment(void);
...
static uint8_t _exec_aline_head()
{
	...
	if (mr.section_state == MOVE_STATE_RUN1) {
		...
		if (_exec_aline_segment() == TG_COMPLETE) { 	  		// set up for second half
	...
	if (mr.section_state == MOVE_STATE_RUN2) {
		...
		if (_exec_aline_segment() == TG_COMPLETE) {
...
static uint8_t _exec_aline_tail() 
{
	...
	if (mr.section_state == MOVE_STATE_RUN2) {
		mr.segment_velocity = mr.midpoint_velocity - 
							 ...
							 (square(mr.elapsed_accel_time) * mr.jerk_div2);
		if (_exec_aline_segment() == TG_COMPLETE) { return (TG_OK);}	// end the move
	}
	return(TG_EAGAIN);
}
 ...
 */
static uint8_t _exec_aline_segment()
{
	uint8_t i;
	double travel[AXES];
	double steps[MOTORS];
...
/*
 * _finalize_aline_segment() - last segment runner helper
 *
 ...
 */
/*
static void _finalize_aline_segment()
{
	uint8_t i;
	double travel[AXES];
	double steps[MOTORS];
