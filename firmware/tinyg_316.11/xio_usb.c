...
*/
	...
	if ((IGNORECR(USB.flags) == TRUE) && (c == CR)) {
		return;
	}
	if ((IGNORELF(USB.flags) == TRUE) && (c == LF)) {
		return;
	}
	...
	if (USBu.rx_buf_head != USBu.rx_buf_tail) {
		USBu.rx_buf[USBu.rx_buf_head] = c;	// write char unless full
		if ((EN_XOFF(USB.flags) == TRUE) && (xio_get_rx_bufcount_usart(&USBu) > XOFF_HI_WATER_MARK)) {
