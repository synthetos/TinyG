...
struct GCodeModel {						// Gcode model- meaning depends on context
	...
	uint8_t mist_coolant;				// M7
	uint8_t flood_coolant;				// M8
	uint8_t	feed_override_enable;		// TRUE = overrides enabled (M48), F=(M49)
	uint8_t spindle_mode;				// M3-CW, M4=CCW, M5=off
...
enum cmMotionMode {					// G Modal Group 1
	MOTION_MODE_CANCEL_MOTION_MODE=0,// G80
	MOTION_MODE_STRAIGHT_TRAVERSE,	// G0 - traverse
...
uint8_t cm_cancel_motion_mode(void);				// G80
