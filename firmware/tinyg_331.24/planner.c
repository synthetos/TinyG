...
struct mpMoveRuntimeSingleton {	// persistent runtime variables
	...
	double position[AXES];		// segment (previous) position
	double target[AXES];		// segment target position
	double unit[AXES];			// unit vector for axis scaling & planning
	double bf_target[AXES];		// final target for bf (used to correct rounding errors)
...
static void _plan_block_list(struct mpBuffer *bf);
static void _calculate_trapezoid(struct mpBuffer *bf);
static void _calculate_trapezoid_finalize(struct mpBuffer *bf);
...
static uint8_t _exec_dwell(struct mpBuffer *bf);
static uint8_t _exec_stop(struct mpBuffer *bf);
static uint8_t _exec_end(struct mpBuffer *bf);
...
uint8_t mp_exec_move() 
{
	...
	if ((bf = _get_run_buffer()) == NULL) { return (TG_NOOP);}
	...
	switch (bf->move_type) {
		...
		case MOVE_TYPE_STOP: { return (_exec_stop(bf));}
		case MOVE_TYPE_END: { return (_exec_end(bf));}
	}
...
/**** STOP & END *********************************************************
 * mp_queue_program_stop()	- entry point
 * mp_queue_program_end()	- entry point
 * _exec_stop()				- execute command from command dispatcher
 * _exec_end()				- execute command from command dispatcher
 */
void mp_queue_program_stop() 
{
	if (_get_write_buffer() == NULL) {
		return;
	}
	_queue_write_buffer(MOVE_TYPE_STOP);
}
static uint8_t _exec_stop(struct mpBuffer *bf)
{
	_free_run_buffer();
...
void mp_queue_program_end()
{
	...
	_queue_write_buffer(MOVE_TYPE_END);
}
static uint8_t _exec_end(struct mpBuffer *bf)
{
	_free_run_buffer();
 ...
 *		optimizations.
 */
void _plan_block_list(struct mpBuffer *bf)
{
	struct mpBuffer *bp = bf;
...
static void _calculate_trapezoid(struct mpBuffer *bf) 
{
	...
	if (bf->length < (minimum_length - PLANNER_LENGTH_TOLERANCE)) {
		...
		bf->body_length = 0;
//		INFO4(PSTR("Degraded line in _calculate_trapezoid() %f  %f  %f  %f"), bf->length, bf->entry_velocity, bf->cruise_velocity, bf->exit_velocity);
		return;
	}
...
//			INFO(PSTR("_calculate_trapezoid() failed to converge")); 
//		}
...
/* 
 * mp_end_hold_callback() - end a feedhold
 *
 ...
 *	parts of the S curve in the "head"). Periods 4 and 5 are the first 
 *	and second halves of the deceleration ramp (the tail). Period 3 is the
 *	period for the constant-velocity plateau of the trapezoid (the body).
 *
 ...
 *
 *	While the math to drive a move to the exact end-point is correct (AFAIK), 
 *	rounding errors do creep in. What AVRGCC calls doubles are actually what 
 *	other compilers call floats, and AVR only uses 32 bit FP arithmetic. Floating 
 *	point errors can accumulate at roughly +/-2 PPM per centimeter. To keep this 
 *	from reaching the thousandths digit (inches or mm) for long or complicated 
 *	moves the last interpolation segment executed for each planner buffer move 
 *	is set to the exact end position of the planner move.
 */
...
static uint8_t _exec_aline(struct mpBuffer *bf)
{
	...
	if (mr.move_state == MOVE_STATE_OFF) {
		...
		copy_axis_vector(mr.bf_target, bf->target);	// save the final target of the move
	}
	...
	if ((cm.hold_state == FEEDHOLD_DECEL) && (status == TG_OK) && (bf->hold_point == true)) {
		...
		rpt_force_status_report();				// send a hold status report
	}
...
static uint8_t _exec_aline_tail() 
{
	...
	if (mr.section_state == MOVE_STATE_RUN2) {
		mr.segment_velocity = mr.midpoint_velocity - 
							 ...
							 (square(mr.elapsed_accel_time) * mr.jerk_div2);
		if (_exec_aline_segment(true) == TG_COMPLETE) { 	// end the move with error correction
			return (TG_OK);
		}
	}
	return(TG_EAGAIN);
...
//	uint8_t i;
	...
	for (uint8_t i=0; i < AXES; i++) {
		if ((correction_flag == true) && (mr.segment_count == 1)) {
			mr.target[i] = mr.bf_target[i];	// rounding error correction for last segment
		} else {
...
void _test_calculate_trapezoid(void);
void _test_trapezoid(double entry_velocity, double cruise_velocity, double exit_velocity, double length, struct mpBuffer *bf);
