...
//		if ((flag[i] < EPSILON) || (cfg.a[i].axis_mode == AXIS_DISABLED)) {
		if (fp_FALSE(flag[i]) || (cfg.a[i].axis_mode == AXIS_DISABLED)) {
			...
			((fp_TRUE(flag[X])) || (fp_TRUE(flag[Y])) || (fp_TRUE(flag[Z])))) {
...
uint8_t cm_set_origin_offsets(double offset[], double flag[])	// G92
{
	...
	for (uint8_t i=0; i<AXES; i++) {
		if (fp_TRUE(flag[i])) {	 	// behaves according to NIST 3.5.18
			gm.origin_offset[i] = 
				gm.position[i] - cfg.offset[gm.coord_system][i] - _to_millimeters(offset[i]);
		}
	}
	return (TG_OK);
}
 ...
 *
 * cm_straight_traverse() - G0 linear traverse
 */
...
uint8_t cm_spindle_control(uint8_t spindle_mode)
{
	...
	if (spindle_mode == SPINDLE_CCW) {
		return (cm_start_spindle_counterclockwise());
	}
	return(cm_stop_spindle_turning());	// failsafe: any error causes stop
}
...
void _exec_finalize()
{
	// various functions to run on stop or end
	mp_zero_segment_velocity();		// for reporting purposes
	rpt_force_status_report();		// send final status report (if enabled)
...
void cm_exec_stop() 
{
	...
	_exec_finalize();				// finalization routines
}
