 ...
 */
/* ------
 * 	This module buffers line segments generated by the line planner, and 
 *	also some other commands such as dwells, and stops. It manages the 
 *	buffers that are consumed by the stepper routines. It also does all
 *	the pre-computation for the fractional DDA parameters.
 */
...
#include "system.h"
#include "xio.h"					// supports INFO and print statements
#include "tinyg.h"
#include "util.h"
...
#define MQ_BUFFER_SIZE 3 			// limited to 255 - but try to keep as small as possible
...
/***** move execution and prep routines *****/
...
#ifdef __DISABLE_QUEUE_LINE		// disabled for faster simulation
	return (TG_OK);				// always returns successfully.
#endif

	// trap conditions that would prevent queueing the line
	if (isfinite(microseconds) == FALSE) {
		INFO(PSTR("Trapped infinite time move in mp_queue_line()"));
//		dump_everything();		// requires __DEBUG to be defined
//		roll_over_and_die();	// requires __DEBUG to be defined
		return (TG_ZERO_LENGTH_MOVE);
	}
	if (microseconds < EPSILON) {
		INFO(PSTR("Trapped zero time move in mp_queue_line()"));
		return (TG_ZERO_LENGTH_MOVE);
	}
	// trap zero step moves - and get the major axis while you're at it
 	// Also, test the zero-length case: vector > 1 but all axes are < 1
	for (i=0; i<MOTORS; i++) {
		length_sqr += square(steps[i]);
		if (major_axis_steps < fabs(steps[i])) { 
			major_axis_steps = fabs(steps[i]); 
		}
	}
//	if ((sqrt(length_sqr) < 1) || (major_axis_steps < 1)) {
//	//	INFO(PSTR("Trapped zero length move in mp_queue_line()"));
//		return (TG_ZERO_LENGTH_MOVE);
//	}
	// get the buffer last - so you don't have to free in in case of error
	if ((mq.p = mq_queue_motor_buffer()) == NULL) {	// shouldn't ever fail
		INFO(PSTR("Fatal buffer full error in mp_queue_line()"));
