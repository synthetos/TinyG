		...
		switch(letter) {
			case 'G':
				switch((uint8_t)value) {
					...
					case 4:  SET_NON_MODAL (next_action, NEXT_ACTION_DWELL);  // treated as non-modal
					...
					case 61: {
						switch (_point(value)) {
							case 0: SET_MODAL (MODAL_GROUP_G12, path_control, PATH_EXACT_PATH);
							case 1: SET_MODAL (MODAL_GROUP_G12, path_control, PATH_EXACT_STOP); 
							default: status = TG_UNRECOGNIZED_COMMAND;
						}
						break;
					}
					case 64: SET_MODAL (MODAL_GROUP_G12,path_control, PATH_CONTINUOUS);
			...
			case 'M':
				switch((uint8_t)value) {
					case 0: case 1: 
							SET_MODAL (MODAL_GROUP_M4, program_flow, PROGRAM_FLOW_PAUSED);
					case 2: case 30: case 60:
							SET_MODAL (MODAL_GROUP_M4, program_flow, PROGRAM_FLOW_COMPLETED);
					...
					case 6: SET_MODAL (MODAL_GROUP_M6, change_tool, true);
					case 7: SET_MODAL (MODAL_GROUP_M8, mist_coolant, COOLANT_ON);
					case 8: SET_MODAL (MODAL_GROUP_M8, flood_coolant, COOLANT_ON);
					case 9: SET_MODAL (MODAL_GROUP_M8, flood_coolant, COOLANT_OFF);
	...
	switch (gn.next_action) {
		...
		case NEXT_ACTION_DEFAULT: { 
			...
			switch (gn.motion_mode) {
				case MOTION_MODE_CANCEL_MOTION_MODE: { status = cm_cancel_motion_mode(); break;}
...
static uint8_t _gcode_error(uint8_t status)
{
	cm_cancel_motion_mode();
	return(status);
}
...
static uint8_t _check_gcode_block()
{
	...
	if ((gp.modals[MODAL_GROUP_G0] == true) && (gp.modals[MODAL_GROUP_G1] == true)) {
		return(_gcode_error(TG_MODAL_GROUP_VIOLATION));
	}
	
	// look for commands that require an axis word to be present
	if ((gp.modals[MODAL_GROUP_G0] == true) || (gp.modals[MODAL_GROUP_G1] == true)) {
		if (_axis_changed() == false)
		return(_gcode_error(TG_GCODE_AXIS_WORD_MISSING));
	}
	return (TG_OK);
}
