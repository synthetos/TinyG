...
static void _mp_get_unit_vector(double unit[], double target[], double position[])
{
	double length = mp_get_vector_length(target, position);
	for (uint8_t i=0; i < AXES; i++) {
...
uint8_t mp_line(const double target[], const double minutes)
{
	...
	bf->time = minutes;
	mp_copy_vector(bf->target, target);			// target to bf_target
	bf->length = mp_get_vector_length(target, mr.position);
...
static uint8_t _mp_run_line(struct mpBuffer *bf) 
{
	...
	for (i=0; i < AXES; i++) {
		steps[i] = _steps(i, bf->target[i]) - _steps(i, mr.position[i]);
	}
 ...
 *
 * Note: Most other routines pass axes as vectors. This doesn't work too 
 *	for ihis arc function as the order may be scrambled due to the active 
 *	plane.
 */
				...
				const double minutes)		// time to complete the move
{
	...
	mp_copy_vector(bf->target, target);
	bf->time = minutes;
	bf->a.theta = theta;
	bf->a.radius = radius;
	bf->a.axis_1 = axis_1;
...
uint8_t mp_aline(const double target[], const double minutes)
{
	...
	mp_copy_vector(mm.target, target);			// set mm.target
	m->length = mp_get_vector_length(mm.target, mm.position);
	...
	mm.linear_jerk_div2 = cfg.linear_jerk_max/2;// init linear jerk terms
	mm.linear_jerk_rad3 = pow(cfg.linear_jerk_max, 0.33333333333334);
	...
	if (m->path_mode == PATH_CONTINUOUS) {
		if (m->angular_jerk > cfg.corner_jerk_lower) {
	...
	if (m->path_mode == PATH_EXACT_PATH) {
		if (m->angular_jerk > cfg.corner_jerk_upper) {
			...
			m->initial_velocity_req *= (1 - (m->angular_jerk - cfg.corner_jerk_lower));
		}
	}
	// do the actual work
...
//	return (pow(cfg.linear_jerk_max, 0.33333334) * pow(L, 0.666666667) + V);
}
...
static uint8_t _mp_run_cruise(struct mpBuffer *bf)
{
	...
	bf->replannable = FALSE;						// stop replanning
	if (bf->length < MIN_LINE_LENGTH) {			// toss the line
		return (TG_OK);
	}
...
static void _mp_aline_run_finalize(struct mpBuffer *bf)
{
	...
	mr.length = mp_get_vector_length(mr.target, mr.position);
	if (mr.length < MIN_LINE_LENGTH) {		// trap zero-length case
		return;
	}
	mr.time = mr.length / bf->end_velocity; // ++++ 1#INF problem here (get time from length
