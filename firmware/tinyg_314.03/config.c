...
struct cfgSetting cfgList[COUNT_SETTINGS] = { 
	...
	{ X, RA, fmmRA, finRA, _appmmRA, _appinRA, (double) 0 },
	...
	{ Y, RA, fmmRA, finRA, _appmmRA, _appinRA, (double) 0 },
	...
	{ Z, RA, fmmRA, finRA, _appmmRA, _appinRA, (double) 0 },
	...
	{ A, RA, fmmRA, finRA, _appmmRA, _appinRA, (double) A_RADIUS },
	...
	{ B, RA, fmmRA, finRA, _appmmRA, _appinRA, (double) B_RADIUS },
	...
	{ C, RA, fmmRA, finRA, _appmmRA, _appinRA, (double) C_RADIUS },
...
static void _appmmRA(struct cfgSetting *s) { cfg.a[s->axis].radius = s->value; }
...
void cfg_init_gcode_model()
{
	...
	s = _cfg_get_setting_by_key(NON_AXIS, GU); s->app_mm(s); // unit mode
	s = _cfg_get_setting_by_key(NON_AXIS, GL); s->app_mm(s); // plane select
	s = _cfg_get_setting_by_key(NON_AXIS, GP); s->app_mm(s); // path control
	s = _cfg_get_setting_by_key(NON_AXIS, GD); s->app_mm(s); // distance mode
}
...
static void _cfg_print_settings(const char c)
{
	...
	switch(c) {
		...
		case 'A': case 'B': case 'C': {
			...
			_cfg_print_axis_advisories(axis, axis_char);
			_cfg_print_rate_advisories(axis, axis_char);
			return;
		}
		case 'H': { 						// print help
...
static void _cfg_print_setting(const struct cfgSetting *s)
{
	...
	char axis_char = cfg_get_axis_char(s->axis);
	char *mnem = _cfg_get_mnemonic_string(s->mnemonic);
	...
	if (cm_get_inches_mode() == FALSE) {// mm mode displays
		if (s->axis != NON_AXIS) {
			fprintf_P(stderr, PSTR("%c axis - "), axis_char); 
			fprintf_P(stderr, (PGM_P)s->fmt_mm, value, axis_char, mnem, value);
		} else {
		...
		if (s->axis != NON_AXIS) {
			fprintf_P(stderr, PSTR("%c axis - "), axis_char);
			fprintf_P(stderr, (PGM_P)s->fmt_in, value, axis_char, mnem, value);
		} else {
			fprintf_P(stderr, (PGM_P)s->fmt_in, value, mnem, value);
		}
	}
	switch (s->mnemonic) {
		case MO: { _cfg_print_axis_advisories(s->axis, axis_char); break; }
		case SA:
		case TR:
		case SR:
		case FR: { _cfg_print_rate_advisories(s->axis, axis_char); break; }
	}
}

/*
 * _cfg_print_rate_advisories() - print step rates resulting from the settings
 */
static void _cfg_print_rate_advisories(const int8_t axis, const char axis_char)
{
