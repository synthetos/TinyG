...
#define RX_BUFFER_SIZE 128		// USART ISR RX buffer size (255 max)
#define TX_BUFFER_SIZE 128		// USART ISR TX buffer size (255 max)
...
//	uint16_t uflags;			  // usart sub-system flags (UNUSED)
	uint8_t rx_buf_tail;		  // RX buffer read index  (not VOLATILE)
	...
	uint8_t tx_buf_head;		  // TX buffer write index (not VOLATILE)
...
// down here by its lonesome becuase it needs xioUSART defined
