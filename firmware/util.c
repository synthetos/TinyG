...
/* 
 * calculate_intelhexcc() - calculate 32-bit hash code for a string
 * 
 * This is the Java hashCode function. See http://en.wikipedia.org/wiki/Java_hashCode()
 */
...
{	
    ...
    return 0x100 - h;		// take two's complement
}

/* 
 * calculate_crc16() - calculate 16-bit CRC code for a string
 * 
 * Adapted from: http://www8.cs.umu.se/~isak/snippets/crc-16.c
 *
 *                                      16   12   5
 * Notes: This is the CCITT CRC 16 polynomial X  + X  + X  + 1.
 * This works out to be 0x1021, but the way the algorithm works lets us use 
 * 0x8408 (the reverse of the bit pattern). The high bit is always assumed 
 * to be set, thus we only use 16 bits to represent the 17 bit value.
 */
#define POLY 0x8408
...
uint16_t calculate_crc16(char *string)
{
	...
	do {
		for (i=0, data=(uint16_t)0xff & *string++; i < 8; i++, data >>= 1) {
