 ...
 */
#define CFG_PROFILE ((double)1.11) // NVM profile (use a recognizable number)
#define CFG_VERSION ((double)2.22) // NVM version
...
//static void _cfg_load_NVM_from_RAM(struct cfgSetting *s);
...
static void _applyP_(struct cfgSetting *s);	// profile header (& trailer)
static void _applyV_(struct cfgSetting *s);	// version record
...
char fmtP_[] PROGMEM = "Profile Number     %3.2f   $%s%1.2f\n";		// print profile number
char fmtV_[] PROGMEM = "Version Number     %3.2f   $%s%1.2f\n";		// print version number
...
/*---- APPLY FUNCTIONS -------------------------------------------------*/
// Note: standard format conventions abandoned for density & readability
static void _applyP_(struct _S *s) { return; }	// profile header & trailer
static void _applyV_(struct _S *s) { return; }	// version number
...
/*
 * _cfg_load_NVM_from_RAM - load content of RAM to NVM
 */
 /*
static void _cfg_load_NVM_from_RAM(struct cfgSetting *s)
{
	...
	fprintf_P(stderr, PSTR("Writing RAM to EEPROM"));
	for (i=0; i<COUNT_SETTINGS; i++) {
...
//		TRAP2(PSTR("cfg_config_parser() failed for axis %d, mnemonic %d"), s->axis, s->mnemonic)
		return (cs.status);
	}
...
//		DEBUG1(dbCONFIG, PSTR("_cfg_get_NVM_setting_by_index() failed for index %d"), index);
//		DEBUG3(dbCONFIG, PSTR("  axis %d, mnemonic %d, value %f"), cs.s.axis, cs.s.mnemonic, cs.s.value);
...
//	DEBUG3(dbCONFIG, PSTR("Writing to EEPROM: %d %d %f"), nvm_record[0], nvm_record[1], s->value);
