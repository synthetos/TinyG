					...
					const uint8_t outset) 
{
	...
	dx->tx_buf_head = 1;
	dx->tx_buf_tail = 1;
	dx->rx_buf_count = 0;			// should be redundant, but be sure
...
void xio_xoff_usart(const uint8_t dev)
{
	...
	if ((EN_XOFF(d->flags) != 0) && (IN_XOFF(d->flags) == 0)) {
...
#ifdef __RTSMODE
/* 
 * xio_deassert_rts_usart() - Disable RTS (enter flow control)
 * xio_assert_rts_usart() - Enable RTS  (leave flow control)
 */
...
void xio_deassert_rts_usart(const uint8_t dev)
{
	...
	if (EN_XOFF(d->flags) != 0) {
		dx->port->OUTCLR = USB_RTS_bp;
	}
}
...
int xio_putc_usart(const uint8_t dev, const char c, FILE *stream)
{
	...
	while(dx->next_tx_buf_head == dx->tx_buf_tail) { // sleep or return
		if (BLOCKING(d->flags) != 0) {
			...
			en_bit_on(0x08);		// +++++++++++++++++ signal overflow
			return(_FDEV_ERR);
		}
	};
	...
	en_bit_off(0x08);			// +++++++++++++++++ clear overflow condition
	if ((dx->usart->STATUS & USART_DREIF_bm) != 0) {// 0x20
		...
		if (--(dx->tx_buf_tail) == 0) {		// advance tail & wrap if needed
			dx->tx_buf_tail = TX_BUFFER_SIZE-1;	// -1 avoid off-by-one (OBOE)
		}
...
int xio_getc_usart(const uint8_t dev, FILE *stream)
{
	...
	if (--(dx->rx_buf_count) < XOFF_LO_WATER_MARK) {
#ifdef __XONXOFF
		xio_xon_usart(XIO_DEV_USB);				// ++++++++++++++++++
#endif
#ifdef __RTSMODE
		xio_assert_rts_usart(XIO_DEV_USB);		// ++++++++++++++++++
#endif
		en_bit_off(0x01);						// no flow control ++++++++++++++++++
	}
