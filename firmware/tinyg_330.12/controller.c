...
static void _json_gcode_return(uint8_t status, char *in_buf, char *out_buf);
	...
	//	case 'G': case 'M': case 'N': case 'F': case 'Q': case '(': case '%': case '\\':
		default: {								// Gcode - is anything else
			if (tg.communications_mode == TG_JSON_MODE) {
				_json_gcode_return(gc_gcode_parser(tg.in_buf), tg.in_buf, tg.out_buf);
			} else {
...
//	xio_queue_RX_string_usb("{\"sr\":{\"xam\":true,\"yam\":true}}\n");	// set status report
