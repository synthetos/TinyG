...
#define _to_millimeters(a) ((gm.units_mode == INCHES_MODE) ? (a * MM_PER_INCH) : a)
...
uint8_t cm_get_next_action() { return gm.next_action; }
 ...
 * cm_get_model_position() - return position from the gm struct into gn struct form (external form)
 * cm_get_model_position_vector() - return model position vector in externalized form
 * cm_get_model_position_canonical() - return model position vector in internal canonical form
 * cm_get_machine_runtime position() - return current machine position in external form 
 * cm_get_work_runtime position() - return current work coordinate position in external form 
 */

double cm_get_coord_offset(uint8_t axis)
{
	if (gm.origin_offset_mode == true) {
		return (cfg.offset[cfg.coord_system][axis] - gm.origin_offset[axis]);
	} else {
		return (cfg.offset[cfg.coord_system][axis]);
	}
}

double cm_get_model_position(uint8_t axis) 
{
...
double *cm_get_model_position_vector(double position[]) 
{
	for (uint8_t i=0; i<AXES; i++) {
		position[i] = cm_get_model_position(i);
	}
	return (position);
}

double *cm_get_model_position_canonical(double position[])
{
	copy_axis_vector(position, gm.position);	
	return (position);
}

double cm_get_machine_runtime_position(uint8_t axis) 
{
	if (gm.units_mode == INCHES_MODE) {
...
double cm_get_work_runtime_position(uint8_t axis) 
{
	if (gm.units_mode == INCHES_MODE) {
...
uint8_t cm_set_origin_offsets(uint8_t origin_offset_mode, double offset[], double flag[])
{
	uint8_t i;

	switch (origin_offset_mode) {
		case ORIGIN_OFFSET_SET: {
			gm.origin_offset_mode = true;
			for (i=0; i<AXES; i++) {
				if (flag[i] > EPSILON) { 
					gm.origin_offset[i] = offset[i];
				}
			}
			break;
		}
		case ORIGIN_OFFSET_CANCEL: {
		...
		case ORIGIN_OFFSET_SUSPEND: {
			gm.origin_offset_mode = true; break;
		}
		case ORIGIN_OFFSET_RESUME: {
			gm.origin_offset_mode = false; break;
		}
	}
	return (TG_OK);
}
...
uint8_t _cm_compute_center_arc()
{
	...
	theta_start = _cm_get_theta(-gm.arc_offset[gm.plane_axis_0], -gm.arc_offset[gm.plane_axis_1]);
