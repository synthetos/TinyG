...
//	uint8_t status;					// unified TG_ status codes
...
#define HOMING_RATE 300				// F word for homing
#define BACKOFF_X 5					// mm to back off when min hit
#define BACKOFF_Y 5
#define BACKOFF_Z 5
...
uint8_t cm_set_origin_offsets(double x, double y, double z, double a)
{
	...
	mc_set_position(gm.position[X], gm.position[Y], 
					gm.position[Z], gm.position[A]);
	return (TG_OK);
}
...
//	double length		// length of linear move
	...
	if (gm.inverse_feed_rate_mode) {
		...
		linear_time = sqrt(square(gm.target[X] - gm.position[X]) +
						 ...
						 square(gm.target[A] - gm.position[A])) /
						 gm.feed_rate;
 ...
 *
 *	The continue is coded as a state machine. It is entered while the 
 *	lower layers (motion) are idle. It sequences through the various moves
 *	and reacts to limit switch interrupts. This one is tricky because
 *	the routine can be re-entered if a limit switch is hit (ANY limit switch)
 *	or if the previously queued move completes.
 *
 *	Operation:
 *	  - cm_return_to_home()
 *		- save the current gcode model state
 ...
 *	wrong switch was hit. This may need to be fixed.
 */
...
//	cm_set_origin_offsets(0, 0, 0, 0);			// zero the new model
	cm_use_length_units(MILLIMETER_MODE);
	cm_set_distance_mode(INCREMENTAL_MODE);
	cm_set_feed_rate(cfg.homing_rate);
	ls_clear_limit_switches();				// reset the switch flags
	cy.state = CY_STATE_HOMING_X_START;		// start on X

//#define HOMING_TRAVEL 10
//	CFG(X).travel_max = HOMING_TRAVEL;	// ++++ test only
//	CFG(Y).travel_max = HOMING_TRAVEL;	// ++++ test only
...
{
	// sanity check and sync to motors
	if (cy.state == CY_STATE_OFF) { return (TG_NOOP);}
	if (mc_isbusy()) { return (TG_EAGAIN);} // this is how you sync the queue

	// initial time in
//	if (cy.state == CY_STATE_NEW) {
//		memset(gm.position, 0, sizeof(gm.position));// G92 zero equiv
//		cy.state = CY_STATE_HOMING_X_START;
//	}

	// issue a max_travel X move and then wait for end or a limit switch
	if (cy.state == CY_STATE_HOMING_X_START) {
		cy.state = CY_STATE_HOMING_X_WAIT;
		return(cm_straight_feed(-(CFG(X).travel_max), 0, 0, 0));
	}
	if (cy.state == CY_STATE_HOMING_X_WAIT) {
//		if (ls.min[X] == FALSE) { return (TG_EAGAIN); }	// UNUSED
		ls_clear_limit_switches();
		gt.position[X] = CFG(X).travel_offset + BACKOFF_X;
		cy.state = CY_STATE_HOMING_Y_START;
		return(cm_straight_traverse(BACKOFF_X, 0, 0, 0));
	}

	// issue a max_travel Y move and then wait for end or a limit switch
	if (cy.state == CY_STATE_HOMING_Y_START) {
		cy.state = CY_STATE_HOMING_Y_WAIT;
		return(cm_straight_feed(0, -(CFG(Y).travel_max), 0, 0));
	}
	if (cy.state == CY_STATE_HOMING_Y_WAIT) {
//		if (ls.min[Y] == FALSE) { return (TG_EAGAIN); }	// UNUSED - FIX Y!!!
		ls_clear_limit_switches();
		gt.position[Y] = CFG(Y).travel_offset + BACKOFF_Y;
//		cy.state = CY_STATE_HOMING_Y_START;	// uncomment to activate Z 
		cy.state = CY_STATE_HOMING_FINAL;
		return(cm_straight_traverse(0, BACKOFF_Y, 0, 0));
	}

	// issue a max_travel Z move and then wait for end or a limit switch
	if (cy.state == CY_STATE_HOMING_Z_START) {
		cy.state = CY_STATE_HOMING_Z_WAIT;
		return(cm_straight_feed(0, 0, -(CFG(Z).travel_max), 0));
	}
	if (cy.state == CY_STATE_HOMING_Z_WAIT) {
//		if (ls.min[Z] == FALSE) { return (TG_EAGAIN); }	// UNUSED
		ls_clear_limit_switches();
		gt.position[Z] = CFG(Z).travel_offset + BACKOFF_Z;
		cy.state = CY_STATE_HOMING_FINAL;
		return(cm_straight_traverse(0, 0, BACKOFF_Z, 0));
	}
	if (cy.state == CY_STATE_HOMING_FINAL) {
		memcpy(&gm, &gt, sizeof(struct GCodeModel));// restore gcode model
		mc_set_position(gt.position[X], gt.position[Y], // MC must agree...
						gt.position[Z], gt.position[A]);//...with gt values
		cm_set_distance_mode(ABSOLUTE_MODE);
		cfg.homing_cycle_active = FALSE;			// not a homer anymore
		cy.state = CY_STATE_OFF;
		return(cm_straight_traverse(0, 0, 0, 0));
	}
	return (TG_OK);
}
...
//	double axis_time;
	...
	if (gm.inverse_feed_rate_mode) {
		...
		mm_of_travel = hypot((angular_travel * radius_tmp), labs(linear_travel));
...
#define GC_MSG_FEEDRATE 25
#define GC_MSG_LIMITSWITCHES 26
...
char gms23[] PROGMEM = "Offset K:     %8.3f %s\n";
char gms24[] PROGMEM = "Seek Rate:    %8.3f %s \\ min\n";
char gms25[] PROGMEM = "Feed Rate:    %8.3f %s \\ min\n";
char gms26[] PROGMEM = "Limit Switches: %3.0f %s\n";
...
PGM_P gcMsg[] PROGMEM = {	
	...
	gms20, gms21, gms22, gms23, gms24, gms25, gms26
};
...
void cm_print_machine_state()
{
	...
	if (gm.inches_mode) {
		...
		printf_P((PGM_P)pgm_read_word(&gcMsg[GC_MSG_I]), gm.offset[0] / (25.4), units);
		printf_P((PGM_P)pgm_read_word(&gcMsg[GC_MSG_J]), gm.offset[1] / (25.4), units);
		printf_P((PGM_P)pgm_read_word(&gcMsg[GC_MSG_K]), gm.offset[2] / (25.4), units);
		...
		printf_P((PGM_P)pgm_read_word(&gcMsg[GC_MSG_A]), gm.position[A],"degrees");
		printf_P((PGM_P)pgm_read_word(&gcMsg[GC_MSG_I]), gm.offset[0], units);
		printf_P((PGM_P)pgm_read_word(&gcMsg[GC_MSG_J]), gm.offset[1], units);
		printf_P((PGM_P)pgm_read_word(&gcMsg[GC_MSG_K]), gm.offset[2], units);
...
//	printf_P((PGM_P)pgm_read_word(&gcMsg[GC_MSG_LIMITSWITCHES]), ls.min[X]);
