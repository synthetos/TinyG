...
struct mpMoveRuntimeSingleton {	// persistent runtime variables
	...
	uint8_t move_state;			// move status
...
static uint8_t _mp_exec_accel_0(struct mpBuffer *bf);
static uint8_t _mp_exec_accel_1(struct mpBuffer *bf);
static uint8_t _mp_exec_accel_2(struct mpBuffer *bf);
static uint8_t _mp_exec_cruise(struct mpBuffer *bf);
static uint8_t _mp_exec_decel_0(struct mpBuffer *bf);
static uint8_t _mp_exec_decel_1(struct mpBuffer *bf);
static uint8_t _mp_exec_decel_2(struct mpBuffer *bf);
static uint8_t _mp_exec_segment(struct mpBuffer *bf);
static void _mp_finalize_segment(struct mpBuffer *bf);
...
/*
 * mp_set_plan_position() - sets planning peosition (for G92)
...
uint8_t mp_aline(const double target[], const double minutes)
{
	...
	// some statements useful for debugging
	if (tg.linenumber > EPSILON) { bf->linenum = tg.linenumber; }
	else { bf->linenum = tg.linecount; }
//	if (tg.linecount == 12) {
	if (tg.linenumber == 32) { 
		tg.nop += 1;
	}
 ...
 *
 *	This rather brute-force function sets segment lengths and velocities based 
 ...
 *	and accurate or apprixomate velocities. We care about accuracy on lengths, less so 
 *	on velocity (as long as velocity err's on the side of too slow).
 *
 *	Various cases handled;
 *	  ZERO	zero segments - the line is too short to plan
 *	  HBT	Ve<Vt>Vx	3 segment trapezoid
 ...
 *
 *	The HT cases (2 segment cases) deserve special attention There are 2 sub-cases: 
...
/*	Handle various edge cases where segments are less than min line lengths. 
...
/**** ALINE EXEC ROUTINES ************************************************/
/*************************************************************************
 ----> Everything here fires from MED interrupt and must be interrupt safe
 *
 *  _mp_exec_aline()		- entry point for runtime acceleration lines
 *	_mp_exec_accel_0()	- initialize acceleration phase
 *	_mp_exec_accel_1()	- run 1st half of acceleration
 *	_mp_exec_accel_2()	- run 2nd half of acceleration
 *	_mp_exec_cruise()	- run cruise phase
 *	_mp_exec_decel_0()	- initialize deceleration phase
 *	_mp_exec_decel_1()	- run 1st half of deceleration
 *	_mp_exec_decel_2()	- run 2nd half of deceleration
 *	_mp_exec_segment()	- helper for running a segment
 *	_mp_exec_finalize()	- helper for running last segment
 *
 ...
 *
 *	These routines are actually a single function broken out in to a state 
 *	machine. The first routine is the dispatcher called by the controller, 
 *	the rest handle line gneration states. The last 2 are helpers.
 *
 ...
 *	There are various degraded trapezoids possible, including 2 segment 
 *	combinations (head and tail; head and body; body and tail), and single 
 *	segments - any one of the three.
 *
...
static uint8_t _mp_exec_aline(struct mpBuffer *bf)
{
	uint8_t status = TG_NOOP;

	switch (mr.move_state = bf->move_state) {
		case MOVE_STATE_IDLE:	 { status = TG_NOOP; break; }
		case MOVE_STATE_NEW:	 { status = _mp_exec_accel_0(bf); break; }
		case MOVE_STATE_ACCEL_1: { status = _mp_exec_accel_1(bf); break; }
		case MOVE_STATE_ACCEL_2: { status = _mp_exec_accel_2(bf); break; }
		case MOVE_STATE_CRUISE:  { status = _mp_exec_cruise(bf);  break; }
		case MOVE_STATE_DECEL_0: { status = _mp_exec_decel_0(bf); break; }
		case MOVE_STATE_DECEL_1: { status = _mp_exec_decel_1(bf); break; }
		case MOVE_STATE_DECEL_2: { status = _mp_exec_decel_2(bf); break; }
	}
	// This test solves a potential race condition where the current move ends but
	// the new move has not started because the previous move is still being run by 
	// the steppers. Planning can overwrite the new move.
	if (status != TG_EAGAIN) {
		_mp_stop_replanning(_mp_get_next_buffer(bf));	// prevent overplanning
	}
	return (status);
}
...
static uint8_t _mp_exec_accel_0(struct mpBuffer *bf) 
{
	...
	if (bf->head_length < MIN_LINE_LENGTH) {
		bf->move_state = MOVE_STATE_CRUISE;
		return (_mp_exec_cruise(bf));
	}
	mr.jerk_div2 = bf->jerk/2;
	mr.midpoint_velocity = (bf->entry_velocity + bf->cruise_velocity) / 2;
	if (mr.midpoint_velocity == 0) {INFO1(PSTR("Midpoint velocity = 0 in _mp_exec_accel_0(): %f"), mr.midpoint_velocity)};
	mr.move_time = bf->head_length / mr.midpoint_velocity;	// time for entire accel
	mr.accel_time = 2*sqrt((bf->cruise_velocity - bf->entry_velocity) / bf->jerk);
	mr.midpoint_acceleration = 2*(bf->cruise_velocity - bf->entry_velocity) / mr.accel_time;

	// number of segments in *each half*
	mr.segments = ceil(uSec(mr.move_time) / (2 * cfg.estd_segment_usec));
	if ((uint16_t)mr.segments == 0) {
		INFO(PSTR("No acceleration segments in _mp_exec_accel_0()"));
	...
	bf->move_state = MOVE_STATE_ACCEL_1;
	return (_mp_exec_accel_1(bf));
}

static uint8_t _mp_exec_accel_1(struct mpBuffer *bf) 
{
	mr.segment_velocity = bf->entry_velocity + (square(mr.elapsed_accel_time) * mr.jerk_div2);
	if (_mp_exec_segment(bf) == TG_COMPLETE) {		// signals 1st half of accel is done
		mr.segment_count = (uint32_t)mr.segments;	// setup for second half
		mr.elapsed_accel_time = mr.segment_accel_time / 2;// start time from midpoint of segment
		bf->move_state = MOVE_STATE_ACCEL_2;
	}
	return(TG_EAGAIN);			// in either case the line is not done yet
}
...
static uint8_t _mp_exec_accel_2(struct mpBuffer *bf) 
{
	...
	if (_mp_exec_segment(bf) == TG_COMPLETE) {		// signals 2nd half of accel is done
		bf->move_state = MOVE_STATE_CRUISE;
	}
...
static uint8_t _mp_exec_cruise(struct mpBuffer *bf)
{
	...
	bf->move_state = MOVE_STATE_DECEL_0;	// you are going into decel after this, regardless

	// look for various premature end conditions
	if ((bf->body_length < MIN_LINE_LENGTH) || 	(bf->cruise_velocity < EPSILON)) {
		return (_mp_exec_decel_0(bf));				// skip ahead
	}

	// do the cruise for real
	mr.move_time = bf->body_length / bf->cruise_velocity;
	if (mr.move_time == 0) { INFO1(PSTR("Zero time in _mp_exec_cruise(): %f"), mr.move_time)};
	mr.microseconds = uSec(mr.move_time);

	for (i=0; i < AXES; i++) {
		mr.target[i] += bf->unit[i] * bf->body_length;
		travel[i] = mr.target[i] - mr.position[i];
	}
	(void)ik_kinematics(travel, steps, mr.microseconds);
	SEGMENT_LOGGER	// DEBUG statement
	if (st_prep_line(steps, mr.microseconds) != TG_ZERO_LENGTH_MOVE) {
		copy_axis_vector(mr.position, mr.target); // update runtime position from target	
	}
	return (TG_EAGAIN);
}
...
{
	if (bf->tail_length < MIN_LINE_LENGTH) {
		_mp_finalize_segment(bf);			// finalize for accuracy
		return (TG_OK);						// end the move & free the buffer
	}
	mr.midpoint_velocity = (bf->cruise_velocity + bf->exit_velocity) / 2;
	if (mr.midpoint_velocity == 0) {INFO1(PSTR("Midpoint velocity = 0 in _mp_exec_decel_0(): %f"), mr.midpoint_velocity)};
	...
	if ((uint16_t)mr.segments == 0) {	// more efficient than comparing to < EPSILON
		INFO(PSTR("No deceleration segments in _mp_exec_decel_0"));
		return (TG_OK);					// cancel the move if too small	
	}
	...
	mr.microseconds = uSec(mr.segment_move_time);
	mr.segment_count = (uint32_t)mr.segments;
	bf->move_state = MOVE_STATE_DECEL_1;
	return (_mp_exec_decel_1(bf));
}

static uint8_t _mp_exec_decel_1(struct mpBuffer *bf) 
{
	mr.segment_velocity = bf->cruise_velocity - (square(mr.elapsed_accel_time) * mr.jerk_div2);
	if (_mp_exec_segment(bf) == TG_COMPLETE) {	  // signals 1st half of decel is done
		mr.segment_count = (uint32_t)mr.segments; // setup for second half
		mr.elapsed_accel_time = mr.segment_accel_time / 2;
		bf->move_state = MOVE_STATE_DECEL_2;
	}
	return(TG_EAGAIN);			// in either case the line is not done yet
}
...
static uint8_t _mp_exec_decel_2(struct mpBuffer *bf) 
{
	mr.segment_velocity = mr.midpoint_velocity - 
						 (mr.elapsed_accel_time * mr.midpoint_acceleration) +
						 (square(mr.elapsed_accel_time) * mr.jerk_div2);
	if (_mp_exec_segment(bf) == TG_COMPLETE){	// means entire line is done 
		_mp_finalize_segment(bf);				// finalize for accuracy
...
/*
 * _mp_exec_segment() - segment runner helper
 */
static uint8_t _mp_exec_segment(struct mpBuffer *bf)
{
	uint8_t i;
...
/*
 * _mp_finalize_segment() - last segment runner helper
 */
static void _mp_finalize_segment(struct mpBuffer *bf)
{
	uint8_t i;
	...
	mr.length = get_axis_vector_length(bf->target, mr.position);
	if ((mr.length < MIN_LINE_LENGTH) || (bf->exit_velocity < EPSILON)) {
		return; 				// trap zero-length cases
	}
	mr.move_time = mr.length / bf->exit_velocity;
	mr.microseconds = uSec(mr.move_time);

	for (i=0; i < AXES; i++) {
		travel[i] = bf->target[i] - mr.position[i];	// bf->target is final target of move
	}
	(void)ik_kinematics(travel, steps, mr.microseconds);
	SEGMENT_LOGGER	// DEBUG statement
	if (st_prep_line(steps, mr.microseconds) == TG_OK) {	
		copy_axis_vector(mr.position, bf->target);	// update runtime position from initial target
	}
	return;
}
...
// ZERO segment cases: line below minimum velocity or length
//				   	   Ve  	Vt		Vx		L
