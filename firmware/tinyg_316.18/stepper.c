...
void _st_load_move()
{
...
#ifdef __DISABLE_STEPPERS	// bypasses the load for fast simulations
	...
	if ((st.p = mq_dequeue_motor_buffer()) == NULL) {// NULL = empty buffer
		return;
  	} 
	if (st.p->mq_state != MQ_BUFFER_READY) { 	 // prevent race condition
		return;
	}
	if (st.p->timer_period < TIMER_PERIOD_MIN) { // defensive programming
		TRAP1(PSTR("Timer period too short in st_load_move() %d"), st.p->timer_period);
		return;
	}
