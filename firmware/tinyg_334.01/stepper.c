 ...
 *	which is fine for the TI DRV8811 chips in TinyG but may not suffice
 *	for other drivers.
 */
 ...
 *	planner to generate lines, arcs, dwells and to queue synchronous stops,
 *	ends, and other Mcode commands.
 *
 ...
 *	lines, and to take dwells and Mcodes into account. ("plan" stage).
 *
 ...
 *	or the "exec" level. There are 3 types of exec():
 *
 *	  - Moves (G0 - G3) are executed by generating the next interpolation 
 *		segment (acceleration, cruise, or deceleration segment) for the 
 *		line currently running from the planner. It then "preps" the segment
 *		for loading into the DDA control structure by pre-calculating the DDA
 *		values so this doesn;t have to be done at load time.
 *
 *	  - Dwells (G4) execute line moves, but are much simpler and need only 
 *		transfer the dwell time (as microseconds) via a prep() call.
 *
 *	  - Mcodes are executed as they are found in the planning queue, and must
 *		call a null prep() to keep the control flow from stalling. 
 */
 ...
 *	  	  for the current move. This runs continuously.
 *
 *	  - When the move is finished the stepper interrupt loads the next 
 ...
 *		  by calling the exec routine in planner.c. Or it gets and runs the 
 *		  next buffer in the planning queue - depending on the move_type and 
 *		  state. Once the segment has been computed the exec handler finshes up 
 *		  by running the prep routine in stepper.c. This computes the DDA values 
 *		  and gets the segment into the prep buffer.
 *
 ...
 *	 3b If the steppers are not running this will set a short timer to cause 
 *		a "software interrupt" that will ultimately call st_exec_move().
 *
 ...
 *		case the call is initated via the software interrupt which causes 
 *		_exec_move() to run at the MEDium interupt level.
 *		 
 *	 5	_exec_move() calls back to planner.mp_exec_move() which generates 
 *		the next interpolation segment from the mr singleton. 
 *
 ...
 *
 *		Alternately _exec_move() can return NOOP in which case the cycle ends.
 *
...
void _load_move()
{
#ifdef __DISABLE_STEPPERS						// skip execution for fast simulations
	sp.exec_state = PREP_BUFFER_OWNED_BY_EXEC;
	return;	
#endif
	if (st.timer_ticks_downcount != 0) { return;}		// exit if it's still busy
	if (sp.exec_state != PREP_BUFFER_OWNED_BY_LOADER) {	return;} // defensive programming

	sp.exec_state = PREP_BUFFER_OWNED_BY_EXEC;			// flip the buffer back
	if (sp.move_type == MOVE_TYPE_DWELL) {				// load a dwwll 
	...
	if (sp.move_type == MOVE_TYPE_ALINE) {				// load a move
		st.timer_ticks_downcount = sp.timer_ticks;
	...
	// this last part is executed for dwells, moves AND ALSO NULLs (Mcodes)
	st_request_exec_move();								// exec and prep next move
}
...
uint8_t st_prep_line(double steps[], double microseconds)
{
	...
	if (sp.exec_state != PREP_BUFFER_OWNED_BY_EXEC) {  	// defensive programming
		return (TG_INTERNAL_ERROR);						// prep buffer not owned by exec
	} else if (isfinite(microseconds) == FALSE) {
		return (TG_ZERO_LENGTH_MOVE);					// infinite time move
	} else if (microseconds < EPSILON) {
		return (TG_ZERO_LENGTH_MOVE);					// zero time move
	}
	// get the major axis
	for (i=0; i<MOTORS; i++) {
