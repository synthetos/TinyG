 ...
 *	segments. The length of the segments is configured in planner.h as
 *	MM_PER_ARC_SEGMENT.
 *
 ...
 * _get_arc_time() - apply axis constraints to get minimum arc execution time 
 */
					...
					uint8_t motion_mode)			// defined motion mode
{
	...
	gm.motion_mode = motion_mode;	// copy parameters into the current state

	if ((gm.inverse_feed_rate_mode == false) && (gm.feed_rate == 0)) {
		return (TG_GCODE_FEEDRATE_ERROR);	// trap zero feed rate condition
	}
	cm_set_target(target, flags);			// set parameters
