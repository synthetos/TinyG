...
#define EXEC_FUNC(f,v) if((uint8_t)gf.v != 0) { status = f(gn.v);}
/* Derived from:
	if ((uint8_t)gf.feed_rate != 0) {		// != 0 either means true or non-zero value
		status = cm_set_feed_rate(gn.feed_rate);
	}
 */
...
static uint8_t _execute_gcode_block()
{
	...
	if (gf.program_flow == true) {
		if (gn.program_flow == PROGRAM_STOP) { cm_program_stop();}	// M0, M1
		if (gn.program_flow == PROGRAM_END) { cm_program_end();}	// M2, M30, M60
	}
	return (status);
}
