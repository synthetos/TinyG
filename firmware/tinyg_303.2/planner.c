...
uint8_t mp_aline(double x, double y, double z, double a, double minutes)
{
	...
	if (m->path_mode == PATH_EXACT_PATH) {
		if (m->angular_jerk > cfg.angular_jerk_upper) {// downgrade path
			m->path_mode = PATH_EXACT_STOP; // illustrative, but unnecessary
			m->initial_velocity_req = 0;
...
static uint8_t _mp_backplan(struct mpMovePlanner *m)
{
	...
	double actual_target_velocity = _mp_lookback(p,m); 	// achievable Vt for chain
	...
	// set last move to non-replannable if exact stop mode 
	if (m->path_mode == PATH_EXACT_STOP) {
		m->head->replannable = FALSE;
		m->body->replannable = FALSE;
		m->tail->replannable = FALSE;
	}
	return (TG_OK);
}
...
static uint8_t _mp_run_decel(struct mpBuffer *b)
{
	...
	if (b->move_state == MP_STATE_NEW) {
		...
		if (b->length < MIN_SEGMENT_LENGTH) {//++++++++++++++++++++++++
