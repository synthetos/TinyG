...
#include <stdlib.h>
#include <math.h>
#include <stdio.h>				// only needed for trap strings
//#include <avr/io.h>
...
//####### TRAPS #######
	if (m->length > 50) {
		sprintf(tg.msg, "Length: %f", m->length);
		tg_trap(tg.msg);
	}
	if (m->target_velocity > 300) {
		sprintf(tg.msg, "Speed: %f", m->target_velocity);
		tg_trap(tg.msg);
	}
	if (m->cruise_velocity < 20) {
		sprintf(tg.msg, "Speed: %f", m->cruise_velocity);
		tg_trap(tg.msg);
	}
//####### END TRAPS #######
...
static uint8_t _mp_backplan(struct mpMovePlanner *m)
{
	...
	_mp_lookback(p,m);		// set first Vir to achieve full braking
	while (_mp_make_previous_move(p,m) != TG_COMPLETE) {
		...
		if (++i > 20) {
			sprintf(tg.msg, "Backplanning error: %f", m->length);
			tg_trap(tg.msg);
			break;
		}
	}
...
//		p->initial_velocity_req = p->head->request_velocity;	

		// #### Batman, it's trap! ####
		if (++i > 20) {
			sprintf(tg.msg, "Lookback error: %f", m->length);
			tg_trap(tg.msg);
			break;
		}
	} while (p->head->pv->replannable);

	double spanning_velocity = _mp_get_velocity(0, m->length);
	p->initial_velocity_req = min(spanning_velocity, p->initial_velocity_req);
	return (p->initial_velocity_req);
}
...
//	double test_head_velocity = _mp_get_velocity(Vir,m->head_length); 
	...
	do { // iterate head and tail adjustments to remove body & set Vc
		...
		m->body_length = m->length - m->head_length - m->tail_length;
		if (++i > 100) {
			sprintf(tg.msg, "Iteration error: %f", m->body_length);
			tg_trap(tg.msg);
			break;
		}
	...
	if (m->body_length > 0.01) { 
		sprintf(tg.msg, "Region error: %f", m->body_length);
		tg_trap(tg.msg);
	}
	m->body_length = 0;
