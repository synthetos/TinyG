h2. Readme for TinyG Development Branch
This is the TinyG development branch. The code is completely experimental. There's a good chance that it won't work. If you are interested in the official supported release see the Master branch. If you are experimenting you might check out the Edge branch, which is typically at an alpha state.   


h2. Readme for TinyG 0.93 release
...
* Obeys NIST RS274/NGCv3 with additional guidance from Smid's CNC Programming Handbook v3

New functionality added in 0.93:
* Feedhold and resume - Use ! to stop, ~ to resume - maintains position throughout
* Software reset (abort) - hit ^x to reset from the USB port
* Separate machine and work coordinate systems - supports 6 work coordinate systems (G54 - G59)
* Full support for G92 offsets (G92, G92.1, G92.2, G92.3)
* Homing support - full search + latch homing cycles on any or all 6 axes
* Spindle support - on, direction and off (M3, M4, M5)
* Coolant support - on, off (M7/M8, M9)
* Configurable automatic status reports for "DRO" style output during motion
