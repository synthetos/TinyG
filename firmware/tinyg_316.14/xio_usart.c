...
void xio_xoff_usart(const uint8_t dev)
{
	...
	// if flow control is enabled and you are not already in it....
//	if ((EN_XOFF(d->flags) != FALSE) && (IN_XOFF(d->flags) == FALSE)) {
	if ((EN_XOFF(d->flags) == TRUE) && (IN_XOFF(d->flags) == FALSE)) {
		d->flags |= XIO_FLAG_DO_XOFF_bm;	// flag to do an XOFF
		dx->usart->CTRLA = CTRLA_RXON_TXON;	// force a TX interrupt
		en_bit_on(0x01);					// turn on XOFF LED
//		PMIC_EnableLowLevel(); 				// enable TX interrupts
		PMIC_EnableMediumLevel(); 			// enable TX interrupts
...
void xio_xon_usart(const uint8_t dev)
{
	...
	// if flow control is enabled and you are already in it....
//	if ((EN_XOFF(d->flags) != FALSE) && (IN_XOFF(d->flags) != FALSE)) {
	if ((EN_XOFF(d->flags) == TRUE) && (IN_XOFF(d->flags) != FALSE)) {
		d->flags |= XIO_FLAG_DO_XON_bm;		// flag to do an XON
		dx->usart->CTRLA = CTRLA_RXON_TXON;	// force a TX interrupt
		en_bit_off(0x01);					// turn off XOFF LED
///		PMIC_EnableLowLevel(); 				// enable TX interrupts
		PMIC_EnableMediumLevel(); 			// enable TX interrupts
		sei();								// enable global ints
	}
 ...
 *
 *	Remember: The rx queue fills from top to bottom, w/0 being the wrap
 */
...
int xio_putc_usart(const uint8_t dev, const char c, FILE *stream)
{
	...
	while(dx->next_tx_buf_head == dx->tx_buf_tail) { // sleep or return
		if (BLOCKING(d->flags) != 0) {
			sleep_mode();
		} else {
			d->signal = XIO_SIG_EAGAIN;
			en_bit_on(0x08);					// +++++++ light XOFF LED
			return(_FDEV_ERR);
		}
	};
	...
	en_bit_off(0x08);							// +++++++ clear OXOFF LED
	if ((dx->usart->STATUS & USART_DREIF_bm) != 0) {// 0x20
		if (dx->tx_buf_head == dx->tx_buf_tail) {//might be empty if IRQ got it
			return (XIO_OK);
		}
		d->flags |= XIO_FLAG_TX_MUTEX_bm;// mutual exclusion w/ISR
		if (--(dx->tx_buf_tail) == 0) {	// advance tail & wrap if needed
			dx->tx_buf_tail = TX_BUFFER_SIZE-1;	// -1 to avoid off-by-one (OBOE)
		}
		dx->usart->DATA = dx->tx_buf[dx->tx_buf_tail];// write to DATA reg
...
//	PMIC_EnableLowLevel(); 				// enable USART TX interrupts
	PMIC_EnableMediumLevel(); 			// enable USART TX interrupts
...
int xio_getc_usart(const uint8_t dev, FILE *stream)
{
	...
	if ((dev == XIO_DEV_USB) && (xio_get_rx_bufcount_usart(dx) < XOFF_LO_WATER_MARK)) {
