...
// turning USART interrupts on and off (these are the combos actually used)
// These need to change if you change the interrupt priority levels used
...
//Choose one: (this define chages data type in all places that need it)
//#define _BUFTYPE uint8_t		// faster, but limits buffer to 255 char max
#define _BUFTYPE uint16_t		// slower, but larger buffers
...
//#define RX_BUFFER_SIZE 64		// uint8_t buffer type is OK
//#define RX_BUFFER_SIZE 128	// uint8_t buffer type is OK
//#define RX_BUFFER_SIZE 255	// uint8_t buffer type is OK
#define RX_BUFFER_SIZE 256		// uint16_t buffer type is required
...
//	uint16_t uflags;			  // usart sub-system flags (UNUSED)
	_BUFTYPE rx_buf_tail;		  // RX buffer read index  (not VOLATILE)
	volatile _BUFTYPE rx_buf_head;// RX buffer write index (written by ISR)
	volatile _BUFTYPE tx_buf_tail; // TX buffer read index  (written by ISR)
	_BUFTYPE tx_buf_head;		  // TX buffer write index (not VOLATILE)
	_BUFTYPE next_tx_buf_head;	  // next TX buffer write index
...
// down here by its lonesome becaase it needs xioUSART defined
_BUFTYPE xio_get_rx_bufcount_usart(const struct xioUSART *dx);
