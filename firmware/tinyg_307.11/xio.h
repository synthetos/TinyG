...
//FILE * xio_std_in;				// stdin device assignment
//FILE * xio_std_out;				// stdout device assignment
//FILE * xio_std_err;				// stderr device assignment
...
struct xioDEVICE {					// common device struct (one per dev)
	...
	int size;						// test buffer length (dynamic)
	...
	void *x;						// extended IO binding (static)
...
/*---- Notes on control characters and signals ----
  ...
  (e.g. a file device), while pause, resume and kill are still active from 
  another device (e.g. the USB port being used as a console).
