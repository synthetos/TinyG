...
void ls_init(void) 
{
	...
	for (i=0; i<MOTORS; i++) {
		...
		MOTOR(i).port->DIRSET = MIN_LIMIT_BIT_bm;	// set min to output
		MOTOR(i).port->OUTSET = MIN_LIMIT_BIT_bm;	// min bit off
		MOTOR(i).port->DIRSET = MAX_LIMIT_BIT_bm;	// set max to output
		MOTOR(i).port->OUTSET = MAX_LIMIT_BIT_bm;	// max bit off

		// setup ports bits as ipputs
		MOTOR(i).port->DIRCLR = MIN_LIMIT_BIT_bm;		 // set min input
		MOTOR(i).port->PIN6CTRL = (LS_OPC_gc | LS_ISC_gc);// pin modes
		MOTOR(i).port->INT0MASK = MIN_LIMIT_BIT_bm;		 // min on INT0

		MOTOR(i).port->DIRCLR = MAX_LIMIT_BIT_bm;		 // set max input
		MOTOR(i).port->PIN7CTRL = (LS_OPC_gc | LS_ISC_gc);// pin modes
		MOTOR(i).port->INT1MASK = MAX_LIMIT_BIT_bm;		 // max on INT1

		// set interrupt levels. Interrupts must be enabled in main()
		MOTOR(i).port->INTCTRL = (PORT_INT0LVL_MED_gc|PORT_INT1LVL_MED_gc);
	}
...
void ls_read_limit_switches() 
{
	...
	for (i=0; i<MOTORS; i++) {
		if (!(MOTOR(i).port->IN & MIN_LIMIT_BIT_bm)) { 	// min
			ls.flag[flag] = TRUE;
			ls.thrown = TRUE;
		}
		flag++;
		if (!(MOTOR(i).port->IN & MAX_LIMIT_BIT_bm)) { 	// max
			ls.flag[flag] = TRUE;
			ls.thrown = TRUE;
		}
		flag++;
