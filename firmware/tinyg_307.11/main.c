...
---- ToDo now ----
  ...
  Networking
	- reconcile operating modes with init_stdio. Not in the right place yet

---- ToDo next ----
	- add BC rotary axes
	- add slaved UVW axes
	- add M code extensions and RS485 command
...
---- ToDo later ----
	...
	- add a PWN channel controllable like an axis (ARM only?)
...
void tg_system_init(void)
{
	...
	tg_init();				// (3) tinyg controller (selects std devices)
	xio_init_stdio();		// (4) set stdin, stdout, stderr
	cfg_init();				// (5) get config record from eeprom (reqs xio)
	sig_init();				// (6) signal flags
	rtc_init();				// (7) real time counter
//	cfg_dump_NVM(0, 40 "From system init");
...
//	EEPROM_tests();				// EEPROM functions
	cfg_unit_tests();			// config functions
...
/*
 * tg_trap() - trap and log an exception
 *
 *	Memory usage notes: The print to stderr is program memory
 *	The msg passed to it is in RAM. Typically the strings that get loaded 
 *	into the RAM buffer are from program memory - see tinyg.h TRAP macros
 */

void tg_trap(char *msg)
{
#ifndef __SIMULATION_MODE	// a convenience
	fprintf_P(stderr, PSTR("#### TRAP %s ####\n"), msg);
#endif
	return;
}
