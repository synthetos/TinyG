...
void en_write(uint8_t b)
{
	...
	if (b & 0x01) { // b0 is on MOTOR_4 (A axis)
		MOTOR(MOTOR_4).port->OUTSET = ENCODER_OUT_BIT_bm;
	} else {
		MOTOR(MOTOR_4).port->OUTCLR = ENCODER_OUT_BIT_bm;
	}

	if (b & 0x02) { // b1 is on MOTOR_3 (Z axis)
		MOTOR(MOTOR_3).port->OUTSET = ENCODER_OUT_BIT_bm;
	} else {
		MOTOR(MOTOR_3).port->OUTCLR = ENCODER_OUT_BIT_bm;
	}

	if (b & 0x04) { // b2 is on MOTOR_2 (Y axis)
		MOTOR(MOTOR_2).port->OUTSET = ENCODER_OUT_BIT_bm;
	} else {
		MOTOR(MOTOR_2).port->OUTCLR = ENCODER_OUT_BIT_bm;
	}

	if (b & 0x08) { // b3 is on MOTOR_1 (X axis)
		MOTOR(MOTOR_1).port->OUTSET = ENCODER_OUT_BIT_bm;
	} else {
		MOTOR(MOTOR_1).port->OUTCLR = ENCODER_OUT_BIT_bm;
	}
}
