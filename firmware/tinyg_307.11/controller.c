...
static uint8_t _tg_read_next_line()
{
	...
	if ((tg.status = xio_gets(tg.src, tg.buf, sizeof(tg.buf))) == TG_OK) {
		...
		fprintf_P(stderr, PSTR("Read next line %s\n"), tg.buf);
#endif
...
{
	fprintf_P(stderr, PSTR("tinyg[%S]ok> "),(PGM_P)pgm_read_word(&tgModeStrings[tg.mode]));
...
//	xio_queue_RX_string_usb("g0 x20 y23 a40\n");	// G0's
