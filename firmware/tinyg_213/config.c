...
{
/*
	// = 360 / (degree_per_step/microstep) / travel_rev
	...
	cfg.max_feed_rate = ((60 * (double)cfg.a[X].feed_steps_sec) /
						 (360/cfg.a[X].degree_per_step/cfg.a[X].travel_rev));
...
*/
	for (uint8_t i=X; i<=A; i++) {
		// = 360 / (degree_per_step/microstep) / travel_per_rev
		CFG(i).steps_per_unit = (360 / (CFG(i).degree_per_step / CFG(i).microstep)) / CFG(i).travel_rev;
...
void _cfg_load_hardwired_settings()
{
	...
	cfg.gcode_homing_mode = HOMING_MODE;
//	cfg.gcode_feed_rate = GCODE_FEED_RATE;
	cfg.gcode_tool = GCODE_TOOL;
	cfg.gcode_spindle_speed = SPINDLE_SPEED;
