...
struct MotionControlArc {			// vars used by arc generation & continue
	int8_t arc_continue_state;		// arc generator continuation state
	...
	double segment_time;			// time to complete a segment
...
int mc_arc(double theta, 			// starting angle
		   ...
		   double feed_rate, 		// feed rate or inverse feed rate
		   uint8_t invert_feed_rate)// feed rate mode
{
	...
	// compute times for arc and segments
	if (invert_feed_rate) {
		ma.segment_time = (feed_rate / ma.segments);
	} else {
		ma.segment_time = (ma.mm_of_travel / feed_rate) / ma.segments;
	}
...
int mc_arc_continue() 
{
	...
	while (ma.segment_counter <= ma.segments) {
		...
		mc_line(ma.dtarget[X], ma.dtarget[Y], 
				ma.dtarget[Z], ma.dtarget[A], ma.segment_time);
  	}
