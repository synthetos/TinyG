...
{ 
	if ((gm.absolute_mode == TRUE) || (gm.absolute_override == TRUE)) {
		gm.target[X] = _to_millimeters(target[X]);
		gm.target[Y] = _to_millimeters(target[Y]);
		gm.target[Z] = _to_millimeters(target[Z]);
		gm.target[A] = target[A];
		gm.target[B] = target[B];
		gm.target[C] = target[C];
	} else {
		gm.target[X] += _to_millimeters(target[X]);
		gm.target[Y] += _to_millimeters(target[Y]);
		gm.target[Z] += _to_millimeters(target[Z]);
 ...
 *	- conversion to linear units to canonical form (internal): mm, degrees
 *	- conversion of relative mode to absolute (internal form)
 ...
 *
 * 		CIRCUMFERENCE
 *		  - ABC axis value is presented in linear units
 *		  - Target is set to degrees based on axis' Circumference value
 *
 ...
 *
 *	  Circumference and slave modes are only processed for ABC axes.
 *	  Attempts to apply them for XYZ are trapped. Inputs are ignored.
 */
...
{ 
	...
	for (i=A; i<C; i++) {
		switch (cfg.a[i].axis_mode) {
			...
			case (AXIS_CIRCUMFERENCE): {
				target_tmp = _to_millimeters(target[i]) * 360 / cfg.a[i].circumference;
				break;
			}
			case (AXIS_SLAVE_X): {
				length = (target[X] - gm.position[X]);
				target_tmp = length * 360 / (cfg.a[i].circumference);
				break;
			}
			case (AXIS_SLAVE_Y): {
