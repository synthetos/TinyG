...
enum cfgMnemonics { P_, V_,		// <-- profile and version records. Must be first.
	...
	GU, GL, GP, GD, EA, 
	JA, ML, MC, MT, HM, 
	EC, IC, IL, EX, EE,			// 15 general settings (adjust #define COUNT_GENERAL)
...
#define COUNT_GENERAL 15		// <-- this must agree with above
...
char finJM[] PROGMEM = "Jerk maximum    %8.2f in/min^3     $%c%s%1.0f\n";
...
static void _appinJM(struct cfgSetting *s) { s->value *= MM_IN_CUBED; _appmmJM(s); }
...
static void _applyGC(struct cfgSetting *s) { 
	switch ((int)s->value*10) {	// *10 to pick up 61.1 and any other XX.X gcode words
...
static void _cfg_print_setting(const struct cfgSetting *s)
{
	...
	// convert that nasty linear jerk value to inches
	// mm mode displays
	if (inches_mode == FALSE) {
		...
		if (UNIT_CONVERSION_REQUIRED) {
			if (s->mnem == JM) {
				value /= MM_IN_CUBED;	// jerk is a special case
			} else {
				value = s->value / MM_PER_INCH;
			}
		}
		if (s->grp == CFG_GENERAL) {
