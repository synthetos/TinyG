...
#define NV_SHARED_STRING_LEN 256		// shared string for string values
...
typedef enum {						    // value typing for config and JSON parsing
    // exception types
    ...
    TYPE_INTEGER = 2,                   // value is integer, may be be converted to SIGNED or FLOAT later
	TYPE_STRING = 3,                    // value is in string field

    // derived data types               // must be 4-7. Do not change.
	TYPE_FLOAT = 4,                     // floating point number
    TYPE_SIGNED = 5,                    // signed integer
	TYPE_DATA = 6,                      // derived value is blind cast to uint32_t (deprecated)
    TYPE_TXTCON = 7,                    // special type NV pair filled in by _js_run_container_as_text()
...
#define F_TYPE_MASK     0x07            // 3 LSB's used for data type
#define F_RESERVED      0x08            // may be used for array types

#define F_INITIALIZE	0x10			// initialize this item (run set during initialization)
#define F_PERSIST 		0x20			// persist this item when set is run
#define F_NOSTRIP		0x40			// do not strip the group prefix from the token
#define F_CONVERT		0x80			// set if unit conversion is required
...
#define _nn0    (TYPE_NULL | F_NOSTRIP)

#define _s0     (TYPE_STRING)
#define _sn0    (TYPE_STRING | F_NOSTRIP)
...
#ifdef __BITFIELDS
    uint8_t nv_num      : 6 ;           // handy diagnostic
	int8_t depth        : 3 ;           // depth of object in the tree. 0 is root (-1 is invalid)
	valueType valuetype : 6 ;           // see valueType enum
	int8_t precision    : 4 ;           // decimal precision for reporting (JSON)
#else
...
//    char *group;
//    char *token;
	char *str;				            // pointer to string in RAM
...
//	uint16_t flags;						// operations flags - see defines below
...
bool cfg_has_flag(index_t index, uint8_t bitmask);  // test for flag in cfgArray
valueType cfg_is_type(index_t index);       // return data type of cfgArray element
...
stat_t set_str(nvObj_t *nv);                // set string value - just for test, performs no action

stat_t get_nul(nvObj_t *nv);				// get null value type
stat_t get_str(nvObj_t *nv);                // get str value from NV
...
stat_t get_int(nvObj_t *nv);                // get value as a signed integer (up to 32 bits)
...
nvObj_t *nv_prev(nvObj_t *nv);
nvObj_t *nv_next(nvObj_t *nv);
