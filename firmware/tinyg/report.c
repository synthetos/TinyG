...
{
	return(GET_TEXT_ITEM(stat_msg, status));
/*
#ifdef __AVR
//	strncpy_P(status_message,(const char *)pgm_read_word(&stat_msg[status]), STATUS_MESSAGE_LEN);
//	return (status_message);
	return(strcpy_P(status_message,(const char *)pgm_read_word(&stat_msg[status])));
...
{
	_startup_helper(STAT_INITIALIZING, (const char_t PROGMEM *)INIT_MESSAGE);
}

void rpt_print_loading_configs_message(void)
{
	_startup_helper(STAT_INITIALIZING, (const char_t PROGMEM *)"Loading configs from EEPROM");
}

void rpt_print_system_ready_message(void)
{
	_startup_helper(STAT_OK, (const char_t PROGMEM *)"SYSTEM READY");
