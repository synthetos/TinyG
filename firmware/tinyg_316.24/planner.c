...
enum mpMoveType {				// bf->move_type values 
	...
	MP_TYPE_ACCEL,				// controlled jerk acceleration segment
	MP_TYPE_CRUISE,				// cruise at fixed velocity
	MP_TYPE_DECEL,				// controlled jerk deceleration segment
...
struct mpBuffer {				// move segment structure (3 segment model)
	...
	uint8_t replannable;		// TRUE if segment can be replanned

	double time;				// line, helix or dwell time in minutes
	double length;				// line or helix length in mm
	double start_velocity;		// actual starting velocity of a segment
	double end_velocity;		// actual ending velocity of a segment
...
struct mpMoveMasterSingleton {	// common variables for planning (move master)
	...
	double tangential_v_max[AXES];//for computing safe junction speed
	double linear_jerk_rad3;	// cube root of max linear jerk
};

struct mpMovePlanner {			// used to compute or recompute segments
...
//		read in spite of the longer lines.
...
static uint8_t _mp_compute_segments(const double Vir, const double Vt, const double Vf, struct mpMovePlanner *m);
...
static void	_mp_set_tangential_v_max(const double jerk_max, const double time);
static double _mp_get_join_velocity(double a_unit[], double b_unit[], 
									double a_velocity, double b_velocity);
static double _mp_get_tangential_jerk_factor(double a_unit[], double b_unit[]);
...
//	cfg.tangential_jerk_upper_boundary = TANGENTIAL_JERK_UPPER_BOUNDARY;
//	cfg.tangential_jerk_lower_boundary = TANGENTIAL_JERK_LOWER_BOUNDARY;
}
 ...
 *
 *	This is referred to in the literature as a 5 segment move, as the accel
 *	and decel segments are divided in to concave and convex halves of the S 
 *	curve. So in a 5 segment move there are two acceleration segments, 
 *	followed by a cruise segment, followed by two deceleration segments. 
 *
 ...
 */
/*	Aline() plans a linear move as 3 segments:
...
uint8_t mp_aline(const double target[], const double minutes)
{
	...
	_mp_set_unit_vector(mm.unit, mm.target, mm.position);
//	mp_copy_vector(m->head->unit, mm.unit, AXES); // need this in the head

	// initialize jerk terms
	mr.linear_jerk_div2 = cfg.linear_jerk_max/2; // used by runtime
	mm.linear_jerk_rad3 = cubert(cfg.linear_jerk_max);
	_mp_set_tangential_v_max(cfg.linear_jerk_max, TANGENTIAL_JERK_MINUTES);
	...
	if ((t->move_type == MP_TYPE_ARC) && (t->buffer_state != MP_BUFFER_EMPTY)) {
		...
		(void)_mp_compute_segments(m->initial_velocity_req, m->target_velocity, 0, m);
	...
	if (t->buffer_state != MP_BUFFER_QUEUED) {
		m->path_mode = PATH_EXACT_STOP;			// downgrade path
		m->initial_velocity_req = 0;			// junction velocity = zero
	} else { 
		// find the optimal velocity for a jerk-limited junction
		m->initial_velocity_req = _mp_get_join_velocity(t->unit, mm.unit, 
														t->request_velocity, m->target_velocity);
	}

	// do the actual work
	if (_mp_compute_segments(m->initial_velocity_req, m->target_velocity, 0, m) == 0) {;
 ...
 * _mp_compute_segments() 	  - compute segment lengths and velocity contours
 ...
 * _mp_queue_move()			  - queue 3 segments of a move
 ...
 *	segment lengths for each of the constituent moves. If a move becomes 
...
static void _mp_backplan(struct mpMovePlanner *m)
{
	...
	while (_mp_make_previous_move(p,m) != TG_COMPLETE) {
		(void)_mp_compute_segments(p->initial_velocity_req, p->target_velocity, m->initial_velocity, p);
		_mp_update_move(p,m); 
...
static uint8_t _mp_compute_segments(const double Vir, const double Vt, const double Vf, struct mpMovePlanner *m) 
{
	...
	if (m->length < MIN_LINE_LENGTH) {	// line is too short or zero
		TRAP1(PSTR("Line too short in _mp_compute_segments() len = %f"), m->length);
		return (0);
	}
	...
	do { // iterate head and tail adjustments to remove body & set Vc
		...
		m->body_length = m->length - m->head_length - m->tail_length;
		if (++i > 100) {
			TRAP1(PSTR("Iteration error in in _mp_compute_segments() len = %f"), m->body_length);
			break;
		}
	} while (fabs(_tmp_length - m->body_length) > EPSILON);

	if (m->body_length > 0.01) { 
		TRAP1(PSTR("Segment error in in _mp_compute_segments() len = %f"), m->body_length);
	}
	m->body_length = 0;
...
/*	
 * _mp_set_tangential_vmax()
 *
 *  Sets the maximum allowable change in velocity (deltaV) for each axis 
 *	based on the linear jerk maximum and the time interval in which to 
 *	transition at that jerk.
 *
 *	For now this rutine is effectively scalar - all axes are the same.
 *	In the future each axis can have a different value that more accurately
 *	describes the machine characteristics.
 *
 *	Also, the time value is somewhat arbitrary and the output is very 
 *	sensitive to changes in time. Nature of the beast.
 */

static void	_mp_set_tangential_v_max(const double jerk_max, const double time)
{
	for (uint8_t i=0; i<AXES; i++) {
		mm.tangential_v_max[i] = jerk_max * (square(time))/2;
	}
}
 ...
 *
 *	Inputs:
 *		a	- Segment structure containing line A tail
 *		b 	- Segment structure containing line B head
 *
 *	The function then scales the resulting velocity using the join angle.
 *	No scaling occurs for a junction with no angle (a straight join), 
 *	and the velocity is multipled by zero (set to zero) for a 180 degree 
 *	reversal. In between values return a cosine value that is computed 
 *	from half the join angle. The scaling equations are:
 *
 *	theta = arcosine((AxBx + AyBy + AzBz + AaBa + AbBa + AcBc) / AB)
 *	scaled_velocity = velocity * cos(theta/2) 
 *
 *	...where AB is the dot product of the vectors; but since the vectors 
 *	are unit vectors we know the length is 1 and don't have to compute AB.
 *
 *	ref: http://chemistry.about.com/od/workedchemistryproblems/a/scalar-product-vectors-problem.htm
 *
 *	First segment line (0 - 90 degrees) is y = mx + b
 *		y = (Vjerk-request_velocity)/(pi/2) * theta + request_velocity
 *
 *	Second segment line (90 - 180 degrees) is y = mx + b
 *		y = (Vjerk)/(pi/2) * (theta - pi/2) + Vjerk
//	Vtest = (a_velocity * _mp_get_tangential_jerk_factor(a_unit, b_unit));
//	fprintf(stderr, (",%s,"), tg.buf); 
//	fprintf(stderr, ("Av,%5.2f,[,%1.5f,%1.5f,%1.5f,],"), a_velocity, a_unit[X], a_unit[Y], a_unit[Z]);
//	fprintf(stderr, ("Bv,%5.2f,[,%1.5f,%1.5f,%1.5f,],"), b_velocity, b_unit[X], b_unit[Y], b_unit[Z]);
//	fprintf(stderr, ("%5.2f,%5.2f\n"), Vtest, Vresult); 

  
 */

static double _mp_get_join_velocity(double a_unit[], double b_unit[], double a_velocity, double b_velocity)
{
	double Va, Vdelta, Vfactor;
	double b_vel = b_velocity;
	double VfactorMin = 1;
	double Vjerk[AXES] = { 100, 100, 50, 1000, 1000, 1000 };

	for (uint8_t i=0; i<AXES; i++) {
		// STEPS:
		// handle case where planning line (b) has higher feedrate than a
		// skip non-contributing axes
		// compute component velocity and trap zero cases
		// compute change in velocity. Ignore if within jerk limits
		// compute adjustment factor to keep velocity within jerk limits
		// capture the minimum (smallest) factor - which will be used to adjust the move velocity

		if (a_velocity < b_velocity) { 
			b_vel = a_velocity; 
		}
		if ((a_unit[i] < EPSILON) && (b_unit[i] < EPSILON)) { 
			continue;
		}
		if ((Va = a_unit[i] * a_velocity) < EPSILON) {
			continue;
		}
		if ((Vdelta = fabs((b_unit[i] * b_vel) - Va)) <= Vjerk[i]) {
			continue;
		}
//		Vadj = max(Va - max((Vdelta - Vjerk[i]),0),0);
		Vfactor = (max(Va - max((Vdelta - Vjerk[i]),0),0))/Va;
		VfactorMin = min(VfactorMin, Vfactor);
	}
	return (a_velocity * VfactorMin);
}

/*	
 * _mp_get_tangential_jerk_factor()
 *
 */

static double _mp_get_tangential_jerk_factor(double a_unit[], double b_unit[])
{
	// get the angle from 0 degrees to 180 degrees from the unit vectors
	double theta = acos((a_unit[X] * b_unit[X]) +
						(a_unit[Y] * b_unit[Y]) +
						(a_unit[Z] * b_unit[Z]) +
						(a_unit[A] * b_unit[A]) +
						(a_unit[B] * b_unit[B]) +
						(a_unit[C] * b_unit[C]));
	if (theta > M_PI) {	// is this adjustment necessary?
		theta -= M_PI;	// shift > 180 degree angles to < 180 degrees
	}
/*
	if (m->initial_velocity_req > cfg.tangential_jerk_upper_boundary) {
		return (1-sin(theta/2));		// apply 1-sin rule
	} else if (m->initial_velocity_req < cfg.tangential_jerk_lower_boundary) {
		return (cos(theta/2));			// apply cos rule
	} else {
		return (1-(theta/(M_PI/2)));	// apply linear rule
	}
*/
	return (cos(theta/2));			// apply cos rule
}
										 ...
										 const double Vr, const double len)
{
	...
	for (i=0; i < AXES; i++) { 			// copy unit vector from mm
		bf->unit[i] = mm.unit[i]; 
		mm.position[i] += len * bf->unit[i]; // set mm position
...
static void _mp_update_move(const struct mpMovePlanner *p, const struct mpMovePlanner *m) 
{
	// update segment buffers from planning structure
...
void _mp_call_recompute_vt(double l, double Vp, double Vi, double Vt) 
{
	...
	_mp_compute_segments(m->initial_velocity_req, m->target_velocity, 0, m);
}

void _mp_test_recompute_vt()
{
//						  Len	Vp	 Vi	  Vt
	_mp_call_recompute_vt( 3.0, 250, 100, 400);	// 3 segments - fits
	_mp_call_recompute_vt( 2.0, 250, 100, 400);	// 2 segments - simple reduction
	_mp_call_recompute_vt( 1.0, 250, 100, 400);	// 1 segment - more extreme reduction
	_mp_call_recompute_vt( 0.5, 250, 100, 400);	// 1 segment - Vi reduces below Vp
												// 1 segment - zero legnth line
