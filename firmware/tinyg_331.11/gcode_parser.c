		...
		switch(letter) {
			case 'G':
				switch((uint8_t)value) {
					...
					case 20: SET_NEXT_STATE(units_mode, INCHES);
					case 21: SET_NEXT_STATE(units_mode, MILLIMETERS);
...
static uint8_t _gc_execute_gcode_block() 
{
	...
	CALL_FUNC(cm_set_inverse_feed_rate_mode, inverse_feed_rate_mode);
	CALL_FUNC(cm_set_feed_rate, feed_rate);
	CALL_FUNC(cm_set_spindle_speed, spindle_speed);
	CALL_FUNC(cm_select_tool, tool);
	CALL_FUNC(cm_change_tool, tool);
	CALL_FUNC(cm_spindle_control, spindle_mode); 	// spindle on or off
	...
	CALL_FUNC(cm_select_plane, select_plane);
	CALL_FUNC(cm_set_units_mode, units_mode);
	//--> cutter radius compensation goes here
	//--> cutter length compensation goes here
	CALL_FUNC(cm_set_coord_system, coord_system);
	CALL_FUNC(cm_set_path_control, path_control);
	CALL_FUNC(cm_set_distance_mode, distance_mode);
	...
	if ((uint8_t)gf.origin_offset_mode != 0) {			// G92's - set/cancel/suspend/resume axis offsets
		return(cm_set_origin_offsets(gn.origin_offset_mode, gn.target, gf.target));
	}
