...
{
	_get_ui8(cmd);
	return(_get_msg(cmd, (prog_char_ptr)msg_am, cmd->value));
}
//old method:
//	_get_ui8(cmd);
...
{
	cmdObj cmd;
	char exclude[] = {"sr,gc"};		// don't try to SET these tokens
	...
	cmd_read_NVM_value(&cmd);		// read the first record in NVM
...
{
	if ((cmd->index < 0) || (cmd->index >= CMD_MAX_INDEX)) return (TG_UNRECOGNIZED_COMMAND);
	int8_t nvm_record[NVM_RECORD_LEN];
	uint16_t nvm_address = cfg.nvm_profile_base + (cmd->index * NVM_RECORD_LEN);
...
uint8_t cmd_write_NVM_record(cmdObj *cmd)
{
	if ((cmd->index < 0) || (cmd->index >= CMD_MAX_INDEX)) return (TG_UNRECOGNIZED_COMMAND);
	int8_t nvm_record[NVM_RECORD_LEN];
	uint16_t nvm_address = cfg.nvm_profile_base + (cmd->index * NVM_RECORD_LEN);
