...
struct mpBuffer {				// move/sub-move motion control structure
	...
	uint8_t plannable;			// TRUE if region can be replanned
...
struct mpMovePlanner {			// used to compute or recompute regions
	...
	uint8_t waypoint_status;	// waypoint status or path control mode 
...
uint8_t mp_aline(double x, double y, double z, double a, double minutes)
{
	...
	m->path_mode = cm_get_path_control_mode();	// starting path mode
	...
	if (m->path_mode == PATH_CONTINUOUS) {
		if (m->angular_jerk > cfg.angular_jerk_lower) {
			m->path_mode = PATH_EXACT_PATH; 	// downgrade path
 ...
 * _mp_lookback()
 *
 *	Starting with the current M struct, look backwards to find the stopping 
 *	point for backwards replan: Waypoint conditions may be:
 *	(1) the waypoiint is an exact stop waypoint (path_mode = exact stop)
 *	(2) an exact path waypoint that's at it's max velocity (Vf = Vf requested)
 *	(3) a continuous mode waypoint that's at it's maximum v (Vf = Vt)
 *	(4) an M struct that's not queued ("run off the end")
 *
 *	Label the waypoint as the stopping point.
 *
 *	In the process, accumulate the total path length of the recomputable
 *	segments and use this to determine the maximum achievable velocity 
 *	between the (fixed) Vi and Vf = 0
 */
static double _mp_lookback(struct mpMovePlanner *m)
{
	struct mpMovePlanner *p = &mp[1];// pointer to a move in bkwds chain
	struct mpMovePlanner *tmp;
