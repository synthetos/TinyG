...
char cfs04[] PROGMEM = "  Gcode: {G20/G21} Units (21=mm)  %1.0f";
...
char cfs27[] PROGMEM = "  %c axis - Homing rate        %5.0f";
...
{
	printf_P(PSTR("*** TinyG Configuration Help ***\n\
You are in CONFIG mode as you can see by the prompt\n\
- To see general parameters hit '?'\n\
- To see axis parameters hit '?x' (or whatever axis you want x,y,z,a)\n\
- To set parameters type in a string with the right token and value\n\
	For example yfe800 to set the Y max feed rate to 800 steps per second\n\
	Input is very forgiving of caps, spaces and extra characters\n\
	The value taken will be echoed back to the console\n\
- To leave CONFIG mode hit 'q'\n\
