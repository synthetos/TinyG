...
enum cmMachineState {				// check status strings for cm_print_machine_state()
	...
	MACHINE_HOMING					// machine in homing cycle
};
...
enum cmHomingState {				// homing progress and completion states
	HOMING_NOT_HOMED,				// machine homed state
	HOMING_HOMED,
									// states for homing cycle progress 
	HOMING_NEW,						// initial call to cycle
	HOMING_X_START,					// start X homing move
	HOMING_X_WAIT,					// wait for limit switch or end-of-move
	HOMING_Y_START,
	HOMING_Y_WAIT,
	HOMING_Z_START,
	HOMING_Z_WAIT,
	HOMING_A_START,
	HOMING_A_WAIT,
	HOMING_RTZ_START,				// return to zero move
	HOMING_RTZ_WAIT
};
...
void cm_run_status_report(void);
...
uint8_t cm_old_homing_cycle(void);
uint8_t cm_old_homing_callback(void);
