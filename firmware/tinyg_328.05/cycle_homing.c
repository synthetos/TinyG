...
uint8_t _cm_old_run_homing_cycle(void);
uint8_t _cm_homing_kernal(void);

/* 
 * cm_homing_cycle() - homing cycle using limit switches
 * cm_homing_kernal() - core routines for homing cycle
 * cm_homing_callback() - continuation for the above
 */
/*	The homing cycle sequences through the various homing moves and reacts to 
 *	limit switch closures. It's a bit tricky because the routine can be re-entered 
 *	if a limit switch is hit (ANY limit switch) or if the previously queued move 
 *	completes.
 *
 ...
 *		- setup for incremental travel and perform some other inits
 *
 *	  - cm_run_homing_cycle()
 ...
 *		  - issue a move to -travel_max in that dimension
 *		  - when the move completes or a limit switch is hit:
 *			- backoff from that edge by a nominal amount
...
{
	double flags[AXES] = { 1,1,1,1,1,1 };	// axes to be homed

	// initialize this whole operation
	cm.machine_state = MACHINE_HOMING;
	cm.homing_state = HOMING_NEW;
	...
	sw_clear_limit_switches();				// reset the switch flags
	cm.homing_state = HOMING_NEW;
	return (TG_OK);
}
/*
double *cm_set_homing_flags(double flags[])
{
	return (flags);
}
*/
...
{
	if ((cm.homing_state == HOMING_HOMED) || (cm.homing_state == HOMING_NOT_HOMED)) { 
		return (TG_NOOP);
	}
	if (mp_isbusy() == TRUE) {   			// sync to the move queue
		return (TG_EAGAIN); 
	}
	if (_cm_old_run_homing_cycle() == TG_COMPLETE) { 
		return (TG_OK); 
	} 
	return (TG_EAGAIN);
}
...
/* 
 * cm_old_homing_cycle() - homing cycle using limit switches
 * cm_homing_cycle_callback() - continuation for the above
 *
 *	The homing_cycle is coded as a continuation state machine. It sequences
...
uint8_t cm_old_homing_callback()				// outer runtime routine
{
	return (TG_OK);
}

uint8_t _cm_old_run_homing_cycle()				// inner runtime routine
{
	// handle any initial switch closures by backing off the switch
	if (cm.homing_state == HOMING_NEW) {
		cm.homing_state = HOMING_X_START;
		sw_read_limit_switches();
		if (sw_xmin_thrown() == TRUE) {
			sw_clear_limit_switches();
			cm_set_vector(CFG(X).homing_backoff, 0, 0, 0, 0, 0);
			return(HOMING_BACKOFF_MOVE(vector));
		}
		if (sw_ymin_thrown() == TRUE) {
			sw_clear_limit_switches();
			cm_set_vector(0, CFG(Y).homing_backoff, 0, 0, 0, 0);
			return(HOMING_BACKOFF_MOVE(vector));
		}
		if (sw_zmin_thrown() == TRUE) {
			sw_clear_limit_switches();
			cm_set_vector(0, 0, CFG(Z).homing_backoff, 0, 0, 0);
			return(HOMING_BACKOFF_MOVE(vector));
		}
		if (sw_amin_thrown() == TRUE) {
			sw_clear_limit_switches();
	...
	if ((CFG(X).homing_enable  == TRUE) && (cm.homing_state == HOMING_X_START)) {
		cm.homing_state = HOMING_X_WAIT;
	...
	if (cm.homing_state == HOMING_X_WAIT) {
		cm.homing_state = HOMING_Y_START;
		sw_clear_limit_switches();
	...
	if ((CFG(Y).homing_enable  == TRUE) && (cm.homing_state == HOMING_Y_START)) {
		cm.homing_state = HOMING_Y_WAIT;
	...
	if (cm.homing_state == HOMING_Y_WAIT) {
		cm.homing_state = HOMING_Z_START;
		sw_clear_limit_switches();
	...
	if ((CFG(Z).homing_enable  == TRUE) && (cm.homing_state == HOMING_Z_START)) {
		cm.homing_state = HOMING_Z_WAIT;
	...
	if (cm.homing_state == HOMING_Z_WAIT) {
		cm.homing_state = HOMING_A_START;
		sw_clear_limit_switches();
	...
	if ((CFG(A).homing_enable  == TRUE) && (cm.homing_state == HOMING_A_START)) {
		cm.homing_state = HOMING_A_WAIT;
	...
	if (cm.homing_state == HOMING_A_WAIT) {
		cm.homing_state = HOMING_RTZ_START;
		sw_clear_limit_switches();
	...
	if (cm.homing_state != HOMING_RTZ_WAIT) {
		cm.homing_state = HOMING_RTZ_WAIT;
