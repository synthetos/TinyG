...
static void _mc_set_mm_position(double target[AXES]) ;
static void _mc_set_mr_position(double target[AXES]);
 ...
 * _mc_set_mm_position()	  - set move final position for traj planning
 * _mc_set_mr_position() 	  - set move/sub-move position for runtime
 *
...
uint8_t mc_set_position(const double x, const double y, const double z, const double a)
{
	...
	mm.position[Z] = z; 
	mm.position[A] = a; 
	_mc_set_mr_position(mm.position);
	return (TG_OK);
}

// set move final position for trajectory planning
inline static void _mc_set_mm_position(double target[AXES]) 
{ 
...
inline static void _mc_set_mr_position(double target[AXES]) 
{ 
...
uint8_t mc_line(double x, double y, double z, double a, double minutes)
{
	...
	if ((m->time = minutes) == 0) {
		mc_unget_write_buffer();		// early exit requires free buffer
	...
	_mc_set_mm_position(m->target);		// set mm position for planning
	return(TG_OK);
}
...
static uint8_t _mc_run_line(struct mcBuffer *m) 
{
	...
	_mc_set_mr_position(m->target);		// set mr position for runtime
	return (TG_OK);
}
...
uint8_t mc_aline(double x, double y, double z, double a, double minutes)
{
	...
	if ((p->move_type == MC_TYPE_ARC) && 
		...
		ritorno(_mc_queue_head());		// queue a head if body_length != 0
...
static uint8_t _mc_queue_line_buffer(double Vi, double Vt, double len, uint8_t type)
{
	...
	if ((m = mc_get_write_buffer()) == NULL) { 	// get buffer or die trying
		return (TG_BUFFER_FULL_FATAL); 
	} 
	...
	return(mc_queue_write_buffer(type));		// queue buffer and return
}
