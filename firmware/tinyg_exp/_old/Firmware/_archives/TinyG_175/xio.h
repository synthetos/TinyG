...
typedef int (*fptr_sig) (uint8_t s);// pointer to a signal handler function 
typedef int (*fptr_line) (char *b); // pointer to a line handler function
...
/* uncomment this if not complied with tinyg.h 
enum tgStatus {
	TG_OK,						// 0 always = general purpose OK return
	TG_DONE,					// 1 always = generator is done
	TG_CONTINUE					// 2 always = continuation in progress (generator, readln)
};
*/
