...
struct GCodeState {
	...
	uint8_t radius_mode;
	uint8_t set_origin_mode;		// 1 = in set origin mode
	...
	double x;						// some temp variables used by arc routines
	double y;
	double h_x2_div_d;
	double theta_start;
	double theta_end;
...
static int _gc_compute_offset_arc(void);
static void gc_print_status(uint8_t status, char *textbuf);
...
void gc_init() {
	...
	gc.x = 0;								// some temps used by arc routines
	gc.y = 0;
	gc.h_x2_div_d = 0;
	gc.theta_start = 0;
	gc.theta_end = 0;
...
/*
 * _gc_gcode_parser() - parse a block (line) of gcode
 */
...
uint8_t gc_gcode_parser(char *block)
{
	...
	gc_print_status(gc.status, block);
	return (gc.status);
}
...
uint8_t gc_execute_block(char *buf) 
{
  ...
  // Perform any physical actions
	 ...
	 * The endpoint is not moved if there has been an interpreter error.
	 */
	if ((gc.status == TG_OK) || (gc.status == TG_DONE) || (gc.status == TG_CONTINUE)) {
...
int _gc_compute_arc()
{
	...
	return (_gc_compute_offset_arc());
}

/*
 * _gc_compute_radius_arc()
 *
 * Compute arc center (offset) from radius. Used to prep for comuting an offset arc.
 */
...
/*  We need to calculate the center of the circle that has the designated 
	...
	*/
	...
	gc.x = gc.target[gc.plane_axis_0]-gc.position[gc.plane_axis_0];
	gc.y = gc.target[gc.plane_axis_1]-gc.position[gc.plane_axis_1];
        
	clear_vector(&gc.offset);
	 // == -(h * 2 / d)
//	gc.h_x2_div_d = -sqrt(4 * gc.radius^2 - (gc.x^2 - gc.y^2)) / hypot(gc.x, gc.y);
	gc.h_x2_div_d = -sqrt(4 * gc.radius*gc.radius - ((gc.x*gc.x) - (gc.y*gc.y))) / hypot(gc.x, gc.y);
...
*/                
	...
	gc.offset[gc.plane_axis_0] = (gc.x-(gc.y*gc.h_x2_div_d))/2;
	gc.offset[gc.plane_axis_1] = (gc.y+(gc.x*gc.h_x2_div_d))/2;
	return (gc.status);
} 
    
/*
 * _gc_compute_offset_arc()
 *
 * Compute the arc move given I and J (arc center point - found in offset vector).
 */

int _gc_compute_offset_arc()
{
	...
	*/
	...
	gc.theta_start = theta(-gc.offset[gc.plane_axis_0], -gc.offset[gc.plane_axis_1]);
	if(isnan(gc.theta_start)) { 
		FAIL(TG_ARC_ERROR); 
		return(gc.status); 
	}

	// calculate the theta (angle) of the target point
	gc.theta_end = theta(gc.target[gc.plane_axis_0] 
	...
	if(isnan(gc.theta_end)) { 
		FAIL(TG_ARC_ERROR); 
		return(gc.status); 
	}

	// ensure that the difference is positive so that we have clockwise travel
	if (gc.theta_end < gc.theta_start) {
		gc.theta_end += 2*M_PI;
	}
	gc.angular_travel = gc.theta_end-gc.theta_start;

	// Invert angular motion if the g-code wanted a counterclockwise arc
	if (gc.motion_mode == MOTION_MODE_CCW_ARC) {
		gc.angular_travel = gc.angular_travel-2*M_PI;
	}

	// Find the radius
	gc.radius_tmp = hypot(gc.offset[gc.plane_axis_0], gc.offset[gc.plane_axis_1]);

	// Calculate the motion along the depth axis of the helix
	gc.depth = gc.target[gc.plane_axis_2]-gc.position[gc.plane_axis_2];

	// Trace the arc
	gc.status = mc_arc_nonblock(gc.theta_start, gc.angular_travel, gc.radius_tmp, gc.depth, 
