...
void tg_controller()
{
	...
	// run line generator
	if (mc_line_continuation() == _DONE) {	
		tg_prompt();		
	}

	// run arc generator
	if (mc_arc_continuation() == _DONE) {	
		tg_prompt();		
	}

	// non-blocking read line from USB (dispatches to tg_parser on completed line)
	if (xio_usb_readln() == GC_STATUS_CONTINUE) {
		tg_prompt();
	}

	// blocking read line from PGM file (dispatches to tg_parser on completed line)
	rtn = xio_pgm_readln();					// read from program memory "file"
	if (rtn == _FDEV_ERR) {					// typically the EOF condition
		tg_set_source(TG_SRC_STDIN);		// return source to stdin
		tg_prompt();
	} else if (rtn == 0) {
		return;
	} else if (rtn != GC_STATUS_CONTINUE) {	// complete command unless continue
		tg_prompt();
	}
}
