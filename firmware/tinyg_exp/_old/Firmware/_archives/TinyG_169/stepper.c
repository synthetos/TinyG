...
struct Line *_st_get_next_line(void);// Return pointer to next line struct
...
int st_execute_line()
{
	...
	if ((ln = _st_get_next_line()) == NULL) {	// NULL is empty buffer condition
		ln_busy = FALSE;
		return (0);
...
/* 
 * _st_get_next_line() - return the next line from the line buffer & advance buffer tail
 */ 

struct Line *_st_get_next_line()
{
