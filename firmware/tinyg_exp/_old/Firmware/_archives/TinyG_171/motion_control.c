...
int mc_line_continuation() 
{
    ...
    st_buffer_move(mc.steps[X_AXIS], mc.steps[Y_AXIS], mc.steps[Z_AXIS], mc.microseconds); 
	mc.line_state = _OFF;			// line is done. turn the generator off.
	return (0);
}
...
{
	if (ma.arc_state == _OFF) {
		return (0);
	} else if (ma.arc_state == _NEW) {
		ma.i=0;
		ma.arc_state = _ACTIVE;
	}
	...
	ma.arc_state = _OFF;			// arc is done. turn the generator off.
	return (0);
}
