...
static void tg_select_file_mode(void);
...
struct tgState {						// main state struct for parsing and other
	...
	int (*src_readln)();				// pointer to source line reader function
...
void tg_init() 
{
	...
	tg_set_source(TG_SRC_USB);			// set initial line source
...
void tg_controller()
{
	...
	xio_pgm_readln();					// read from program memory
}
