...
struct tgState {						// main state struct for parsing and other
	uint8_t status;						// returns from functions (see tinyg.h)
	uint8_t state;						// controller state (tgControllerState)
	uint8_t	filemode;					// TRUE=running commands from file
	uint8_t mode;						// current operating mode (tgMode)
	uint8_t source;						// current input source (tgSource)
	uint8_t src_stdin;					// standard input (tgSource)
	uint8_t src_stdout;					// standard output (tgSource)
	uint8_t src_stderr;					// standard error (tgSource)
...
void _tg_prompt()
{
	if ((tg.state == TG_STATE_READY_UNPROMPTED) && (tg.filemode == FALSE)) {
