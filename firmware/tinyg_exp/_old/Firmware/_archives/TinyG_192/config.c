 ...
 *
 *	  I think this code is modified the original Grbl source beyond recognition 
 *	  to change the copyright notice. If this is in error please inform.
 *
...
int cfg_parse(char *text)
{
	...
	cfg.status = TG_OK;
	switch (text[0]) {
		case '?': cfg_dump(); status (TG_DONE); break;
		case '(': status (TG_DONE);	break;			// ignore comment lines
	...
	if (cfg.status == TG_OK) {
		status(TG_DONE);							// pre-emptive setting
		switch (text[1]) {
