 ...
 *	Punctionation and whitespace is ignored 
...
void cfg_dump()
{
	printf_P(PSTR("***** CONFIGURATION [version %d] ****\r\n"), cfg.config_version);
	printf_P(PSTR("G-code Model Configuration Values ---\r\n"));
	printf_P(PSTR("  mm_per_arc_segment: %4.2f mm / segment\r\n"), cfg.mm_per_arc_segment);
	printf_P(PSTR("  default_seek_rate:  %4.2f mm / second\r\n"), cfg.default_seek_rate);
	printf_P(PSTR("  default_feed_rate:  %4.2f mm / second\r\n"), cfg.default_feed_rate);
...
void _cfg_dump_axis(uint8_t	axis)
{
	...
	printf_P(PSTR("  mm_per_rev:      %7.2f millimeters / revolution\r\n"), CFG(axis).mm_per_rev);
...
/* 
 * cfg_read() - read config data from program ROM into the config struct 
 */
