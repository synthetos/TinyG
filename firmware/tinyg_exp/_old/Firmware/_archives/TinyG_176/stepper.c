...
/*
struct stMoves {
	volatile uint8_t move_busy; 				// TRUE when st_execute_line running 
	volatile uint8_t move_buffer_head = 0;		// persistent index of written lines
	volatile uint8_t move_buffer_tail = 0;		// persistent index of read lines
	struct stMove move_buffer[MOVE_BUFFER_SIZE];// buffer for line instructions
	struct stMove *mv;
};
*/
...
#ifdef __DEBUG
	_st_print_exec_line(*mv, ax.active_axes);
#endif
...
void st_buffer_move(int32_t steps_x, int32_t steps_y, int32_t steps_z, uint32_t microseconds) 
{
	...
	while(move_buffer_tail == next_buffer_head_tmp) {// tail advances, breaking loop
