...
int8_t xio_pgm_control(const uint16_t control, const int16_t arg)
{
	...
	if (control & XIO_SIG_FUNC) {
		fpgm.sig_func = (fptr_sig)arg;
		return (0);	
	}
	if (control & XIO_LINE_FUNC) {
		fpgm.line_func = (fptr_line)arg;
		return (0);
	}
...
int xio_pgm_readln()
{
	if (!(fpgm.pgmbase_P)) {					// return if no file is open
		return (TG_CONTINUE);
	}
	fpgm.sig = XIO_SIG_OK;						// initialize signal
	if (fgets(fpgm.buf, fpgm.len, &dev_pgm) == NULL) {
		printf_P(PSTR("End of file encountered\n"));
		fpgm.pgmbase_P = NULL;
		clearerr(&dev_pgm);
		return (TG_DONE);
	}
	return ((int)fpgm.line_func(fpgm.buf));		// call line handler function
}
