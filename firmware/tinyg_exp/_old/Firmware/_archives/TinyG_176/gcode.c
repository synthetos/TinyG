...
int _gc_arc_move()
{
	...
	*/
	...
	// Trace the arc
	gc.status = mc_arc_nonblock(theta_start, angular_travel, radius, depth, 
...
//			printf_P(PSTR("\n"));
		...
		case TG_QUIT: printf_P(PSTR("\nQuitting Gcode Mode ")); break;
		case TG_CONTINUE: printf_P(PSTR("\nGcode Continuation")); break;
		case TG_BAD_NUMBER_FORMAT: printf_P(PSTR("\nBad Number Format ")); break;
		case TG_EXPECTED_COMMAND_LETTER: printf_P(PSTR("\nExpected Command Letter ")); break;
		case TG_UNSUPPORTED_STATEMENT: printf_P(PSTR("\nUnsupported Statement ")); break;
		case TG_MOTION_CONTROL_ERROR: printf_P(PSTR("\nMotion Control Error ")); break;
		case TG_FLOATING_POINT_ERROR: printf_P(PSTR("\nFloating Point Error ")); break;
	}
	return;
}
