...
#define STEPPER_DELAY _delay_us(STEP_PULSE_MICROSECONDS);
#else
#define STEPPER_DELAY
#endif
 ...
 *		- ports have input and output directions set
...
ISR(X_TIMER_vect)
{
	...
	ax.a[X_AXIS].postscale_counter = ax.a[X_AXIS].postscale_value;// reset postscaler
...
void st_execute_move()
{
	...
	ax.exec_mutex = TRUE;		// set mutual exclusion flag to prevent pre-emption
