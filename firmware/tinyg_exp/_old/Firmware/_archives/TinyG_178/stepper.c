...
void st_execute_move()
{
	...
	(mv.buf->steps_x < 0) ? (ax.x.port->OUTSET = DIRECTION_BIT_bm) : 	// CCW
							(ax.x.port->OUTCLR = DIRECTION_BIT_bm);		// CW
	(mv.buf->steps_y < 0) ? (ax.y.port->OUTSET = DIRECTION_BIT_bm) : 
							(ax.y.port->OUTCLR = DIRECTION_BIT_bm);
	(mv.buf->steps_z < 0) ? (ax.z.port->OUTSET = DIRECTION_BIT_bm) : 
							(ax.z.port->OUTCLR = DIRECTION_BIT_bm);
	// load timers
	ticks = mv.buf->microseconds * TICKS_PER_MICROSECOND;

	if (labs(mv.buf->steps_x) != 0) { 							// X axis
		_st_load_timer(&ax.x, labs(mv.buf->steps_x), (uint32_t)(ticks / labs(mv.buf->steps_x)));
		ax.active_axes |= (X_ACTIVE_BIT_bm);					// set X active
	}
	if (labs(mv.buf->steps_y) != 0) {  							// Y axis
		_st_load_timer(&ax.y, labs(mv.buf->steps_y), (uint32_t)(ticks / labs(mv.buf->steps_y)));
		ax.active_axes |= (Y_ACTIVE_BIT_bm);
	}
	if (labs(mv.buf->steps_z) != 0) { 							// Z axis
		_st_load_timer(&ax.z, labs(mv.buf->steps_z), (uint32_t)(ticks / labs(mv.buf->steps_z)));
		ax.active_axes |= (Z_ACTIVE_BIT_bm);
	}
