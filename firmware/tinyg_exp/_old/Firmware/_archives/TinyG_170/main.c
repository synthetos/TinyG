 ...
 *	Main loop.
 *
 *  The mechanism is if a zero is returned if will drop to the next routine
 *  Any other return value will caase the loop to restart.
 *	This supports generators and continuation routines w/o backing up the queues
 *  So if a 1 is returned the for loop starts back again from the top
 */

	for(;;){
		tg_controller();						// start the controller
//		if (xio_usb_readln()) continue;			// read from USB port
//		if (xio_pgm_readln()) continue;			// read from program memory
	}
}
