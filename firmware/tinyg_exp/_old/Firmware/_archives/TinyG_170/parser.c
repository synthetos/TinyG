...
void tg_init() 
{
	...
	tg_set_source(TG_USB);				// set initial line source
	printf_P(PSTR("TinyG - Version %S\r\n"), (PSTR(TINYG_VERSION)));
	tg_prompt();
}

/*
 * tg_set_source()  Setup line source device
 */
 ...
 *	It accepts a pointer to a command line buffer and executes the correct lower
 *	level interpreter.
 *	It also keeps the system MODE, which to-tade includes:
...
/* 
 * top_signal() - process top-level signals 
 */
