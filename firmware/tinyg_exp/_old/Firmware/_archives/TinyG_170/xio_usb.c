...
#include "xio_usb.h"
#include "parser.h"
#include "xmega_interrupts.h"	// experimental
...
void xio_usb_init(uint16_t control)
{
	...
	f.len = sizeof(f.buf)-1;				// offset to zero

	// bindings
	f.sig_func = &top_signal;				// bind signal handler function pointer
	f.line_func = &top_parser;				// bind line handler function pointer
	...
	f.usart->CTRLA = USARTC0_CTRLA_RXON_TXON;
...
/*
 * xio_usb_queue_RX() - fake ISR to put a char in the RX buffer
 */

void xio_usb_queue_RX(char cin)
{
	// normal path
...
//		tmp = f.usart->CTRLA;
//		tmp &= ~0x03;							// disable interrupts
//		tmp &= ~USART_DREINTLVL_LO_gc;			// disable interrupts
//		f.usart->CTRLA = tmp;
		f.usart->CTRLA = USARTC0_CTRLA_RXON_TXOFF;
...
ISR(USARTC0_TXC_vect)		//ISR(USARTC0_TXC_vect)	// USARTC0 transmission complete
{
	_usb_dequeue_TX();
}
...
void _usb_dequeue_TX()
{
	...
	// You have the busy semaphore, 
	// the TX buffer has data, 
	// and TX DATA reg is not occupied
	// Do the write.
	if (--(f.tx_buf_tail) == 0) {				// advance tail and wrap if needed
		f.tx_buf_tail = TX_BUFFER_SIZE-1;		// -1 avoids off-by-one error (OBOE)
	}
...
//	uint8_t tmp;
	...
	while(next_tx_buf_head == f.tx_buf_tail) { 	// TX buffer full. sleep or return.
		if (BLOCKING(f.flags)) {
//			_usb_dequeue_TX();
			sleep_mode();				// SLEEP fails below 70 char buffer. Why?
...
//	f.usart->CTRLA |= USART_DREINTLVL_LO_gc;	// enable interrupts
//	tmp = f.usart->CTRLA;
//	tmp |= USART_DREINTLVL_LO_gc;
//	f.usart->CTRLA = tmp;
...
int xio_usb_getc(FILE *stream)
{
	...
	return (((fptr_int)(pgm_read_word(&getcFuncs[f.c])))());
 ...
 *	called multiple times without damage. 
...
int xio_usb_readln()
{
	...
	return (((fptr_int)(pgm_read_word(&readlnFuncs[f.c])))()); // dispatch on character
}
...
{
	if (f.i > f.len) {							// prevent buffer overflow
		f.sig = XIO_SIG_EOL;
...
static int _readln_NEWLINE(void)				// handles any valid newline char
{
	f.sig = XIO_SIG_EOL;
	f.buf[f.i] = NUL;
	f.flags &= ~XIO_FLAG_IN_LINE_bm;			// clear in-line state
