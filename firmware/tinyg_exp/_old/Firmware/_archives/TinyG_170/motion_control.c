...
enum genState {						// generator states
		GEN_OFF,					// off - not generating a line or arc
		GEN_NEW,					// first pass through a line or arc (transient state)
		GEN_BUSY					// generator is in process and cannot start a new move
};

#define ARC_OFF -1					// 3 values for controlling arc generation state
#define ARC_NEW 0
#define ARC_ACTIVE 1

#define LINE_OFF -1					// 3 values for controlling arc generation state
#define LINE_NEW 0
...
void mc_line(double x, double y, double z, double feed_rate, int invert_feed_rate)
{
    ...
    st_buffer_move(mc.steps[X_AXIS], mc.steps[Y_AXIS], mc.steps[Z_AXIS], mc.microseconds); 
...
{
	if (mc.line_state == LINE_OFF) {
		return (0);					// return 0 for non-started line
	}
	if (st_buffer_full()) {			// this is where you would block
		return (1);
	}
    st_buffer_move(mc.steps[X_AXIS], mc.steps[Y_AXIS], mc.steps[Z_AXIS], mc.microseconds); 
	mc.line_state = LINE_OFF;		// line is done. turn the generator off.
	return (0);
}
...
int mc_arc_nonblock(double theta, double angular_travel, double radius, 
	double linear_travel, int axis_1, int axis_2, int axis_linear, 
	double feed_rate, int invert_feed_rate)
{
	// load the arc struct
	ma.arc_state = ARC_NEW;		// new arc, NJ.
	ma.theta = theta;
	ma.radius = radius;
	ma.angular_travel = angular_travel;
	ma.linear_travel = linear_travel;
 ...
 *
 * arc_state
 *	mc.arc_state	ARC_OFF	 	no arc is being drawn
 *	mc.arc_state 	ARC_NEW	 	arc is being drawn but not yet started
 *	mc.arc_state 	ARC_ACTIVE	arc is being drawn and is in process
 */
int mc_arc_continuation() 
{
	if (ma.arc_state == ARC_OFF) {
		return (0);
	} else if (ma.arc_state == ARC_NEW) {
		ma.i=0;
		ma.arc_state = ARC_ACTIVE;
	}
	...
	ma.arc_state = ARC_OFF;			// arc is done. turn the generator off.
	return (0);
}
