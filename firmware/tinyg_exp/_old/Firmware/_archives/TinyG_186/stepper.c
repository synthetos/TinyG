...
struct Axes {						// All axes grouped in 1 struct + some extra stuff
	...
	struct mvMove2 *p;				// pointer to dequeued move structure
...
//			st_execute_move();					// ...run next line
			st_execute_move2();					// ...run next line
		}
	}
...
/*
 * st_execute_move2() - dequeue move and load into stepper motors (if possible)
 *
...
void st_execute_move2()
{
	...
	if ((ax.p = mv_dequeue_move_buffer2()) == NULL) {// NULL is empty buffer condition
		ax.exec_busy = FALSE;
		return;
...
void st_execute_move()
{
	...
	((ax.m->steps_x < 0) ^ ax.a[X_AXIS].polarity) ? 
		(ax.a[X_AXIS].port->OUTSET = DIRECTION_BIT_bm):			// CCW
		(ax.a[X_AXIS].port->OUTCLR = DIRECTION_BIT_bm);			// CW

	((ax.m->steps_y < 0) ^ ax.a[Y_AXIS].polarity) ? 
		(ax.a[Y_AXIS].port->OUTSET = DIRECTION_BIT_bm): 
		(ax.a[Y_AXIS].port->OUTCLR = DIRECTION_BIT_bm);

	((ax.m->steps_z < 0) ^ ax.a[Z_AXIS].polarity) ? 
		(ax.a[Z_AXIS].port->OUTSET = DIRECTION_BIT_bm): 
		(ax.a[Z_AXIS].port->OUTCLR = DIRECTION_BIT_bm);

	// load timers
	ticks = ax.m->microseconds * TICKS_PER_MICROSECOND;

	if (labs(ax.m->steps_x) != 0) { 					// X axis
		_st_load_timer(&ax.a[X_AXIS], labs(ax.m->steps_x), 
		   (uint32_t)(ticks / labs(ax.m->steps_x)));
		ax.active_axes |= (X_ACTIVE_BIT_bm);			// set X active
	}
	if (labs(ax.m->steps_y) != 0) {  					// Y axis
		_st_load_timer(&ax.a[Y_AXIS], labs(ax.m->steps_y), 
		   (uint32_t)(ticks / labs(ax.m->steps_y)));
		ax.active_axes |= (Y_ACTIVE_BIT_bm);
	}
	if (labs(ax.m->steps_z) != 0) { 					// Z axis
		_st_load_timer(&ax.a[Z_AXIS], labs(ax.m->steps_z), 
		   (uint32_t)(ticks / labs(ax.m->steps_z)));
		ax.active_axes |= (Z_ACTIVE_BIT_bm);
	}

	// enable the all at the same time (roughly). Better for motor sync.
	if (ax.active_axes || X_ACTIVE_BIT_bm) { ax.a[X_AXIS].timer->CTRLA = TC_CLK_ON; }
	if (ax.active_axes || Y_ACTIVE_BIT_bm) { ax.a[Y_AXIS].timer->CTRLA = TC_CLK_ON; }
	if (ax.active_axes || Z_ACTIVE_BIT_bm) { ax.a[Z_AXIS].timer->CTRLA = TC_CLK_ON; }
