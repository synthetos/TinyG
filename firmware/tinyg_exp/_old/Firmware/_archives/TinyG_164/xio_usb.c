...
//static volatile int tx_busy; 	// semaphore set TRUE when st_execute_line running 
...
//	}
	...
	f.usart->CTRLA = (USART_RXCINTLVL_MED_gc | USART_TXCINTLVL_MED_gc);
...
ISR(USB_TX_ISR_vect)		//ISR(USARTC0_TXC_vect)	// serial port C0 RX interrupt 
{
	xio_usb_dequeue_TX();
/*
	// test if TX data register available (this should ALWAYS be true)
	...
	f.usart->DATA = f.rx_buf[f.rx_buf_tail];	// write char to TX register
*/
}

/*
 * xio_usb_dequeue_TX() - dequeue TX buffer and write to USART
 */

void xio_usb_dequeue_TX()
{
...
int xio_usb_putc_blocking(char c, FILE *stream)
{
	...
	while(next_tx_buf_head == f.tx_buf_tail) { 	// TX buffer full. spin.
		sleep_mode();
	};
