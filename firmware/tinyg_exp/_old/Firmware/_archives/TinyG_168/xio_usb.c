...
void xio_usb_init(uint16_t control)
{
	...
	f.line_func = &top_parser2;					// bind line handler function pointer
...
static int _getc_NUL(void)			// convert NULs to newlines if line mode
{
	if (LINEMODE(f.flags)) {
		c = '\n';
	} 
...
static int _getc_SEMICOLON(void)		// convert ;'s newlines if line mode & semi mode
{
