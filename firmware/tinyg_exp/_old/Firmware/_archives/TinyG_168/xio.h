...
typedef char *(*fptr_char_ptr) (void);// pointer to func w/no args returns char ptr
typedef void (*fptr_void) (void); 	// pointer to func w/no args that returns void
typedef int (*fptr_sig_handler) (uint8_t signal);	// pointer to signal handler func
typedef int (*fptr_line_handler) (void);			// pointer to line handler func
...
#define XIO_FLAG_IN_LINE_bm	(1<<14) 	// partial line is in buffer (now)
...
#define LINE_BUFFER_SIZE 128 // size of read line buffer (one based)
