...
static const uint8_t bsel[] PROGMEM = 		// See xmega_io.h file for these values
...
int16_t _read_usb(struct fdUSART *fd_ptr, char **buf, int count);
static int16_t _write_usb(struct fdUSART *fd_ptr, char *buf, int count);
...
/* io_read() - read one or more characters from device 
	...
	count	Number of characters to read
				...
				1-N		Count mode: Read 1-N chars. Returns error if > than SSIZE_MAX
			   -1		Line mode: Read until line delimiter or NULL
						Returns null-terminated string (NULL overwrites delimiter)
			   -2		Null mode: Read until NULL. Null is written to buffer.
...
int16_t io_read(uint8_t fd, char **buf, int count)
{
	...
	if (fd == FD_USB) {
		fd_ptr = &fd_usb;
		return (_read_usb(fd_ptr, buf, count));
	} else {
		errno = EBADF;
		return (-1);
	}
...
int16_t _read_usb(struct fdUSART *fd_ptr, char **buf, int count)
{
	...
	int8_t mode = count;						// preliminary setting
	...
	while((c = _read_char_USART(fd_ptr)) != -1) { // while characters available
		*buf[i] = c;
		...
		} else if (c == 0) { 					// do both LINE and NULL null cases
			return (i);
...
int16_t _write_usb(struct fdUSART *fd_ptr, char *buf, int count)
{
	char c;										// character temp
	...
	while (TRUE) {
		...
		if ((mode != COUNT_MODE) && (c == 0)) { // trap EOS in NULL & LINE MODES
			return (i);							// don't write it, just return
		}
		if (_write_char_USART(fd_ptr, c) == -1) {
			return -1;							// passes errno through
		}
		if (mode == NULL_MODE) {				// nothing else to do in NULL mode
			continue;
		}
		if (mode == COUNT_MODE) {
			if (--count == 0) {					// count complete
				return (i);
			}
		} else if ((c == '\r') || (c == '\n') || (c == ';')) { // LINE mode
			return (i);
