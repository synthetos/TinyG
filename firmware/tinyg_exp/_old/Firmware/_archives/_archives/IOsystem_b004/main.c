...
//	PMIC_EnableLowLevel();		// nothing at this level
	...
	fd_usb = io_open(DEV_USB, IO_RDWR | IO_BAUD_115200);
	io_write(fd_usb, "USB test started\r\n", NULL_MODE);

	for(;;){
//		sleep_mode();
//		_delay_us(1000);	 		// spin loop
		_delay_ms(50); 	 			// spin loop
		io_read(fd_usb, &rdbuf, LINE_MODE);
//		io_read(fd_usb, rdbuf, 10);
		io_write(fd_usb, rdbuf, NULL_MODE);
