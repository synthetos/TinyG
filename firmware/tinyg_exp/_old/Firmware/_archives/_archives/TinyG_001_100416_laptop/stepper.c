...
int busy; // TRUE when st_execute_line is running. Used to avoid retriggering
...
/* st_buffer_line()
	...
	On entry line_buffer_head indexes next available buffer 
*/
...
//	struct Line *ln;
	...
	// If the buffer is full: good! 
	// That means we are well ahead of the robot. 
	// Nap until there is room in the buffer.
	while(line_buffer_tail == next_buffer_head) { 
		sleep_mode(); 
	};
	// drop the line and exit if zero length line
	if ((steps_x == 0) && (steps_y == 0) && (steps_z) == 0) {
		return;
	};
	ln = &line_buffer[line_buffer_head];  	// Setup line record
	ln->steps_x = steps_x;
	ln->steps_y = steps_y;
	ln->steps_z = steps_z;  
