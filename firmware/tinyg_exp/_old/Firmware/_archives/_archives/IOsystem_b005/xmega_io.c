...
static int16_t _write_usb(struct fdUSART *fd_ptr, const char *buf, int count);
...
/* io_read() - read one or more characters from device 
	...
	buf_ptr	Address of buffer to read into - this will be a RAM (string)
	count	Number of characters to read
			   ...
			   -2		Null mode: Read until NUL. Nul is written to buffer.
...
int16_t _read_usb(struct fdUSART *fd_ptr, char *buf, int count)
{
	...
	int8_t mode = (int8_t)count;				// preliminary set to LINE or NUL
...
int16_t io_write(uint8_t fd, const char *buf, int count)
{
//	struct fdUSART *f ;
...
int16_t _write_usb(struct fdUSART *fd_ptr, const char *buf, int count)
{
	char c;										// character temp
	...
	while (TRUE) {
		...
		if ((mode != COUNT_MODE) && (c == 0)) { // trap EOS in LINE & NUL MODES
