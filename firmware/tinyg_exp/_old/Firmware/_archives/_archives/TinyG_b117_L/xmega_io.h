...
unsigned char xio_putc(uint8_t fd, unsigned char c);
unsigned char xio_getc(uint8_t fd);					// see stdio.h to get this right
...
// compatibility with wiring_serial.c (see also xmega_io.c section)
#define serialRead() xio_getc(FD_USB)
#define printByte(c) xio_putc(FD_CONSOLE,c)
#define printString(b) xio_write(FD_CONSOLE,b,STR_MODE)
#define printPgmString(b) xio_write(FD_CONSOLE,b,PSTR_MODE)
