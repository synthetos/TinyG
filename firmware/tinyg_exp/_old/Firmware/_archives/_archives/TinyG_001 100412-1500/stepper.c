...
//	st_print_four_ints(((step_rate_x >> 8) & 0x000000FF), (step_rate_x & 0x000000FF), 0, 0);
	...
	/*
	if (step_rate_x < (DIV1_RANGE)) {			// short timer - up to 2000 uSec
		ax.x.timer->CTRLA = TC_CLK_DIV_1;					// set clock divisor
		ax.x.counter = (ln->microseconds/step_rate_x);	// # of steps to make at this rate
		step_rate_x = (step_rate_x * 32);
		ax.x.timer->PERH = (uint8_t)((step_rate_x >> 8) & 0x000000FF);	// period high
		ax.x.timer->PERL = (uint8_t)(step_rate_x & 0x000000FF);			// period low
	}
	if (step_rate_y < (2048)) {			// short timer - up to 2000 uSec
		ax.y.timer->CTRLA = TC_CLK_DIV_1;					// set clock divisor
		ax.y.counter = (ln->microseconds/step_rate_y);
		step_rate_y = (step_rate_y * 32);
		ax.y.timer->PERH = (uint8_t)((step_rate_y >> 8) & 0x000000FF);
		ax.y.timer->PERL = (uint8_t)(step_rate_y & 0x000000FF);	
	}
	if (step_rate_z < (2048)) {			// short timer - up to 2000 uSec
		ax.z.timer->CTRLA = TC_CLK_DIV_1;
		ax.z.counter = (ln->microseconds/step_rate_z);
		step_rate_z = (step_rate_z * 32);
		ax.z.timer->PERH = (uint8_t)((step_rate_z >> 8) & 0x000000FF);
		ax.z.timer->PERL = (uint8_t)(step_rate_z & 0x000000FF);
	}
	*/
}
