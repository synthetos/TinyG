...
struct xioUSART f;				// create a local USART control struct
...
/*
	...
	if (LINEMODE(f.flags) && SEMICOLONS(f.flags)) {
		if((c == '\r') || (c == '\n') || (c == ';')) {
			if (f.flags & XIO_FLAG_GOT_EOL_bm) {
				return xio_usb_getc(stream); 	// drop EOL & get next char
			} else {
				f.flags |= XIO_FLAG_GOT_EOL_bm;	// set the EOL flag
				return ('\n');
			}
		} else {
			f.flags &= ~XIO_FLAG_GOT_EOL_bm;	// clear the EOL flag
			return (c);
		}
	}
