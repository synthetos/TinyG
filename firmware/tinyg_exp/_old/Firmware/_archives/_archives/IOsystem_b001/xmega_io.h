...
#define RX_BUFSIZE 32				// general buffer sizes
...
struct fdUSART {					// file descriptor struct for serial IO
	...
	uint_fast8_t volatile rx_buf_head; // RX buffer write index (changes via ISR)

	uint_fast8_t tx_buf_max;		// TX buffer maximum count
	uint_fast8_t volatile tx_buf_tail; // TX buffer read index (changes via ISR)
	uint_fast8_t tx_buf_head;		// TX buffer write index

	char rx_buf[RX_BUFSIZE];
	char tx_buf[TX_BUFSIZE];
...
#define TO_LINE -1					// read and write modes
#define TO_NULL -2
