...
---- Read/Write Modes ----
  ...
  There are four modes for read and write:
	...
	SIZE_MODE	Reads or writes exactly SIZE characters before returning. 
				NULs are ignored - i.e. nul characters in strings are passed through
	...
	LINE_MODE	Reads until a delimiter is read from the device (ex. \n, \r, ;)
				...
				The first delimiter is written to the device, 
				  but no terminating nul is written to the device.
	...
	STR_MODE	Reads until a nul is read from the device.
				...
				The nul is not written to the device
	...
	PSTR_MODE	(This mode is not valid for read)
				...
				The nul is not written to the output (the device)
  ...
  Character level functions are also provided (not yet implemented):
...
unsigned char _read_char_USART(struct fdUSART *fd_ptr);
unsigned char _write_char_USART(struct fdUSART *fd_ptr, char c);
...
/* xio_read() - read one or more characters from device 
	...
	size	Number of characters to read (See Read/Write Modes for more details)
			  ...
			  1-N	SIZE_MODE: Read 1-N chars. Returns error if N > than SSIZE_MAX
...
/* xio_write() - write one or more characters to device
	...
	size	Number of characters to write (See Read/Write Modes for more details)
			  0		Returns zero and no other results
			  1-N	SIZE_MODE: Write 1-N chars. Returns error if N > than SSIZE_MAX
	...
	returns   1-N	Number of characters written
			 ...
			 EAGAIN	Non-blocking I/O has been selected using RDNONBLOCK and
					 no data was immediately available for reading.
			 EINVAL	Invalid argument - some negative number other than -1 or -2
			 EFBIG  String exceeded maximum size
*/
...
unsigned char xio_getc(uint8_t fd)
{
	struct fdUSART *f;
	f = fd_ptrs[fd];

	switch (fd) {
		case (FD_USB): return(_read_char_USART(f));
		default: { errno = EBADF; return ERR_EOF; }
	}
}

unsigned char xio_putc(uint8_t fd, unsigned char const c)
{
	struct fdUSART *f;
	f = fd_ptrs[fd];

	switch (fd) {
		case (FD_USB): return(_write_char_USART(f,c));
		default: { errno = EBADF; return ERR_EOF; }
	}
}
...
int16_t _read_usb(struct fdUSART *f, char *buf, int size)
{
	...
	if (size > SSIZE_MAX) { errno = EFBIG; return (-1); }	// too big a request made
	...
	while (TRUE) {
		...
		if ((i) >= SSIZE_MAX) {					// use i to look for SIZE error
			errno = EFBIG;
			return (-1);
		}
		if (mode == SIZE_MODE) {				// in size mode look for next char
			continue;
		}
		if (c == 0) {							// non-SIZE_MODE end condition
			return (i);
		}
		if (mode == LINE_MODE) { 				// line delimiters are end conditions
			if ((c == '\r') || (c == '\n') || (c == ';')) { 
				buf[i] = NUL;
				return (i);
			}
...
unsigned char _read_char_USART(struct fdUSART *f)
{
...
int16_t _write_usb(struct fdUSART *f, const char *buf, int size)
{
	char c;										// character temp
	...
	while (TRUE) {
		if (--size == 0) {						// first test if size is complete
			return (i);
		}
		if ((i+1) >= SSIZE_MAX) {				// use i to look for SIZE error
			errno = EFBIG;
			return (-1);
		}
		if (mode == PSTR_MODE) {				// get input char from buffer type
			c = pgm_read_byte(&buf[i]);
		} else {
			c = buf[i];
		}
		i += 1;
		if ((mode != SIZE_MODE) && (c == 0)) {	// non-SIZE_MODE end condition
...
unsigned char _write_char_USART(struct fdUSART *f, char c)
{
