...
void mc_line(double x, double y, double z, double feed_rate, int invert_feed_rate)
{
    ...
    st_buffer_line(mc.steps[X_AXIS], mc.steps[Y_AXIS], mc.steps[Z_AXIS], mc.microseconds); 
...
void mc_line_nonblock(double x, double y, double z, double feed_rate, int invert_feed_rate)
{
	mc.target[X_AXIS] = lround(x*CFG(X_AXIS).steps_per_mm);
	mc.target[Y_AXIS] = lround(y*CFG(Y_AXIS).steps_per_mm);
	mc.target[Z_AXIS] = lround(z*CFG(Z_AXIS).steps_per_mm); 
...
void mc_arc_nonblock(double theta, double angular_travel, double radius, 
