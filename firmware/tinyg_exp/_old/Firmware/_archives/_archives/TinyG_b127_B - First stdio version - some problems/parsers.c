...
*/
#include "xmega_support.h"		// should be first for F_CPU to have an effect
...
void tg_init() 
{
	...
	printf_P(PSTR(TINYG_VERSION));
	printf_P(PSTR("\r\n"));

//	writepstr(FD_CON,PSTR("\r\nTinyG [TEST MODE] - Version "));
//	writepstr(FD_CON,PSTR(TINYG_VERSION));
//	writepstr(FD_CON,PSTR("\r\n"));
...
//	int i = 0;					// does not work if i is defined here. Don't know why.	
	...
	while((c = serialRead()) != -1) {			// 0x04 is ASCII ETX
		if((c == '\r') || (c == '\n') || (c == ';')) {  // line complete
