...
struct ParserState {
	...
	uint8_t inches_mode;         	//0 = millimeter mode {G21}, 1 = inches mode {G21}
...
int _read_double(char *line,	//  <- string: line of RS274/NGC code being processed
                int *counter,	//  <- pointer to a counter for position on the line 
                double *double_ptr); //  <- pointer to double to be read

int _next_statement(char *letter, double *double_ptr, char *line, int *counter);
...
void gc_init() {
	...
	gc_select_plane(X_AXIS, Y_AXIS, Z_AXIS);
	gc.absolute_mode = TRUE;
}


/* gc_select_plane() - select axis plane */

void gc_select_plane(uint8_t axis_0, uint8_t axis_1, uint8_t axis_2) 
{
...
inline float _to_millimeters(double value) 
{
...
*/
double _theta(double x, double y)
{
...
uint8_t gc_execute_line(char *textline) {
  ...
  // First: parse all statements
	...
	if (textline[0] == '/') { 	// ignore block delete
  	...
  	if (textline[0] == '$') { 	// This is a parameter line intended to change 
	    ...
	    _read_double(textline, &counter, &p);

    	if(textline[counter++] != '=') { 
			return(GCSTATUS_UNSUPPORTED_STATEMENT); 
		}
    	_read_double(textline, &counter, &value);
  ...
  // Pass 1: Commands
	while(_next_statement(&letter, &value, textline, &counter)) {
    	...
    	switch(letter) {
			case 'G':
				switch(int_value) {
					...
					case 17: gc_select_plane(X_AXIS, Y_AXIS, Z_AXIS); break;
					case 18: gc_select_plane(X_AXIS, Z_AXIS, Y_AXIS); break;
					case 19: gc_select_plane(Y_AXIS, Z_AXIS, X_AXIS); break;
  ...
  // Pass 2: Parameters
	while(_next_statement(&letter, &value, textline, &counter)) {
		int_value = trunc(value);
		unit_converted_value = _to_millimeters(value);
		switch(letter) {
			case 'F': 
  ...
  // Perform any physical actions
	switch (next_action) {
      ...
      */
      ...
      double theta_start = _theta(-offset[gc.plane_axis_0], -offset[gc.plane_axis_1]);
		
      // calculate the theta (angle) of the target point
      double theta_end = _theta(target[gc.plane_axis_0] - offset[gc.plane_axis_0] - gc.position[gc.plane_axis_0], 
...
/* _next_statement() - parse next block of Gcode
...
*/
int _next_statement(char *letter, double *double_ptr, char *textline, int *counter) 
{
	if (textline[*counter] == 0) {
	...
	*letter = textline[*counter];
	...
	(*counter)++;
	if (!_read_double(textline, counter, double_ptr)) {
		return(0);
	};
	return(1);
}

/* _read_double() - read a double from a Gcode statement */

int _read_double(char *textline, //!< string: line of RS274/NGC code being processed
                int *counter,    //!< pointer to a counter for position on the line
                double *double_ptr) //!< pointer to double to be read
{
