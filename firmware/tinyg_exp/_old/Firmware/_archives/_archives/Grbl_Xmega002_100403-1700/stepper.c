...
#ifdef __AVR_ATmega328P__		// ***** Need to fix this HW dependency, somehow. Sigh
...
void st_init()
{
	...
	X_TIMER.CTRLA = TC_CLK_DIV_8;				// detault division ratio
	...
	Y_TIMER.CTRLA = TC_CLK_DIV_8;				// detault division ratio
	...
	Z_TIMER.CTRLA = TC_CLK_DIV_8;				// detault division ratio
	...
	A_TIMER.CTRLA = TC_CLK_DIV_8;				// detault division ratio
	...
	// interrupts must be enabled in main()
}
...
/* config_step_timer - configure the prescaler and ceiling of timer 1 to produce the given rate as accurately as possible. */
...
/********
	...
	} else {	  						// Okay, that was slower than we actually go. 
		ceiling = 0xffff;				//...Just set the slowest speed
    	prescaler = 4;
	}
	// Set prescaler
