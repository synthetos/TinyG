...
static char *gc_normalize_gcode_block(char *block);
static int read_double(char *buf, int *i, double *double_ptr);
static int next_statement(char *letter, double *double_ptr, char *line, int *i);
static int gc_arc_move(void);
...
void gc_gcode_parser(char *block)
{
	uint8_t status;

	gc_normalize_gcode_block(block);
	printf_P(PSTR("\r\nREAD GCODE BLOCK: "));
	printf(block);
	printf_P(PSTR("\r\n"));
...
/*
 * gc_normalize_gcode_block() - normalize a block (line) of gcode in place
 */

char *gc_normalize_gcode_block(char *block) {
...
int next_statement(char *letter, double *double_ptr, char *buf, int *i) {
	if (buf[*i] == 0) {
	...
	*letter = buf[*i];
	...
	(*i)++;
	if (!read_double(buf, i, double_ptr)) {
		return(0);
	};
	return(1);
}
...
int read_double(char *buf, int *i, double *double_ptr) 
{
	char *start = buf + *i;
	char *end;
...
uint8_t gc_execute_line(char *buf) 
{
  ...
  // First: parse all statements
	...
	if (buf[0] == 'Q') { 	// quit
		return(GCSTATUS_QUIT);
	} 
	if (buf[0] == '/') { 	// ignore block delete ++ Shouldn't we skip this block?
		i++; 
	} 
  // Pass 1: Commands
	while(next_statement(&gc.letter, &gc.value, buf, &i)) {
    	switch(gc.letter) {
			case 'G':
				switch((int)gc.value) {
...
{
	switch(status_code) {
		case GCSTATUS_OK: printf_P(PSTR("\r\nRunning ")); break;
		case GCSTATUS_QUIT: printf_P(PSTR("\r\nQuitting Gcode Mode ")); break;
		case GCSTATUS_BAD_NUMBER_FORMAT: printf_P(PSTR("\r\nBad Number Format ")); break;
		case GCSTATUS_EXPECTED_COMMAND_LETTER: printf_P(PSTR("\r\nExpected Command Letter ")); break;
		case GCSTATUS_UNSUPPORTED_STATEMENT: printf_P(PSTR("\r\nUnsupported Statement ")); break;
		case GCSTATUS_MOTION_CONTROL_ERROR: printf_P(PSTR("\r\nMotion Control Error ")); break;
		case GCSTATUS_FLOATING_POINT_ERROR: printf_P(PSTR("\r\nFloating Point Error ")); break;
	}
	printf(textbuf);
	printf_P(PSTR("\r\n"));
	printf_P(PSTR("\r\n"));
