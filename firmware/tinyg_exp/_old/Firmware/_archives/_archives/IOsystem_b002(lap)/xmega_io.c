...
static int16_t _read_usb(struct fdUSART *fd_ptr, char *buf, int count);
...
int16_t _write_usb(struct fdUSART *fd_ptr, char *buf, int count)
{
	...
	while (TRUE) {
		...
		if ((mode != COUNT_MODE) && (c == 0)) { // in ASCII modes don't write EOS
			return (i);
		}
		if (_write_char_USART(fd_ptr, c) == -1) {
			return -1;							// passes errno through
		}
		if ((mode == COUNT_MODE) && (--count == 0)) {	// count complete
			return (i);
		} else if ((c == '\r') || (c == '\n') || (c == ';')) { // LINE & NULL modes 
			return (i);
		}
