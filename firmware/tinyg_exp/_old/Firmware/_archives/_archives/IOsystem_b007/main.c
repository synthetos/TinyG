...
//	PMIC_EnableLowLevel();		// nothing at this level
	...
	fdusb = io_open(DEV_USB, IO_RDWR | IO_ECHO | IO_BAUD_115200);
	io_write(fdusb, "USB test started\r\n", NUL_MODE);

	for(;;){
//		sleep_mode();
		io_read(fdusb, rdbuf, LINE_MODE);
		io_write(fdusb, "USB test line\r\n", NUL_MODE);
		io_write(fdusb, rdbuf, NUL_MODE);
