...
{
	char c;
	uint8_t i = 0;									// character string index
...
/* run_gcode_from_memory()

  Provisional for now. Will overflow it's RAM buffer if you rae not careful
  Need to rewrite this to read one block at a time from PROGMEM to RAM

*/

void run_gcode_from_memory()
{
	int i = 0;					// make this an into to allow for large canned progs
	int j = 0;					// output buffer index (textline)
	char c;

	while ((c = pgm_read_byte(&block_P[i++])) != 0) {
		if ((c == '\r') || (c == '\n') || (c == ';') || (c == '(')) {  // Line complete. Execute!
			textline[j] = 0;							// terminate the string
			gc_print_status(gc_execute_line(textline));	// execute cmd & show status
			j = 0;			
...
void run_gcode_from_memory2()
{
	char block[256];						// RAM input buffer
	char c;
	int i = 0;									// input buffer index (block)
	int j = 0;									// output buffer index (textline)

	strcpy_P(block, block_P);				// copy FLASH string to RAM
	
	while((c = block[i++]) != 0) {
		if ((c == '\r') || (c == '\n') || (c == ';') || (c == '(')) {  // Line complete. Execute!
			textline[j] = 0;						// terminate the string
			gc_print_status(gc_execute_line(textline));	// execute command and display status
			j = 0;			
		} else if (c <= ' ') { 						// toss whitespace & ctrl chars
		} else if (c == '(' && j == 0) {		// toss comment line
		} else if (c >= 'a' && c <= 'z') {			// convert lower to upper
			textline[j++] = c-'a'+'A';
		} else {
			textline[j++] = c;						// put number or '.' into line
		}
	}	
}
