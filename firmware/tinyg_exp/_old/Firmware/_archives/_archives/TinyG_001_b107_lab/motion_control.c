...
/* TinyG Notes:
   ...
   Moved a lot of stuff into structs becuase the AVR simulator works better that way
*/
...
{
	mc.target[X_AXIS] = lround(x*settings.steps_per_mm[0]);
	mc.target[Y_AXIS] = lround(y*settings.steps_per_mm[1]);
	mc.target[Z_AXIS] = lround(z*settings.steps_per_mm[2]); 
	...
	if (invert_feed_rate) {
    	st_buffer_line(mc.steps[X_AXIS], mc.steps[Y_AXIS], mc.steps[Z_AXIS], 
 		...
 		mc.millimeters_of_travel = sqrt(square(mc.steps[X_AXIS]/settings.steps_per_mm[0]) + 
									    square(mc.steps[Y_AXIS]/settings.steps_per_mm[1]) + 
									    square(mc.steps[Z_AXIS]/settings.steps_per_mm[2]));
		st_buffer_line(mc.steps[X_AXIS], mc.steps[Y_AXIS], mc.steps[Z_AXIS], 
