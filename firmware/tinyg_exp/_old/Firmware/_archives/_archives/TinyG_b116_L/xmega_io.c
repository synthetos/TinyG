...
/* interrupt routines 
  ...
  See module header note on circular buffers
...
/* USB_RX_ISR - USB receiver interrupt (RX)
   ...
   ORIGINAL CODE for this routine was:
...
ISR(USB_RX_ISR_vect)		//ISR(USARTC0_RXC_vect)	// serial port C0 RX interrupt 
{
	...
	// ought to active flow control here
}
...
int _read_usb(struct fdUSART *f, char *buf, int count)
{
	...
	int	i;				// output buffer index (buf[i])
	uint8_t mode = 0;	// 0 = count mode, 1 = line mode, 2 = null termination mode

	if (count == 0) {								// special case of 0 count
		return (0);
	} else if (count == -1) {						// line termination mode
		mode = 1; 
	} else if (count == -2) {						// NULL termination mode
		mode = 2;
