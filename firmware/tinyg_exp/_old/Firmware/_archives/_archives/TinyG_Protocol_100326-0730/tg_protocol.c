...
//(PGM_VOID_P) (prStateTable[MAX_STATES][MAX_EVENTS])(void) __ATTR_CONST__ PROGMEM = {
	...
	{	
		...
		prAnyLoopDiscard,		// gotDigit	- prAnyLoopDiscard action for gotDigit event
...
void prAnyTrappedError(void)	// exit for error that can be NAK'd ("F"))
{
  prState = listen;
  // send a NAK if in ACK mode
}
...
void prRxNextTalkerSpaceExit(void)	// exit rxFromNode to endFromNode
{
  prState = rxPayload;
  // atoi from node in 
}
