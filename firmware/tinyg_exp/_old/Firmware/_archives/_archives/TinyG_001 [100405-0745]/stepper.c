...
void st_init()
{
 /* initialize X axis */
	...
	ax.x.timer->PERH = 0x80;					// period high
	ax.x.timer->PERL = 0xFF;					// period low
 ...
 /* initialize Y axis */
	...
	ax.y.port->OUT |= DIRECTION_BIT_bm;			// set initial direction
	...
	ax.y.timer->PERH = 0xC0;					// period high
	ax.y.timer->PERL = 0xFF;					// period low
...
/* initialize Z axis */
	...
	ax.z.timer->PERH = 0x80;					// period high
	ax.z.timer->PERL = 0xFF;					// period low
...
/* initialize A axis */
	...
	ax.a.timer = &A_TIMER;
	ax.a.timer->CTRLA = TC_CLK_DIV_2;			// default division ratio
	ax.a.timer->CTRLB = TC_WGMODE;				// waveform generation mode
	ax.a.timer->INTCTRLA = TC_OVFINTLVL;		// interrupt mode
	ax.a.timer->PERH = 0x80;					// period high
	ax.a.timer->PERL = 0xFF;					// period low
