/*
  ...
  This IO subsystem looks like Unix IO but there are sigificant differences.
  	- It's Kabuki Theater: malloc is not used so everything is pre-allocated.
	- Extends basic IO system to handle lines and strings:
		- SIZE mode: read/write N characters (works like UNIX read() & write())
		- LINE mode: read/write to line delimiter (e.g. \r, \n, ';')
		- NUL mode:  read/write to end-of-string (zero)
	- io_control() is not ioctl(). Do the same things but have different interfaces
...
int16_t io_read(uint8_t fd, char *buf, int size)
{
...
int16_t io_write(uint8_t fd, const char *buf, int size)
{
//	struct fdUSART *f ;
