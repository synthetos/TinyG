...
extern struct fdXIO fd_usb;		// needed for USB_RX_ISR
...
ISR(USB_RX_ISR_vect)		//ISR(USARTC0_RXC_vect)	// serial port C0 RX interrupt 
{
	...
	if ((--fd_usb.rx_buf_head) == 0) { 				// wrap condition
		fd_usb.rx_buf_head = RX_BUFFER_DEFAULT_SIZE-1;// -1 avoids the off-by-one error
	}
	...
	if ((++fd_usb.rx_buf_head) > RX_BUFFER_DEFAULT_SIZE-1) { // reset the head
...
{
	struct fdXIO *f;						// USART struct used by USB port
	uint8_t fd;								// temp for file descriptor

	if (dev != DEV_USB) {					// assertion code
...
//	f->port->OUTCLR = USB_RTS_bm; 			// set RTS HI initially (RTS enabled)
	...
	f->xio_getc = (*xio_getc_usb);
	f->xio_putc = (*xio_putc_usb);
...
int8_t xio_close_usb(struct fdXIO *f)
{
	return (0);
}
...
int8_t xio_control_usb(struct fdXIO *f, uint32_t control, int16_t arg)
{
...
char xio_getc_usb(struct fdXIO *f)
{
...
char xio_putc_usb(struct fdXIO *f, const char c)
{
