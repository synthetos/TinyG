...
{
//	uint16_t tmp;
//	static char (*fptr)();	 					//declare the function pointer 
...
//	fptr func;
	...
	c = f.rx_buf[f.rx_buf_tail];				// get character from buffer
	c &= 0x007F;								// mask errant MSBs before lookup

//	return (getcFuncs[(int)c]());				// call action procedure (from RAM)

	((fptr_char)(pgm_read_word(&getcFuncs[(int)c])))();	// call action procedure
//	func = (fptr)(pgm_read_word(&getcFuncs[(int)c]));	// call action procedure
//	func();
//	(fun= pgm_read_word(&do_func[func_no]))(); //func_no is the 
	return (c);

//	if (getcFuncs[(int)c] == NULL) {			// call action procedure
//		c = c;
//	}
