...
// derived from read and write, above
//unsigned char getc(uint8_t fd, unsigned char c); // see stdio.h to get this right
//unsigned char putc(uint8_t fd, unsigned char c);

// macros to redefine names to the UNIX IO names
...
// macros to define line, string and char versions
#define readln(fd,b) xio_read(fd,b,LINE_MODE)
#define writeln(fd,b) xio_write(fd,b,LINE_MODE)
#define readstr(fd,b) xio_read(fd,b,NUL_MODE)
#define writestr(fd,b) xio_write(fd,b,NUL_MODE)
#define readchar(fd,b) xio_read(fd,b,1)		// note: semantics are dif from getc()
#define writechar(fd,b) xio_write(fd,b,1)	// note: semantics are dif from putc()

/*
 * Major IO subsystem configs, constants, and device structures 
 */

#define USART_RX_BUFSIZE 32			// rx buffer - written by ISR (2 bytes unusable)
#define USART_TX_BUFSIZE 1			// tx buffer - (not used)
...
struct fdUSART {					// file descriptor struct for serial IO
	...
	volatile unsigned char rx_buf[USART_RX_BUFSIZE];
...
// fs.flags flags (not the similar bits in the control word)
