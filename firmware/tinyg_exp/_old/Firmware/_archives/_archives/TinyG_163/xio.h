...
// declare fptr_TYPE as pointer to a function that takes no args and returns TYPE
typedef int (*fptr_int) (void); 
typedef char (*fptr_char) (void); 
...
#define XIO__OK 0				// OK
#define XIO__EOF 1				// end-of-file enountered
#define XIO__WOULDBLOCK	2		// would block - no character returned
#define XIO__KILL 3				// cancel operation immediately (^c, ESC)
#define XIO__TERMINATE 4		// terminate operation nicely (^x)
#define XIO__PAUSE 5			// pause operation (^q)
#define XIO__RESUME 6			// resume operation (^p)
#define XIO__SHIFTOUT 8			// shift to mode (^n)
#define XIO__SHIFTIN 9			// shift back (^o)
#define XIO__DELETE 7			// backspace or delete character (BS, DEL)
#define XIO__BELL 8				// BELL character (BEL, ^g)
...
FILE *stddev;						// a convenience corresponding to stdin. out, err
