/*
  tg_parsers.c - TinyG top level parser
...
void tg_prompt()
{
	if (tg.srcin == stdin) {
//		printf_P(PSTR("TinyG ["));
//		printf_P((PGM_P)pgm_read_word(&tgModeStrings[tg.mode]));
//		printf_P(PSTR("]>> "));
...
void top_parser()
{
	...
	if (fgets(tg.buf, BUF_LEN-1, tg.srcin) == NULL) {	
		...
		switch (tg.signal) {
			...
			case XIO__WOULDBLOCK: {} break;
			case XIO__KILL: { st_kill(); } break;
			case XIO__TERMINATE: {} break;
			case XIO__PAUSE: {} break;
			case XIO__RESUME: {} break;
			case XIO__SHIFTOUT: {} break;
			case XIO__SHIFTIN: {} break;
			default: break;
		}
		clearerr(tg.srcin);
		return;
