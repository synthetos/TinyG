...
static int _got_PAUSE(void)
{
	f.signals = XIO__PAUSE;
	return(_FDEV_ERR);
}

static int _got_RESUME(void)
{
	f.signals = XIO__RESUME;
	return(_FDEV_ERR);
}
...
static int _got_DELETE(void)
{
	f.signals = XIO__DELETE;
	return(_FDEV_ERR);
}

static int _got_SHIFTOUT(void)
{
	f.signals = XIO__SHIFTOUT;
	return(_FDEV_ERR);
}

static int _got_SHIFTIN(void)
{
	f.signals = XIO__SHIFTIN;
	return(_FDEV_ERR);
}


/*
 *  xio_usb_getc() - blocking char reader for USB device
 *
...
/*
 *  xio_usb_getc_nonblock() - non-blocking char reader for USB device
 */

int xio_usb_getc_nonblock(FILE *stream)
{
	// figure out heads and tails
	if (f.rx_buf_head == f.rx_buf_tail) {		// buffer empty
		f.signals = XIO__WOULDBLOCK;
		return (_FDEV_ERR);
	}
	if (--(f.rx_buf_tail) == 0) {				// decrement and wrap if needed
		f.rx_buf_tail = RX_BUFFER_SIZE-1;		// -1 avoids off-by-one error (OBOE)
