...
/* TinyG Notes:
   ...
   Moved stuff into global structs because the AVR simulator works better that way
*/
...
{
	mc.target[X_AXIS] = lround(x*settings.steps_per_mm[X_AXIS]);
	mc.target[Y_AXIS] = lround(y*settings.steps_per_mm[Y_AXIS]);
	mc.target[Z_AXIS] = lround(z*settings.steps_per_mm[Z_AXIS]); 
	...
	if (invert_feed_rate) {
    	st_buffer_line(mc.steps[X_AXIS], mc.steps[Y_AXIS], mc.steps[Z_AXIS], 
 		...
 		mc.millimeters_of_travel = sqrt(square(mc.steps[X_AXIS]/settings.steps_per_mm[X_AXIS]) + 
									    square(mc.steps[Y_AXIS]/settings.steps_per_mm[Y_AXIS]) + 
									    square(mc.steps[Z_AXIS]/settings.steps_per_mm[Z_AXIS]));
		st_buffer_line(mc.steps[X_AXIS], mc.steps[Y_AXIS], mc.steps[Z_AXIS], 
  ...
  int axis_linear, double feed_rate, int invert_feed_rate)
{
	...
	mc.segments = ceil(mc.millimeters_of_travel/settings.mm_per_arc_segment);
	...
	mc.center_x = (mc.position[axis_1]/settings.steps_per_mm[axis_1])-sin(mc.theta)*mc.radius;
	mc.center_y = (mc.position[axis_2]/settings.steps_per_mm[axis_2])-cos(mc.theta)*mc.radius;
