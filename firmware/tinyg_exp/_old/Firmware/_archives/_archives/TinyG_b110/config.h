...
/*
  ...
  This file has been largely re-written.from the Grbl original
  ...
  ---- Settings and Config ----
	...
	Grbl takes machine settings from the Settings struct (settings)
	TinyG works form the Config struct (cfg)

	Both are currently defined and have some overlapping values
	Currently migrating from settings to cfg - Will remove settings when migration is complete
...
#define TINYG_VERSION "0.02"	// make this the TinyG version (not Grbl version)
#define real FALSE				// set TRUE if running real HW, FALSE or undefined if simulation
...
void config_parse(char *config_string);  // parse a tag=value config string
...
/* Version of the EEPROM data. 
	...
	GrblX changed version from 1 to 100.
*/
