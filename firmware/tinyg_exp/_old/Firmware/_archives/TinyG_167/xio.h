...
typedef int (*fptr_int) (void); 	// pointer to func w/no args that returns int
typedef char (*fptr_char) (void); 	// pointer to func w/no args that returns char
typedef void (*fptr_void) (void); 	// pointer to func w/no args that returns void
typedef void (*fptr_sig_handler) (uint8_t signal);	// pointer to signal handler func
typedef void (*fptr_line_handler) (void);			// pointer to line handler func

/*
 * XIO signals and error conditions (loadsf.signals register)
 */

enum xio_SIGS {
		XIO_SIG_OK,				// OK
		XIO_SIG_EOF,			// end-of-file enountered
...
#define TX_BUFFER_SIZE 130	// device tx buffer - read by ISRs (2 bytes unusable)
#define LINE_BUFFER_SIZE 128	// size of read line buffer
...
struct xioUSART {
	...
	uint8_t len;						// line buffer maximum length
	fptr_sig_handler (*sh)(uint8_t sig);// pointer to signal handler function
	fptr_line_handler (*lh)();			// pointer to line handler function
	char line_buf[LINE_BUFFER_SIZE];	// input lines are assembled here

	// PRIVATE VARIABLES - can be different by device type
	...
	volatile uint8_t tx_busy;			// TX dequeue routine semaphore
...
FILE *stddev;							// a convenience for stdin. stdout, stderr

/**** FURTHER NOTES ****

  Topics related to xio device system
