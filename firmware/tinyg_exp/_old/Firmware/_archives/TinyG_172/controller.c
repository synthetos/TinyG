...
struct tgState {						// main state struct for parsing and other
	...
	uint8_t stdID;						// enum of standard input / output
...
void tg_init() 
{
	...
	tg.stdID = TG_SRC_USB;				// hard-wire USB to stdio
...
{
	tg.source = src;
	xio_usb_control(XIO_LINE_FUNC, (int)&xio_null_line);	// bind null handlers
...
void tg_controller()
{
	st_execute_move();					// kick start steppers - 
										// should ordinarily run by themselves
	mc_line_continuation();				// run move generators
	mc_arc_continuation();

	// attempt to get a line - srcin dispatches to tg_parser on completed line
	xio_usb_readln();		 				// read from USB port

	if (xio_pgm_readln() == _FDEV_ERR) {	// read from program memory "file"
		tg_set_source(TG_SRC_USB);
	}
}
 ...
 *	aAccepts a pointer to a command line buffer and executes the correct lower
 *	level interpreter.
 *	It also keeps the system MODE, which to-date includes:
