...
enum tgControllerState {		// command receive / execution states
	TG_STATE_READY_UNPROMPTED,	// ready for input; no prompt sent
	TG_STATE_READY_PROMPTED,	// ready for input; prompt has been sent
	TG_STATE_READING_COMMAND,	// not ready for input; reading a command
	TG_STATE_RUNNING_COMMAND,	// not ready for input; running a command
	TG_STATE_MAX				// NB: May be both running a command and reading 
								/// next one. Run state takes precedence
};
...
static void _tg_poll_active(uint8_t d)
{
	...
	if ((tg.status = xio_fget_ln(d, tg.dev[d].buf, tg.dev[d].len)) == TG_OK) {
		// throw an error if a completed line arrives but controller is not ready
		if (tg.state >= TG_STATE_READING_COMMAND) {
			tg.status = TG_BUFFER_FULL;
			printf_P(PSTR("COMMAND INPUT BUFFER OVERFLOW: %s\n"), tg.dev[d].buf);
			return;
		} 
