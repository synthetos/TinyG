...
struct fdXIO *fd_ptrs[FD_MAX];			// array of pointers to IO structs
struct fdXIO fd_usb, fd_rs485;			// pre-allocate FD structs
 ...
 */
const uint8_t fdes[] PROGMEM = 	// STANDARD DEVICES
				{ 1, 1, 1,					// stdin, stdout, stderr
				  ...
				  0, 0,						// ADCA, ADCB
				  0, 0,						// DACA, DACB
				  0, 0, 3, 0, 0, 			// SRAM, EEPROM, PGM, TABLE, BOOT
				  				// DERIVED DEVICES
				  1, 2,						// USB, RS485, 
				  0, 0, 					// ENCODERS, BRIDGE
...
void xio_init(void)
{ 
	fd_ptrs[FD_NULL] = NULL;				// always /dev/null
	fd_ptrs[FD_USB] = &fd_usb;
	fd_ptrs[FD_RS485] = &fd_rs485;
//	fd_ptrs[FD_PGM] = &fd_pgm;
...
/* xio_open() - open a device such as a serial port or program memory "file" handle 
	...
	returns	File descriptor for device (fd)
			...
			Error returns -1 and sets errno
				...
				EBADF	bad device number for the xio_open_DEVICE() call
...
//		case DEV_PROGMEM: return xio_open_pgm(dev, control);
//		case DEV_RS485: return _open_rs485(dev, control);
		default: errno = ENODEV; return XIO_ERR;
	}
	errno = EWTF;
...
//		case DEV_PROGMEM: return xio_control_pgm(fd_ptrs[fd], control, arg);
...
//		case DEV_PROGMEM: return (xio_read_pgm(fd_ptrs[fd], buf, size));
		default: { errno = EBADF; return XIO_ERR; }
	}
	errno = EWTF;
	return (-1);		// should never execute this return
}
...
//		case DEV_PROGMEM: return (xio_write_pgm(fd_ptrs[fd], buf, size));
...
{
	switch (fd) {
		case FD_USB: return(xio_getc_usb(fd_ptrs[fd]));
//		case DEV_PROGMEM: return(xio_getc_pgm(fd_ptrs[fd]));
		default: { errno = EBADF; return XIO_ERR; }
	}
	errno = EWTF;
	return (-1);		// should never execute this return
...
{
	switch (fd) {
		case FD_USB: return(xio_putc_usb(fd_ptrs[fd],c));
//		case DEV_PROGMEM: return(xio_putc_pgm(fd_ptrs[fd],c));
		default: { errno = EBADF; return XIO_ERR; }
	}
	errno = EWTF;
	return (-1);		// should never execute this return
...
/*	
 * xio_readsz() - read from device by size (byte count) (generic)
 *
 *	fd		file descriptor returned from xio_open()
 *	buf		Address of SRAM buffer to read into
 *	size	Number of characters to read (See Read/Write Modes for more details)
 *			  0		Returns zero and no other results
 *			  1-N	Read 1-N chars
 *
 ...
 *			 -1 	Error returns -1 and sets errno:
 *
 *			 EBADF	fd_ptr is not valid file descriptor or is not open for reading.
 *			 EAGAIN	Non-blocking I/O has been selected using RDNONBLOCK and
 *					 no data was immediately available for reading.
 *			 EFBIG	String requested is too big and was not read
 *			 EMSGSIZE String exceeded maximum size and was read up to the max
...
int16_t xio_readsz(uint8_t fd, char *buf, int16_t size)
{
	struct fdXIO *f
	uint16_t i = 0; 							// output buffer index

	if (size == 0) 								// special case of 0
		{ return (0); 
	}
	if (size > f->read_size_max) {  			// too big a request made
		errno = EFBIG;
		return XIO_ERR; 
	}
	while (TRUE) {
		if ((buf[i++] = f->xio_getc(f)) == -1) {
			return XIO_ERR;						// passes errno through			
		}
...
/*	
 * xio_readln() - read a line terminated by a delimiter from device (generic)
 *
 *	fd		file descriptor returned from xio_open()
 *	buf		Address of SRAM buffer to read into
 *
...
int16_t xio_readln(uint8_t fd, char *buf)
{
	...
	struct fdXIO *f
	uint16_t i = 0; 							// output buffer index
	uint16_t size;								// checks for buffer overrun

	size = f->read_size_max;					// sets max size or NO_LIMIT
	while (TRUE) {
		if ((c = f->xio_getc(f)) == EOF) {
			return XIO_ERR;						// passes errno through			
		}
		buf[i++] = c;
		...
		if (c == NUL) {							// read NUL - exit condition
			return (i);
		}
	}
	errno = EWTF;		// shouldn't ever get here.
...
/*
 * xio_readstr() - read a line terminated by NUL from device (generic)
 *
 *	fd		file descriptor returned from xio_open()
 *	buf		Address of SRAM buffer to read into
 *
...
/* 
 *	xio_writesz() - write to device by size (byte count) (generic)
 *
 *	fd		file descriptor returned from xio_open()
 *	buf		address of SRAM buffer to write from (source buffer)
...
int16_t xio_writesz(uint8_t fd, const char *buf, int16_t size)
{
	char c;										// character temp
	struct fdXIO *f
	uint16_t i = 0; 							// source buffer index

	if (size == 0) 								// special case of 0
		{ return (0); 
	}
	if (size > f->write_size_max) {  			// too big a request made
		errno = EFBIG;
		return XIO_ERR; 
	}
	while (TRUE) {
		if (--size == 0) {						// size is complete
			return (i);
		}
		if (f->xio_putc(f,buf[i++]) == EOF) {	// write char to output
			return XIO_ERR;						// passes errno through
		}
	}
...
/* 
 *	xio_writeln() - write a line terminated by a delimiter to device (generic)
 *
 *	fd		file descriptor returned from xio_open()
 *	buf		address of SRAM buffer to write from (source buffer)
 *
...
int16_t xio_writeln(uint8_t fd, const char *buf)
{
	...
	size = f->write_size_max;					// sets max size or NO_LIMIT
	while (TRUE) {
		if (size != NO_LIMIT) {					// using 2 lines forces eval order
			if (--size == 0) {					// test if size is complete
		...
		if (f->xio_putc(f,c) == EOF) {			// write char to output channel
			return XIO_ERR;						// passes errno through
		}
		if ((c == '\r') || (c == '\n') || (c == ';')) { 
			return (i);
...
/* 
 *	xio_writestr() - write a line terminated by a NUL to device (generic)
 *
 *	fd		file descriptor returned from xio_open()
 *	buf		address of SRAM buffer to write from (source buffer)
 *
...
/* 
 *	xio_writestr_P() - write a PGM line terminated by a NUL to device (generic)
 *
 *	fd		file descriptor returned from xio_open()
 *	buf		address of PROGMEM buffer to write from (source buffer)
 *
 *	returns  1-N	Number of characters written
 *			 -1 	Error returns -1 and sets errno:
 *
