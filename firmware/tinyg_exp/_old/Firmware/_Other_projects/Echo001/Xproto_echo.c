/*
	Xproto_echo
	test program to echo characters through serial port using RS485 connection
...
/* UNCOMMENT TO TEST CLOCK SPEED ON PORTE-bit7
...
*/
  ...
  PORTA.DIR = 0xFF;				// set PORTB:0 output for LED
  PORTD.DIR = 0xFF;				// set PORTB:0 output for LED
  PORTE.DIR = 0xFF;				// set PORTB:0 output for LED
  PORTF.DIR = 0xFF;				// set PORTB:0 output for LED

  while(1)
  {
	data=UsartReadChar(); 		// read char
	UsartWriteChar(data); 		// write char
	PORTA.OUTTGL = 0xFF; 		// toggle LEDs
	PORTD.OUTTGL = 0xFF; 		// toggle LEDs
	PORTE.OUTTGL = 0xFF; 		// toggle LEDs
	PORTF.OUTTGL = 0xFF; 		// toggle LEDs
  };
};
