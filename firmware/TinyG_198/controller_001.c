...
#define LINE_BUFFER_SIZE 80				// text lines
...
struct tgInput {
	...
	void (*poll_func)(void);			// polling function for scanning readln
	int (*read_func)(void);				// line reader function - collect a text line
	int (*line_func)(char * buf);		// line dispatch function - interpreter
	int (*sig_func)(uint8_t sig);		// signal handler function
	char buf[LINE_BUFFER_SIZE];			// line buffer for line reader
};
...
//static void _tg_poll_primary(struct tgInput *in);
...
static void _tg_poll_primary(void);
static void _tg_poll_secondary(void);
...
void tg_init() 
{
	...
	tg.input[TG_SRC_USB].state = TG_INPUT_PRIMARY;
	tg.input[TG_SRC_USB].status = 0;
	tg.input[TG_SRC_USB].len = sizeof(tg.input[TG_SRC_USB].buf);
	tg.input[TG_SRC_USB].poll_func = &_tg_poll_primary;
	tg.input[TG_SRC_USB].read_func = (fptr_int)&xio_usb_readln;
	xio_usb_control(XIO_LINE_FUNC, (int)&xio_null_line);
	xio_usb_control(XIO_SIG_FUNC, (int)&tg_signal);

	// initialize AUX input (arduino)
	/*
	tg.input[TG_SRC_AUX].state = TG_INPUT_SECONDARY;
	tg.input[TG_SRC_AUX].poll_func = &_tg_poll_secondary;
	tg.input[TG_SRC_AUX].line_func = &tg_null_line;
	tg.input[TG_SRC_AUX].read_func = (fptr_int)&xio_aux_readln;
	...
	*/
	...
	/*
	tg.input[TG_SRC_RS485].state = TG_INPUT_SECONDARY;
	tg.input[TG_SRC_RS485].poll_func = &_tg_poll_secondary;
	tg.input[TG_SRC_RS485].line_func = &tg_null_line;
	tg.input[TG_SRC_RS485].read_func = (fptr_int)&xio_rs485_readln;
	...
	*/
	...
	tg.input[TG_SRC_PGM].state = TG_INPUT_INACTIVE;
	tg.input[TG_SRC_PGM].poll_func = &_tg_poll_secondary;
	tg.input[TG_SRC_PGM].line_func = &tg_null_line;
	tg.input[TG_SRC_PGM].read_func = (fptr_int)&xio_pgm_readln;
	tg.input[TG_SRC_PGM].sig_func = (fptr_int)&xio_null_signal;
...
void _tg_set_source(uint8_t src)
{
	...
	for (uint8_t i = 0; i < TG_SRC_MAX; i++) {
		...
		tg.input[i].line_func = &tg_null_line;
		tg.input[i].len = sizeof(tg.input[i].buf);
	}
	tg.input[TG_SRC_PGM].state = TG_INPUT_INACTIVE;	// this one's an exception


	tg.input[TG_SRC_USB].line_func = &tg_parser;

	// bind null line handlers to all sources
	...
	_tg_set_stdout(TG_SRC_USB);			// set stdout/stderr for printf's
...
{
	/* Send a prompt - 
	 * but only of the controller is ready for one and it's not in file mode.
	 */
	_tg_prompt();
	...
	/* Perform a non-blocking read_line from the USB port.
	 * Dispatches to tg_parser on completed line if USB is the command SRC.
	 */
	tg.status = xio_usb_readln();
	switch (tg.status) {
