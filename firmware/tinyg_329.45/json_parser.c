...
static cmdObj js[JSON_CMD_ARRAY_SIZE]; // js[0] is the root object

static uint8_t _json_parser(char *str);
static uint8_t _get_nv_pair(cmdObj *cmd, char **pstr, int8_t *level);
...
{
//	fprintf_P(stderr, PSTR("Decoding JSON %s\n"), str);
	uint8_t status = _json_parser(str);
	_make_json_response(&js[0], str, JSON_STRING_LEN);
	return (status);
}
...
static uint8_t _json_parser(char *str)
{
	...
	cmdObj *cmd = &js[0];						// point at first struct in the array

	ritorno(_normalize_json_string(str, JSON_STRING_LEN));// test and normalize input string
	for (i=0; i<JSON_CMD_ARRAY_SIZE; i++) {		// collect the NV pairs into structs
		ritorno(_get_nv_pair(&js[i], &str, &nesting_level));
		if (js[i].nx == NULL) break;			// NULL means it's the last (or only) NV pair
	}
	for (i=0; i<JSON_CMD_ARRAY_SIZE; i++) {		// take action on the parsed string
		if (cmd->value_type == VALUE_TYPE_NULL){// means GET the value
			...
			cmd_write_NVM(cmd->index, cmd);		// persist the value to NVM
		}
		if (cmd->nx != NULL) { cmd = cmd->nx;} else break;
	}
	return (TG_OK);
}
...
static uint8_t _get_nv_pair(struct cmdObject *cmd, char **pstr, int8_t *level) 
{
	char *tmp;
	char terminators[] = {"},"};
	...
	*pstr = ++tmp;
	...
	if (**pstr == 'n') { 
		...
		cmd->value = false;						// (technically not necessary)
	} else if (**pstr == 't') { 
 ...
 *
 *	Validate that the string is properly terminated, convert to lower case,
 *	and remove all whitespace, with the exception of gcode comments.
 */
...
static uint8_t _normalize_json_string(char *str, uint16_t size)
{
	...
	if (strlen(str) > size) {	// that's bad
		return (TG_INPUT_EXCEEDS_MAX_LENGTH);
	}
	while ((c = tolower(str[i++])) != NUL) {
		if (c <= ' ') continue;				// toss controls & whitespace
		if (c == DEL) continue;				// toss DELETEs (0x7F)
		if (c == '(') {						// special handling for Gcode comments
			str[j++] = c;
			while ((c = str[i++]) != NUL) {	// move but do not strip or uppercase
				if (c != ')') { 
					str[j++] = c;
					continue;
				}
...
static uint8_t _make_json_response(cmdObj *cmd, char *str, uint16_t size)
{
	...
	// iterate over JSON object array to make NV responses
	for (uint8_t i=0; i<JSON_CMD_ARRAY_SIZE; i++) {
		...
		// non-error response cases
			...
			if (cmd->value_type == VALUE_TYPE_NULL) {
				...
				str += sprintf(str, "%0.4f", cmd->value);
			} else if (cmd->value_type == VALUE_TYPE_STRING) {
				str += sprintf(str, "\"%s\"", cmd->vstring);
			} 
			str += sprintf(str, ",\"stat\":%d}", cmd->status);	// status NV pair & termination
		}
