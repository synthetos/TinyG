...
static void _return(uint8_t status, char *buf);
...
void tg_init(uint8_t default_src) 
{
	...
	tg.communications_mode = TG_COMMAND_LINE_MODE;
}
...
//	fprintf_P(stderr, PSTR("Executing command %s\n"), buf);
	switch (toupper(tg.buf[0])) {
...
//			_return(cfg_config_parser(tg.buf,true,false),tg.buf);
			_return(cfg_config_parser(tg.buf),tg.buf);
			break;
		}

		case '{': { 							// JSON input
			tg.communications_mode = TG_JSON_MODE;
			_return(js_json_parser(tg.buf),tg.buf); 
			break;
		}

	//	case 'G': case 'M': case 'N': case 'F': 
	//	case 'Q': case '(': case '%': case '\\':// gcode triggers
		default: {								// Gcode (anything else)
			_return(gc_gcode_parser(tg.buf),tg.buf);
		}
	}
...
void _return(uint8_t status, char *buf)
{
	...
	if (tg.communications_mode == TG_COMMAND_LINE_MODE) {
