 ...
 */
/*
 *	Notes: Gcode parameters are kept in the EEPROM records as G words
 *	(e.g. G20), and in configs in the following internal forms:
 *
 *		gcode_set_plane			CANON_PLANE_XY enum (0 - 2)
 *		gcode_inches_mode		1 = inches, 0 = mm
 *		gcode_homing_mode		1 = home on power up, 0 = don't
 *		gcode_spindle_speed		double
 *		gcode_feed_rate			double
 *		gcode_tool				uint8_t
 */
/* TODO:
	- help screen
...
int cfg_parse(char *block)
{
	...
	switch (cp.param) {
		case CFG_GCODE_PLANE:		cfg.gcode_plane = (uint8_t)cp.value; 
									cm_select_plane(cfg.gcode_plane); 
									break;

		case CFG_GCODE_UNITS:	  	cfg.gcode_units = (uint8_t)cp.value;
									cm_use_length_units(cfg.gcode_units); 
									break;

		case CFG_GCODE_HOMING_MODE:	cfg.gcode_homing_mode = (uint8_t)cp.value; 
									break;

		case CFG_GCODE_FEED_RATE:	cfg.gcode_feed_rate = cp.value;
									cm_set_feed_rate(cfg.gcode_feed_rate); 
									break;

		case CFG_GCODE_SPINDLE_SPEED: cfg.gcode_spindle_speed = cp.value;
									cm_set_spindle_speed(cfg.gcode_spindle_speed); 
									break;

		case CFG_GCODE_TOOL:		cfg.gcode_tool = (uint8_t)cp.value;
									cm_select_tool(cfg.gcode_tool); 
									break;
		...
		case CFG_TRAVEL_WARN: 		CFG(cp.axis).travel_warning = cp.value; break;
		case CFG_TRAVEL_PER_REV: 	CFG(cp.axis).travel_per_rev = cp.value; break;
...
inline void _cfg_computed() 
{
	for (uint8_t i=X; i<=A; i++) {
		...
		cfg.a[i].steps_per_unit = (360 / (cfg.a[i].degree_per_step / 
										  cfg.a[i].microstep)) / 
										  cfg.a[i].travel_per_rev;

		// max_seek_rate = 60 * seek_steps_sec / (360/degree_per_step/travel_per_rev)
		cfg.a[i].max_seek_rate = ((60 * (double)cfg.a[i].seek_steps_sec) /
							 	  (360/cfg.a[i].degree_per_step/cfg.a[i].travel_per_rev));

		// max_feed_rate = 60 * feed_steps_sec / (360/degree_per_step/travel_per_rev)
		cfg.a[i].max_feed_rate = ((60 * (double)cfg.a[i].feed_steps_sec) /
								  (360/cfg.a[i].degree_per_step/cfg.a[i].travel_per_rev));
	}
}
 ...
 *	Also displays computed values at end
 */
...
char cfs04[] PROGMEM = "  Gcode: {T} Tool                 %1.0f";
char cfs05[] PROGMEM = "  Gcode: {F} Feed rate     %8.2f";
char cfs06[] PROGMEM = "  Gcode: {S} Spindle speed %8.2f";
...
char cfs16[] PROGMEM = "  %c axis - Trav/rev (mm, deg) %5.2f";
char cfs17[] PROGMEM = "  %c axis - Idle mode          %5.0f";
char cfs18[] PROGMEM = "  %c axis - Limit switches     %5.0f";
char cfs19[] PROGMEM = "Derived: max seek rate, max feed rate (change steps / sec to set)\n";
char cfs20[] PROGMEM = "  %c axis - %7.2f        %7.2f mm/min\n";
char cfs21[] PROGMEM = "  %c axis - %7.2f        %7.2f in/min\n";
char cfs22[] PROGMEM = "  %c axis - %7.2f        %7.2f degrees/min\n";

#define COMPUTED_VALUES 19
...
//		printf_P((PGM_P)pgm_read_word(&cfgShow[COMPUTED_VALUES+2]), axis, CFG(i).max_feed_rate);
//		printf_P((PGM_P)pgm_read_word(&cfgShow[COMPUTED_VALUES+3]), axis, CFG(i).steps_per_unit);
...
void _cfg_print_config_record(char *record)
{
	...
	if ((i = _cfg_tokenize_config_record())) {	// populates cp.struct used below
...
char cff04[] PROGMEM = "T%1.0f";	// T Tool
char cff05[] PROGMEM = "F%1.3f";	// F Feed rate
char cff06[] PROGMEM = "S%1.2f";	// S Spindle speed
...
char cff16[] PROGMEM = "%cRE%1.3f";	// Travel per REvolution (mm, degrees)
...
void _cfg_write_profile_to_NVM(uint16_t base_addr)
{
	...
	// Gcode settings
	_cfg_write_as_record_to_NVM(base_addr, CFG_GCODE_PLANE, 0, (17 + cfg.gcode_plane)); // G17
	_cfg_write_as_record_to_NVM(base_addr, CFG_GCODE_UNITS, 0, (20 + cfg.gcode_units));	// G20
	_cfg_write_as_record_to_NVM(base_addr, CFG_GCODE_HOMING_MODE, 0, 28);	// G28
...
void _cfg_load_hardwired_settings()
{
	...
	cfg.gcode_homing_mode = GCODE_HOMING_MODE;
	...
	cfg.a[X].travel_per_rev = X_TRAVEL_PER_REVOLUTION;
	cfg.a[Y].travel_per_rev = Y_TRAVEL_PER_REVOLUTION;
	cfg.a[Z].travel_per_rev = Z_TRAVEL_PER_REVOLUTION;
	cfg.a[A].travel_per_rev = A_TRAVEL_PER_REVOLUTION;
