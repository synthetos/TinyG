 ...
 *
 * Note: The method of testing for buffer full then writing the buffer as 
 *	a separate, non-atomic operation works as long as there is no 
 *	pre-emption that could invalidate the full/not-full result. 
 *	As this project is currently coded there is no pre-emption possible 
 *	in this critical region - i.e. there's no way somebody else can get 
 *	in there and write to the line buffer between the not-full result and 
 *	the subsequent write. Be careful about changing this condition.
 */
