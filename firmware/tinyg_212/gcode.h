...
struct GCodeModel {					// Gcode model- meaning depends on context
	...
	double position[4];				// X, Y, Z, A - meaning depends on context
	double target[4]; 				// X, Y, Z, A - where the move should go
...
*/
/*---- Notes on Feed Rate and Feed Rate Modes ----

	Ref: NIST RS274NGC_3 section 3.5.19

	To make sure you adjust for units, degrees and other transformations:
	- always use cm_set_feed_rate() to set the feed rate in the gcode model
	- always use cm_set_feed_rate_mode() to set the feed rate mode.
	
	If inverse feed rate mode is active you don't know the feed rate until 
	you know the length of the line. The canonical machine traverse, linear
	feed and arc feed figure this out and pass the correct feed rate to the 
	underlying motion control layer.

*/
/*---- Notes on the A axis (rotational axis) ----
*/
