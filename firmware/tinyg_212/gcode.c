 ...
 * It is adapted from Simen Svale Skogsrud's grbl, ported to the xmega
 * series chips, and has a canonical machine implmented along the lines
 * of the NIST / Kramer spec.
 *
 ...
 *	 changed in gn during the block read. 
 */
static struct GCodeParser gp;		// gcode parser variables
//static struct GCodeModel gm;		// (static to canonical machine.c)
...
static int _gc_read_gcode_block(char *line);	// Parse the block into structs
...
/*
 * gc_gcode_parser() - read and execute a block (line) of gcode
 */
...
uint8_t gc_gcode_parser(char *block)
{
	...
	if (_gc_read_gcode_block(block)) { // parse & exec block or fail trying
...
/*
 * _gc_read_gcode_block() - parses one line of NULL terminated G-Code. 
 *
...
int _gc_read_gcode_block(char *buf) 
{
...
int _gc_execute_gcode_block() 
{
	...
	if (gf.set_origin_mode) {
		gn.next_action = NEXT_ACTION_OFFSET_COORDINATES; 	// cancels motion
		if ((gp.status = cm_set_origin_offsets(
						 gn.target[X], gn.target[Y], gn.target[Z], gn.target[A]))) {
			return (gp.status);								// error return
		}
	}

	// CUTOUT: return now if no target has been set
	...
	if ((gn.next_action == NEXT_ACTION_MOTION) && 
		...
		gp.status = cm_straight_traverse(gn.target[X], gn.target[Y], gn.target[Z], gn.target[A]);
		return (gp.status);
	}
	...
	if ((gn.next_action == NEXT_ACTION_MOTION) && 
		...
		gp.status = cm_straight_feed(gn.target[X], gn.target[Y], gn.target[Z], gn.target[A]);
		return (gp.status);
	}
	...
	if ((gn.next_action == NEXT_ACTION_MOTION) &&
	   ((gn.motion_mode == MOTION_MODE_CW_ARC) || 
		...
		gp.status = cm_arc_feed(gn.target[X], gn.target[Y], gn.target[Z], gn.target[A],
