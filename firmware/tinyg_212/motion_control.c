 ...
 *  Organized variables into static structs
...
struct MotionControlState {			// robot pos & vars used by lines and arcs
	...
	int32_t position[4];    		// current position of tool in abs steps
	int32_t target[4];				// target position of tool in abs steps
	int32_t steps[4];				// target line in relative steps
...
struct MotionControlArc {			// vars used by arc generation & continue
	...
	double dtarget[4];				// segment target in floating point
...
int mc_set_position(double x, double y, double z, double a)
{
	...
	mc.position[A] = lround(z*CFG(A).steps_per_unit); 
	return (TG_OK);
}
...
int mc_line(double x, double y, double z, double a, double feed_rate)
{
	...
	// Ask Phythagoras to estimate how many mm next move will take (thanks Simen!)
 	mc.mm_of_travel = sqrt( square(mc.steps[X]/CFG(X).steps_per_unit) + 
							square(mc.steps[Y]/CFG(Y).steps_per_unit) + 
							square(mc.steps[Z]/CFG(Z).steps_per_unit));
...
int mc_arc(double theta, 			// starting angle
		   ...
		   uint8_t invert_feed_rate)// TRUE = inverse_feed_rate_mode
{
	...
	// convert inverse feed rate to normal-time feed rate
	if (ma.invert_feed_rate) { 
		ma.feed_rate = ma.mm_of_travel / ma.feed_rate; 
	}

	ma.theta_per_segment = ma.angular_travel/ma.segments;
	ma.linear_per_segment = ma.linear_travel/ma.segments;
	ma.center_x = (mc.position[ma.axis_1]/CFG(ma.axis_1).steps_per_unit)-sin(ma.theta)*ma.radius;
	ma.center_y = (mc.position[ma.axis_2]/CFG(ma.axis_2).steps_per_unit)-cos(ma.theta)*ma.radius;

  	// 	A vector to track the end point of each segment. Initialize linear axis
	ma.dtarget[ma.axis_linear] = mc.position[ma.axis_linear]/CFG(Z).steps_per_unit;
...
int mc_arc_continue() 
{
	...
	while (ma.segment_counter <= ma.segments) {
		...
		mc_line(ma.dtarget[X], ma.dtarget[Y], ma.dtarget[Z], ma.dtarget[A], ma.feed_rate);
  	}
