...
/*--- setter and getters for canonical machine / model state ---*/
...
uint8_t cm_set_origin_offsets(double x, double y, double z, double a);// G92
uint8_t cm_use_length_units(uint8_t inches_mode);			// G20, G21
uint8_t cm_set_distance_mode(uint8_t absolute_mode);		// G90, G91
uint8_t cm_set_absolute_override(uint8_t absolute_override);// G53

uint8_t cm_set_traverse_rate(double seek_rate);				// (no code)
uint8_t cm_straight_traverse(double x, double y, double z, double a); // G0

uint8_t cm_set_feed_rate(double feed_rate);					// F parameter
uint8_t cm_set_inverse_feed_rate_mode(uint8_t inverse_feed_rate_mode);
uint8_t cm_set_motion_control_mode(uint8_t motion_control_mode);// G61, G61.1, G64
uint8_t cm_dwell(double seconds);							// G4, P parameter
uint8_t cm_straight_feed(double x, double y, double z, double a);		// G1
...
uint8_t cm_arc_feed(double x, double y, double z, double a,	// G2, G3
