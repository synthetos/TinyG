...
struct cfgStructAxis {
	...
	double travel_per_rev;		// mm or deg of travel per motor revolution
	double travel_max;			// m of travel max in N dimension (ex: 400)
	double travel_warning;		// point to warn of travel violation
	double steps_per_unit;		// steps (usteps)/mm or deg of travel (COMPUTED)
...
struct cfgStructGlobal {
	...
	uint8_t gcode_homing_mode;	// 0=off, 1=power-on (G28)
	uint8_t gcode_tool;
