...
//#define AXES 6		// use this when B abd C axes are added

enum tgAxisNum {		// define axis numbers & array indexes
		X,
		Y,
		Z,
		A,
		B,				// reserved for B and C axes (unimplemented)
		C
};
...
typedef int (*fptr_int_uint8)(uint8_t s);// returns int, unit8_t arg (sig handler) 
typedef int (*fptr_int_char_p) (char *b);// returns int, char pointer (line handler)

/*
 * Various compile-time switches
 */

#ifdef __SLAVE_MODE
#define DEFAULT_SOURCE XIO_DEV_RS485	// input from RS485 in slave mode
#else
#define DEFAULT_SOURCE XIO_DEV_USB		// input from USB in any other mode
#endif
